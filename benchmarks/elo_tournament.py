...
class ELOTournament:
    ...
    def calculate_elo_change(self, rating_a: float, rating_b: float, result: BattleResult, k_factor: float) -> Tuple[float, float]:
    ...
    def determine_battle_winner(self, metrics_a: Dict, metrics_b: Dict, test_type: str) -> Tuple[Optional[str], BattleResult]:
        ...
        quality_a = self._quality_score(metrics_a.get("quality", "excellent"))  # Default to excellent if not provided
        quality_b = self._quality_score(metrics_b.get("quality", "excellent"))
        
        # TPS-focused scoring: 85% TPS, 15% quality (heavily favor TPS)
        score_a = (tps_a * 0.85) + (quality_a * 50 * 0.15)  # Scale quality to match TPS range
        score_b = (tps_b * 0.85) + (quality_b * 50 * 0.15)
        
        # Smaller threshold for more decisive TPS-based wins
        diff_threshold = max(score_a, score_b) * 0.03  # 3% difference needed for decisive win
    ...
    def select_battle_pairs(self) -> List[Tuple[str, str]]:
        ...
        while len(available_models) >= 2 and len(pairs) < self.battles_per_round:
            ...
            for model in available_models:
                ...
                score = rating_diff + (recent_battles * 50)  # Penalty for recent battles
    ...
    def _count_recent_battles(self, model_a: str, model_b: str, recent_rounds: int = 5) -> int:
        ...
        for battle in self.battle_history:
            if battle.round_num >= recent_threshold:
                if (battle.model_a == model_a and battle.model_b == model_b) or \
                   (battle.model_a == model_b and battle.model_b == model_a):
                    count += 1
        
        return count
    
    async def execute_battle(self, model_a: str, model_b: str, test_config: Dict, benchmark_runner) -> Optional[BattleRecord]:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                tasks = [
                    benchmark_runner._execute_enhanced_test("openai", model_a, test_config, True),
                    benchmark_runner._execute_enhanced_test("openai", model_b, test_config, True)
                ]
                
                results = await asyncio.gather(*tasks, return_exceptions=True)
                
                # Handle exceptions
                metrics_a = results[0] if not isinstance(results[0], Exception) else {"success": False, "error": str(results[0]), "model": model_a}
                metrics_b = results[1] if not isinstance(results[1], Exception) else {"success": False, "error": str(results[1]), "model": model_b}
                
                # Determine winner
                winner, battle_result = self.determine_battle_winner(metrics_a, metrics_b, test_config.get("test_type", "unknown"))
                ...
                # Update model stats
                self._update_model_stats(model_a, metrics_a, battle_result, elo_change_a)
                self._update_model_stats(model_b, metrics_b, BattleResult(1.0 - battle_result.value), elo_change_b)
                ...
                if attempt == self.max_retries - 1:
                    logger.error(f"Battle failed after {self.max_retries} attempts: {model_a} vs {model_b}")
                    return None
        
        return None
    
    def _update_model_stats(self, model_name: str, metrics: Dict, result: BattleResult, elo_change: float):
        ...
        if metrics.get("success", False):
            ...
            if response_time > 0:
                stats.avg_response_time = ((stats.avg_response_time * (stats.games_played - 1)) + response_time) / stats.games_played
    ...
    async def run_tournament_round(self, benchmark_runner) -> List[BattleRecord]:
        ...
        for battle in battle_results:
            if battle:
                ...
                print(f"      ELO: {battle.model_a} {battle.elo_change_a:+.1f}, {battle.model_b} {battle.elo_change_b:+.1f}")
    ...
    def display_leaderboard(self):
        ...
        print("=" * 110)
        print(f"{'Rank':<5} {'Model':<20} {'Best TPS':<10} {'ELO':<8} {'W-L-D':<10} {'Win%':<8} {'Avg TPS':<10} {'Reliability':<12}")
        print("-" * 110)
        ...
        for rank, (model_name, stats) in enumerate(leaderboard, 1):
            ...
            if abs(stats.current_streak) >= 3:
                streak_indicator = f" 🔥{stats.current_streak}" if stats.current_streak > 0 else f" ❄️{stats.current_streak}"
            
            print(f"{rank_emoji:<5} {model_name:<20} {best_tps:<10} {stats.elo_rating:.0f}    {record:<10} {win_pct:<8} {avg_tps:<10} {reliability:<12} {streak_indicator}")
    ...
    def generate_tournament_report(self) -> Dict[str, Any]:
        ...
        if len(leaderboard) > 0:
            ...
            report["insights"] = {
                ...
                "most_reliable": max(self.models.items(), key=lambda x: x[1].reliability_score)[0],
                "highest_tps": max(self.models.items(), key=lambda x: x[1].best_tps)[0],
                "most_active": max(self.models.items(), key=lambda x: x[1].games_played)[0],
...
class ELOBenchmarkRunner:
    ...
    async def run_elo_tournament(self, provider: str, rounds: int = 20):
        ...
        print(f"🏆 Victory: Highest ELO rating after all rounds\n")
...
async def run_example_tournament():
    ...
    parser.add_argument("--suite", choices=["quick", "standard", "lightning"], default="quick")
    parser.add_argument("--rounds", type=int, default=15, help="Number of tournament rounds")
    ...
    if args.provider == "openai":
        valid_openai_models = [
            "gpt-4.1","gpt-4.1-mini","gpt-4.1-nano", "gpt-4o-mini", "gpt-4o", "gpt-4-turbo", "gpt-3.5-turbo", 
    ...
    if args.suite == "quick":
        test_configs = [
            ...
            },
            {
                ...
                "messages": [{"role": "user", "content": "Solve: If A > B and B > C, what's the relationship between A and C?"}],
                "max_tokens": 200,
                "temperature": 0,
                "test_type": "reasoning"
            }
