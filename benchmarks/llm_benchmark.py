...
class BenchmarkConfig:
    ...
    expected_features: list[str] | None = None  # ["streaming", "tools", "json"]
    timeout: float = 60.0
...
class PerformanceMetrics:
    ...
    # Streaming metrics
    chunk_count: int = 0
    avg_chunk_interval: float | None = None
    streaming_duration: float | None = None
    ...
    def summary(self) -> str:
        ...
        parts = [f"✅ {self.total_response_time:.2f}s total"]
...
class LLMBenchmark:
        ...
        return [
            ...
            BenchmarkConfig(
                ...
                messages=[
                    {
                        "role": "user",
                        "content": "If a train travels 60 mph for 2 hours, then 80 mph for 1.5 hours, what's the total distance?",
                    }
                ],
            ...
            BenchmarkConfig(
                ...
                messages=[
                    {
                        "role": "user",
                        "content": "Explain the history of the internet in detail.",
                    }
                ],
                max_tokens=500,
            ...
            BenchmarkConfig(
                ...
                messages=[
                    {
                        "role": "user",
                        "content": "Return a JSON object with three programming languages and their primary use cases.",
                    }
                ],
            ...
            BenchmarkConfig(
                ...
                messages=[
                    {
                        "role": "user",
                        "content": "I'm thinking of a number between 1 and 10.",
                    },
                    {
                        "role": "assistant",
                        "content": "That sounds fun! I'd love to guess your number. Is it 7?",
                    },
            ...
            BenchmarkConfig(
                ...
                messages=[
                    {
                        "role": "user",
                        "content": "Count from 1 to 20, putting each number on a new line.",
                    }
                ],
    ...
    async def benchmark_provider_model(
        ...
        provider: str,
        model: str,
        test_configs: list[BenchmarkConfig] | None = None,
        ...
        try:
            ...
            if stream:
                ...
                async for chunk in client.create_completion(
                    config.messages, tools=config.tools, stream=True, **kwargs
                ):
                ...
                result = await asyncio.wait_for(
                    client.create_completion(
                        config.messages, tools=config.tools, stream=False, **kwargs
                    ),
            ...
            if metrics.response_length > 0 and metrics.total_response_time > 0:
                ...
                metrics.tokens_per_second = (
                    estimated_tokens / metrics.total_response_time
                )
    ...
    def generate_report(
        self,
        results: dict[str, list[PerformanceMetrics]] | None = None,
        ...
        all_successful = [
            m for metrics in results.values() for m in metrics if m.success
        ]

        if all_successful:
            avg_response_time = statistics.mean(
                [m.total_response_time for m in all_successful]
            )
            avg_first_token = statistics.mean(
                [m.first_token_latency for m in all_successful if m.first_token_latency]
            )
        ...
        for key, metrics in results.items():
            ...
            if successful:
                ...
                avg_first_token = (
                    statistics.mean(
                        [
                            m.first_token_latency
                            for m in successful
                            if m.first_token_latency
                        ]
                    )
                    or 0
                )
                avg_tokens_sec = (
                    statistics.mean(
                        [m.tokens_per_second for m in successful if m.tokens_per_second]
                    )
                    or 0
                )
        ...
        report_lines.extend(["## Detailed Results by Test", ""])
        ...
        for test_name, test_metrics in by_test.items():
            ...
            for metric in test_metrics:
                ...
                first_token = (
                    f"{metric.first_token_latency:.2f}s"
                    if metric.first_token_latency
                    else "-"
                )
        ...
        if all_successful:
            ...
            report_lines.append(
                f"- **Fastest Overall**: {fastest.provider}-{fastest.model} ({fastest.total_response_time:.2f}s)"
            )

            # Find best first token latency
            with_first_token = [m for m in all_successful if m.first_token_latency]
            if with_first_token:
                fastest_first_token = min(
                    with_first_token, key=lambda m: m.first_token_latency
                )
                report_lines.append(
                    f"- **Fastest First Token**: {fastest_first_token.provider}-{fastest_first_token.model} ({fastest_first_token.first_token_latency:.2f}s)"
                )
            ...
            if with_throughput:
                highest_throughput = max(
                    with_throughput, key=lambda m: m.tokens_per_second
                )
                report_lines.append(
                    f"- **Highest Throughput**: {highest_throughput.provider}-{highest_throughput.model} ({highest_throughput.tokens_per_second:.1f} tokens/sec)"
                )
...
async def streaming_benchmark(provider_models: list[tuple[str, str]]):
    ...
    results = await benchmark.benchmark_multiple(
        provider_models, test_configs=streaming_tests, test_streaming=True
    )
