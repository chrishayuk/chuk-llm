...
class CompellingResultsDisplay:
    ...
    def _get_display_config(self, theme: DisplayTheme) -> dict[str, Any]:
        ...
        base_width = (
            4 + 25 + 12 + 10 + 10 + 15 + 6
        )  # rank + model + tps + time + token + status + spacing
    ...
    def update_model(
        ...
        big_successes = [
            r
            for r in self.test_results[model]
            if r["success"]
            and r.get("throughput")
            and r["token_count"] >= min_tokens_for_throughput
        ]
        if big_successes:
            res["avg_throughput"] = sum(r["throughput"] for r in big_successes) / len(
                big_successes
            )
    ...
    def _collect_model_results(self) -> list[ModelResult]:
        ...
        sorted_models = sorted(throughput_best.items(), key=_sort_key, reverse=True)
        ...
        for rank, (model, data) in enumerate(sorted_models, 1):
            ...
            results.append(
                ModelResult(
                    ...
                    status=self._get_status_for_model(model, successful, total_runs),
    ...
    def _get_status_for_model(
        self, model: str, successful_runs: int, total_runs: int
    ) -> str:
    ...
    def _print_leaderboard_table(
        self, results: list[ModelResult], highlight_model: str = None
    ):
    ...
    def _print_model_row(
        self, result: ModelResult, highlight_model: str, config: dict[str, Any]
    ):
        ...
        self._format_tps_fixed_clean(result.best_tps, tps_width)
        ...
        if config["show_progress"] and result.total_tests > 0:
            ...
            progress_text = (
                f"{result.tests_completed}/{result.total_tests} ({progress_pct:.0%})"
            )
        ...
        status_text = status_parts[1] if len(status_parts) > 1 else result.status
        ...
        if config["show_avg_tps"] and config["show_progress"]:
            print(
                f"{rank_display:<{rank_width}} {model_display} "
    ...
    def _format_tps_fixed_clean(self, tps: float | None, width: int) -> str:
    ...
    def _format_tps_fixed(self, tps: float | None, width: int) -> str:
    ...
    def _format_time_fixed(self, time_val: float | None, width: int) -> str:
    ...
    def _format_tps(self, tps: float | None) -> str:
    ...
    def _format_time(self, time_val: float | None) -> str:
    ...
    def _print_insights(self, results: list[ModelResult]):
        ...
        if results:
            ...
            if leader.model != self.current_leader:
                ...
                print(
                    f"👑 CURRENT LEADER: {leader.model} • {leader.best_tps:.0f} tok/s peak throughput"
                )
            ...
            if leader.best_time and leader.best_time < 2:
                insights.append("🏃 SUB-2S RESPONSE!")
            elif leader.first_token and leader.first_token < 0.5:
                insights.append("⚡ INSTANT FIRST TOKEN!")
            ...
            if len(results) > 1:
                runner_up = results[1]
                gap = (
                    (leader.best_tps - runner_up.best_tps) / runner_up.best_tps
                ) * 100
                if gap < 5:
                    print(
                        f"🔥 TIGHT RACE! {leader.model} leads {runner_up.model} by only {gap:.1f}%"
                    )
                elif gap > 50:
                    print(
                        f"🎯 DOMINATING! {leader.model} is {gap:.0f}% faster than {runner_up.model}"
                    )

        # Show excluded models summary
        all_models = set(self.test_results.keys())
        successful_models = {r.model for r in results}
        failed_models = all_models - successful_models

        if failed_models:
            print(
                f"⚠️ EXCLUDED FROM RANKINGS: {', '.join(sorted(failed_models))} (no successful runs)"
            )
    ...
    def show_final_battle_results(self):
        ...
        print(
            f"   ⏱️  Best First Token:       {winner.first_token:.2f}s"
        ...
        if consistent:
            print(
                f"   🎯 Consistency Champ: {consistent.model} ({consistent.success_rate:.0%} success)"
            )
    ...
    def _get_throughput_leader(self) -> dict[str, Any] | None:
...
def create_results_display(
    models: list[str], theme: str = "detailed"
) -> CompellingResultsDisplay:
