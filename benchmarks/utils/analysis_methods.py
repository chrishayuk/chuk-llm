...
def show_test_by_test_leaderboards(self):
    ...
    for test in tests:
            ...
            rows.append(
                {
                    ...
                    "runs": len(relevant),
                    "best_tps": max(
                        (r.get("throughput", 0) for r in relevant), default=0
                    ),
                }
            )
        ...
        # header
        print(
            f"{'Rank':<6}{'Model':<18}{'Best TPS':<12}{'Best Time':<10}{'Avg Time':<10}{'First Token':<12}{'Runs'}"
        )
        print("-" * 100)
        ...
        if len(rows) > 1:
            ...
            if win["best_tps"] > 0:
                if runner["best_tps"] > 0:
                    ...
                    insights.append(
                        f"🎯 {win['model']} dominates with {win['best_tps']:.0f} tok/s"
                    )
    ...
    if time_winners:
        ...
        print(
            f"⚡ SPEED DEMON: {speed_champ} ({speed_wins}/{len(tests)} fastest times)"
        )
...
def show_comprehensive_analysis(self):
    ...
    print("=" * 110)
    print(
        "📋 Statistics use metrics from each model's best run per test for fair comparison"
    )
    ...
    for model, runs in self.test_results.items():
        ...
        for r in runs:
            ...
            if t not in best_runs:
                ...
                if new_tps > current_tps or (
                    new_tps == current_tps
                    and r["response_time"] < best_runs[t]["response_time"]
                ):
                    best_runs[t] = r
        ...
        stats[model] = {
            ...
            "cv": (statistics.stdev(resp_times) / statistics.mean(resp_times))
            if len(resp_times) > 1
            else 0,
    ...
    print("-" * 130)
    print(
        f"{'Model':<18}{'Success':<9}{'Best TPS':<10}{'Avg TPS':<10}{'Best Time':<10}{'Avg Time':<10}{'Std Dev':<9}{'Best Tok':<10}"
    )
    print("-" * 130)

    # Sort by average TPS (descending), then by average time (ascending)
    for model, s in sorted(
        stats.items(), key=lambda x: (-(x[1]["avg_tps"] or 0), x[1]["avg_time"])
    ):
        ...
        print(
            f"{model:<18}{s['success']:.0%}      {best_tps_disp:<10}{avg_tps_disp:<10}"
    ...
    if stats:
        ...
        print(
            f"🥇 Peak Throughput: {best_peak_tps[0]} ({best_peak_tps[1]['best_tps']:.0f} tok/s)"
            if best_peak_tps
            else "🥇 Peak Throughput: None"
        )
        print(
            f"📈 Best Average TPS: {best_avg_tps[0]} ({best_avg_tps[1]['avg_tps']:.0f} tok/s)"
            if best_avg_tps
            else "📈 Best Average TPS: None"
        )
        print(
            f"⚡ Best Average Time: {best_avg_time[0]} ({best_avg_time[1]['avg_time']:.2f}s)"
        )
        print(
            f"🎯 Most Consistent: {most_consistent[0]} (CV: {most_consistent[1]['cv']:.1%})"
        )
        if fastest_token:
            print(
                f"🚀 Fastest First Token: {fastest_token[0]} ({fastest_token[1]['best_token']:.2f}s)"
            )
    ...
    if stats:
        ...
        if tps_models:
            ...
            for model, s in stats.items():
                if (
                    s.get("avg_tps") and s["cv"] < 1.0
                ):  # Has TPS data and reasonable consistency
                    ...
                    balanced_score = (tps_score * 0.7) + (
                        consistency_score * 0.3
                    )  # 70% TPS, 30% consistency
...
CompellingResultsDisplay._show_test_by_test_leaderboards = (
    show_test_by_test_leaderboards
)
