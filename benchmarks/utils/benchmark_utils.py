...
class StreamMetrics(NamedTuple):
    ...
    token_count: int
    chunk_count: int
    avg_chunk_interval: Optional[float]
...
class ImprovedTokenCounter:
    ...
    def approx_tokens(text: str) -> int:
        ...
        affixes = len(re.findall(r'\b(?:un|re|pre|de|over|under|out)\w+', text, re.IGNORECASE))
...
class StreamingMetrics:
    ...
    def calculate_sustained_tps(
        ...
        if len(timeline) < min_points:
            logger.debug(f"Insufficient points for sustained TPS: {len(timeline)} < {min_points}")
            return None
        ...
        times, tokens = zip(*tail_timeline)
    ...
    def analyze_stream(
        cls,
        chunk_timeline: List[Tuple[float, str]],
        model: str
    ) -> StreamMetrics:
        """
...
class BenchmarkValidator:
    ...
    def validate_response_quality(
        ...
        if token_count < expected_min_tokens:
            if test_type in ["creative", "math", "reasoning"]:
                issues.append(f"Response too short: {token_count} tokens < {expected_min_tokens}")
                if token_count < 20:
        ...
        if test_type == "creative":
            ...
            math_indicators = ['answer', 'solution', 'result', 'therefore', 'thus', '=', 'calculate']
    ...
    def validate_timing_metrics(
        ...
        elif test_type in ["creative", "math"]:
            if total_time < 0.5 and token_count > 100:
    ...
    def validate_benchmark_result(
        ...
        if token_accuracy > 0.2:  # 20% difference
            all_warnings.append(f"Token count discrepancy: estimated {token_count}, actual {actual_tokens}")
        ...
        # Overall validity
        is_valid = (
            quality not in [ResponseQuality.FAILED, ResponseQuality.TRUNCATED] and
            len([issue for issue in all_issues if "Suspiciously" in issue or "Invalid" in issue]) == 0
        )
...
def filter_stable_runs(
    ...
    if len(run_times) <= 3:
        ...
        mad = _stats.median([abs(x - median) for x in run_times])  # Median Absolute Deviation
