...
async def demo_basic_generation():
    ...
    client = GroqAILLMClient(model="llama-3.3-70b-versatile", api_key=GROQ_API_KEY)
...
async def demo_streaming():
    ...
    stream = client.create_completion(
        messages, temperature=0.9, max_tokens=50, stream=True
    )
...
async def demo_function_calling():
    ...
    result = await client.create_completion(
        messages, tools=tools, temperature=0.3, stream=False
    )
...
async def demo_gpt_oss_reasoning():
    ...
    client = GroqAILLMClient(model=model, api_key=GROQ_API_KEY)
...
async def demo_large_context():
    ...
    print(f"\n📚 Model: {client.model}")
    print("   Context window: 131,072 tokens!")
    print("   Max output: 32,768 tokens!")
    ...
    result = await client.create_completion(
        messages, temperature=0.3, max_tokens=150, stream=False
    )
...
async def demo_model_comparison():
    ...
    messages = [
        {
            "role": "user",
            "content": "Write a creative title for a sci-fi movie about AI.",
        }
    ]
    ...
    for model_name in models_to_test:
        client = GroqAILLMClient(model=model_name, api_key=GROQ_API_KEY)
        ...
        try:
            ...
            result = await client.create_completion(
                messages, temperature=0.9, max_tokens=20, stream=False
            )
...
async def demo_error_handling():
    ...
    messages = [
        {
            "role": "user",
            "content": "Use the complex tool to analyze this data: 'test input'.",
        }
    ]

    try:
        result = await client.create_completion(
            messages, tools=complex_tools, stream=False
        )
