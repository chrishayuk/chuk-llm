...
async def test_azure_discovery_with_env():
    ...
    if azure_endpoint:
        ...
        masked_endpoint = (
            f"{azure_endpoint[:20]}...{azure_endpoint[-20:]}"
            if len(azure_endpoint) > 40
            else azure_endpoint
        )
    ...
    try:
        ...
        # Categorize results
        deployed_models = [
            m for m in models if m.get("deployment_status") == "deployed"
        ]
        available_models = [
            m for m in models if "available" in m.get("deployment_status", "")
        ]
        fallback_models = [
            m for m in models if m.get("source", "").endswith("fallback")
        ]
