...
async def structured_output_example():
    ...
    messages = [
        {"role": "system", "content": "You are a professional chef. Return recipes in valid JSON format."},
            ...
            try:
                ...
                if ingredients:
                    ...
                    for ing in ingredients[:3]:
                        print(f"     - {ing.get('amount', '')} {ing.get('unit', '')} {ing.get('item', '')}")
...
async def context_caching_example():
    ...
    messages.append({"role": "user", "content": "How does it relate to quantum computing?"})
...
async def main():
    ...
    parser.add_argument("--mode", choices=["all", "openai", "thinking", "structured", "vision", "generation"],
                       default="all", help="Which examples to run")
    parser.add_argument("--test-thinking", action="store_true", help="Test thinking mode")
    ...
    if successful == total:
        ...
        print(f"🔗 Gemini's advanced features demonstrated:")
        print(f"   • OpenAI compatibility for easy migration")
        print(f"   • Thinking mode for complex reasoning")
        print(f"   • Structured output with JSON schemas")
        print(f"   • Native image understanding")
        print(f"   • Image generation capabilities")
        print(f"   • Advanced function calling")
        print(f"   • Context caching for efficiency")
