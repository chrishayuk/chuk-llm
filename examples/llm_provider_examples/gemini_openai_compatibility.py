...
def create_gemini_async_client():
    ...
    return AsyncOpenAI(api_key=os.getenv("GEMINI_API_KEY"), base_url=GEMINI_BASE_URL)
...
def basic_chat_example():
    ...
    response = client.chat.completions.create(
        model="gemini-1.5-flash", messages=messages, temperature=0.7, max_tokens=150
    )
...
def streaming_example():
    ...
    stream = client.chat.completions.create(
        model="gemini-1.5-flash", messages=messages, stream=True
    )
...
def function_calling_example():
    ...
    messages = [
        {
            "role": "user",
            "content": "What's the weather like in Tokyo and search for recent news about AI?",
        }
    ]
    ...
    if response.choices[0].message.tool_calls:
        print(
            f"✅ Function calls requested: {len(response.choices[0].message.tool_calls)}"
        )
...
def conversation_example():
    ...
    for i, user_input in enumerate(turns, 1):
        ...
        # Get response
        response = client.chat.completions.create(
            model="gemini-1.5-flash", messages=messages, temperature=0.7
        )
...
def error_handling_example():
    ...
    test_cases = [
        ...
        {"name": "Empty messages", "model": "gemini-1.5-flash", "messages": []},
        {
