...
async def streaming_example(model: str = "llama-3.1-8b-instant"):
    ...
    print(f"⚡ Received {chunk_count} chunks at {chunk_count/duration:.1f} chunks/sec")
...
async def speed_benchmark():
    ...
    prompt = "Generate a list of 5 creative uses for artificial intelligence in education."
...
async def function_calling_example(model: str = "llama-3.3-70b-versatile"):
    ...
    messages = [
        {"role": "user", "content": "If I travel 1000 meters in 50 seconds, what's my speed in km/h? Also, what's the weather like in Tokyo?"}
    ]
...
async def model_comparison_example():
    ...
    for model, result in results.items():
        ...
        print(f"      Speed: {result['length']/result['time']:.1f} chars/sec" if result['time'] > 0 else "      Speed: N/A")
...
async def feature_detection_example(model: str = "llama-3.3-70b-versatile"):
    ...
    try:
        ...
        print(f"   Max Context: {model_info.get('max_context_length', 131072):,} tokens")
...
async def parallel_processing_test():
    ...
    for prompt in prompts:
        client = get_client("groq", model=model)
        response = await client.create_completion([{"role": "user", "content": prompt}], max_tokens=50)
    ...
    parallel_responses = await asyncio.gather(*[process_prompt(p) for p in prompts])
...
async def comprehensive_test(model: str = "llama-3.3-70b-versatile"):
    ...
    if supports_tools:
        ...
        messages = [{"role": "user", "content": "My system is processing 1000 requests per second. Analyze this performance."}]
        response = await client.create_completion(messages, tools=tools)
    ...
    if supports_json:
        ...
        messages = [
            ...
            {"role": "user", "content": "Create a JSON object with fields: status, speed, provider."}
        ]
...
async def main():
    ...
    parser.add_argument("--skip-tools", action="store_true", help="Skip function calling examples")
    ...
    print(f"Website: https://groq.com")
    print(f"Console: https://console.groq.com")
    
    # Show model capabilities
    try:
        config = get_config()
        supports_tools = config.supports_feature("groq", Feature.TOOLS, args.model)
        supports_streaming = config.supports_feature("groq", Feature.STREAMING, args.model)
        ...
        print(f"  System Messages: ✅")  # Groq supports system messages
    ...
    if successful == total:
        ...
        print(f"⚡ Groq's LPU technology delivers ultra-fast inference!")
        print(f"🚀 Experience the speed difference with chuk-llm!")
    else:
        print(f"\n⚠️  Some examples failed. Check your API key and model access.")
        
        # Show model recommendations
        print(f"\n💡 Groq Model Recommendations:")
        print(f"   • For speed: llama-3.1-8b-instant")
        print(f"   • For capability: llama-3.3-70b-versatile")
        print(f"   • For reasoning: openai/gpt-oss-120b (if available)")
        print(f"   • Context window: Up to 131k tokens!")
