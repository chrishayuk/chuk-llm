...
class AzureOpenAIModelDiscoverer(BaseModelDiscoverer):
    ...
    def __init__(self, 
        ...
        self.deployment_patterns = {
            'reasoning_deployments': [
                r'.*o1.*', r'.*o3.*', r'.*reasoning.*'
            ],
            'vision_deployments': [
                r'.*gpt-4.*vision.*', r'.*gpt-4o.*', r'.*vision.*'
            ],
            'chat_deployments': [
                r'.*gpt-4.*', r'.*gpt-3.*', r'.*chat.*'
            ],
    ...
    async def discover_models(self) -> List[Dict[str, Any]]:
        """
        Discover Azure OpenAI models via available models API and fallback approach
        
        NOTE: Azure OpenAI deprecated the simple deployments listing API in 2024.
        We can only reliably discover available models, not active deployments,
        without using the Azure Management API (which requires subscription info).
        """
        discovered_models = []
        ...
        try:
            ...
            log.warning(f"Failed to discover available Azure models: {e}")
        ...
        log.info("Azure deployment discovery requires Azure Management API (not implemented)")
        
        # If no models discovered, use fallback
        if not discovered_models:
            log.warning("Azure model discovery failed - using fallback models")
            return self._get_azure_fallback_models()
    ...
    async def _discover_available_models(self) -> List[Dict[str, Any]]:
        ...
        # FIXED: Use the correct Azure OpenAI API endpoint for listing available models
    ...
    def get_deployment_recommendations(self) -> List[Dict[str, Any]]:
        """Get recommended deployments for common use cases"""
        return [
            {
                "model": "gpt-4o",
                "deployment_name": "gpt-4o",
                "use_case": "general_chat_and_vision",
                "recommended_capacity": 20,
                "description": "Best general-purpose model with vision capabilities"
            },
            {
                "model": "gpt-4o-mini", 
                "deployment_name": "gpt-4o-mini",
                "use_case": "high_volume_chat",
