...
async def demonstrate_discovery():
    ...
    try:
        ...
        print(
            f"   🔧 Discovery enabled: {ollama_provider.extra.get('dynamic_discovery', {}).get('enabled', False)}"
        )
    ...
    for alias, _target in list(aliases.items())[:3]:
        try:
    ...
    try:
        ...
        if not test_models and discovery_candidates:
            for model in discovery_candidates:
                if any(
                    base in model.lower()
                    for base in ["llama", "qwen", "granite", "phi"]
                ):
                    test_models.append(model)
                    break
    ...
    try:
        ...
        if not provider_info.get("error"):
            print(
                f"   📋 Total models: {len(provider_info.get('available_models', []))}"
            )
            print(
                f"   🔧 Discovery enabled: {provider_info.get('discovery_enabled', False)}"
            )
