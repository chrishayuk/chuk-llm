...
async def main():
    ...
    api_base = os.getenv("OPENAI_API_BASE", "https://vllm.1jwefpwyy4k2.us-south.codeengine.appdomain.cloud/v1")
    ...
    print("\n🗨️ Conversation:")
    conversation = [
        {"role": "user", "content": "What's Docker?"}
    ]
    ...
    conversation.append({"role": "user", "content": "How does it relate to Kubernetes?"})
    ...
    try:
        response = await client.create_completion(
            messages=[{"role": "user", "content": "What's the weather in San Francisco and what time is it there?"}],
            tools=tools,
            max_tokens=150
        )
