...
async def alice_branching_demo():
    ...
    async with conversation(provider="openai") as alice_chat:
        # Alice's main journey begins
        print("📖 Main Story:")
        print("Alice: I've just fallen down a rabbit hole and landed in a strange place!")
        response = await alice_chat.say("I've just fallen down a rabbit hole and landed in a strange place!")
        print(f"Narrator: {response[:200]}...\n")
        
        print("Alice: I see a table with a bottle labeled 'DRINK ME'. What should I do?")
        response = await alice_chat.say("I see a table with a bottle labeled 'DRINK ME'. What should I do?")
        print(f"Narrator: {response[:200]}...\n")
        ...
        async with alice_chat.branch() as drink_branch:
            ...
            response = await drink_branch.say("I drank it! I'm shrinking rapidly!")
            print(f"[DRINK BRANCH] Narrator: {response[:200]}...\n")
            
            print("[DRINK BRANCH] Alice: Now I'm tiny! I can fit through that little door!")
            response = await drink_branch.say("Now I'm tiny! I can fit through that little door!")
            print(f"[DRINK BRANCH] Narrator: {response[:200]}...\n")
            
            print("[DRINK BRANCH] Alice: Oh no, I left the key on the table above!")
            response = await drink_branch.say("Oh no, I left the key on the table above!")
            print(f"[DRINK BRANCH] Narrator: {response[:200]}...\n")
        ...
        async with alice_chat.branch() as explore_branch:
            ...
            response = await explore_branch.say("Let me look around this room more carefully first.")
            print(f"[EXPLORE BRANCH] Narrator: {response[:200]}...\n")
            
            print("[EXPLORE BRANCH] Alice: I found a cake that says 'EAT ME'!")
            response = await explore_branch.say("I found a cake that says 'EAT ME'!")
            print(f"[EXPLORE BRANCH] Narrator: {response[:200]}...\n")
            
            print("[EXPLORE BRANCH] Alice: Should I eat the cake or drink the bottle?")
            response = await explore_branch.say("Should I eat the cake or drink the bottle?")
            print(f"[EXPLORE BRANCH] Narrator: {response[:200]}...\n")
        ...
        async with alice_chat.branch() as cat_branch:
            ...
            response = await cat_branch.say("A grinning cat just appeared in the air!")
            print(f"[CAT BRANCH] Narrator: {response[:200]}...\n")
            
            print("[CAT BRANCH] Cheshire Cat: 'We're all mad here. Are you mad?'")
            response = await cat_branch.say("The cat says 'We're all mad here. Are you mad?' How should I respond?")
            print(f"[CAT BRANCH] Narrator: {response[:200]}...\n")
        
        # Back to the main story - Alice is still standing by the table
        print("📖 Back to Main Story:")
        print("Alice: I'm still standing here by the table, thinking about my options...")
        response = await alice_chat.say("I'm still standing here by the table, thinking about my options...")
        print(f"Narrator: {response[:200]}...\n")
        
        print("Alice: What was I just considering? Remind me of my situation.")
        response = await alice_chat.say("What was I just considering? Remind me of my situation.")
