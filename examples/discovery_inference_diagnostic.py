...
async def test_discovery_integration():
    ...
    env_vars = {
        "CHUK_LLM_DISCOVERY_ENABLED": os.getenv(
            "CHUK_LLM_DISCOVERY_ENABLED", "not set"
        ),
        ...
        "CHUK_LLM_DISCOVERY_TIMEOUT": os.getenv(
            "CHUK_LLM_DISCOVERY_TIMEOUT", "not set"
        ),
    }
    ...
    if os.getenv("CHUK_LLM_AUTO_DISCOVER") != "true":
        print("💡 Setting CHUK_LLM_AUTO_DISCOVER=true for this test")
    ...
    try:
        ...
        for provider_name in providers_to_test:
            ...
            try:
                ...
                if discovery_config.get("enabled"):
                    ...
                    print(
                        f"         └─ Cache timeout: {discovery_config.get('cache_timeout', 300)}s"
                    )
                else:
                    print("      🔍 Discovery: not configured in YAML")
                ...
                print("      🧪 Testing model availability (triggers discovery):")
                ...
                for model_name in test_models[:2]:  # Test first 2 to avoid timeout
                    ...
                    if hasattr(config_manager, "_ensure_model_available"):
                        ...
                        print(
                            "         ⚠️  _ensure_model_available method not available"
                        )
        ...
        try:
                    ...
                    if "not available" in error_msg.lower():
                        ...
                        if hasattr(config_manager, "_ensure_model_available"):
                            ...
                            if resolved:
                                ...
                                print(
                                    "      💡 Try rerunning - discovery may have updated configuration"
                                )
                            else:
                                print(
                                    "      💡 Model truly not available through discovery"
                                )
            ...
            print("      ⚠️  ChukLLM ask function not available for testing")
        ...
        print("\n✨ What was tested:")
        print("   🔧 Environment variable discovery controls")
        print("   📋 Configuration manager with discovery mixin")
        print("   🔍 Provider discovery configuration")
        print("   🧪 Model availability checking (_ensure_model_available)")
        print("   🎯 Integration with ChukLLM ask() function")

        print("\n💡 Key findings:")

        discovery_working = False
        if hasattr(config_manager, "_ensure_model_available"):
            print("   ✅ Discovery integration is implemented")
            discovery_working = True
        else:
            print("   ❌ Discovery integration method missing")

        if hasattr(config_manager, "get_discovery_settings"):
            print("   ✅ Discovery settings accessible")
        else:
            print("   ❌ Discovery settings not accessible")
        ...
        if settings.get("enabled", False):
            ...
            print("   ⚠️  Discovery may not be enabled - check environment variables")

        if discovery_working:
            print("\n🚀 Discovery integration is ready!")
            print("   Set CHUK_LLM_DISCOVERY_ENABLED=true to enable")
            print("   Configure dynamic_discovery in provider YAML")
            print("   Models will be discovered automatically when requested")
        ...
        for value in test_values:
            ...
            try:
                ...
                if hasattr(config, "get_discovery_settings"):
                    ...
                    print(
                        f"      {var_name}={value} → enabled: {settings.get('enabled', 'unknown')}"
                    )
