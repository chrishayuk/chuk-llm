...
def create_test_image(filename="test_image.png"):
    ...
    # Green triangle
    draw.polygon(
        [(100, 200), (50, 280), (150, 280)], fill="green", outline="black", width=2
    )

    # Yellow star-like shape
    draw.polygon(
        [(250, 200), (230, 240), (270, 240)], fill="yellow", outline="black", width=2
    )
...
async def vision_examples():
    ...
    response = await ask_openai_gpt4o(
        "What shapes and colors do you see in this image?", "test_image.png"
    )
    ...
    response = await ask_gemini_flash(
        "What text do you see in this image?", img_bytes.read()
    )
...
def vision_sync_examples():
    ...
    response = ask_openai_gpt4o_sync(
        "Solve the math problems shown in this image", "math_problem.png"
    )
