...
def demo_cli_queries():
    ...
    print_info("Note: These demos will only work if you have the required API keys set up")
    ...
    run_command('uv run chuk-llm ask_quick "Explain Python in one sentence" --verbose', 
                "Ask with verbose output")
    
    # Demo direct provider calls
    print_step("Direct Provider Calls")
    
    run_command('uv run chuk-llm ask "What is machine learning?" --provider ollama --model granite3.3', 
                "Direct provider and model specification")
    
    # Demo JSON mode
    print_step("JSON Responses")
    
    run_command('uv run chuk-llm ask "List 3 programming languages" --provider ollama --json', 
...
def demo_discovery():
    ...
    run_command("uv run chuk-llm discovered ollama", "Show discovered functions for Ollama")
