...
async def demo_environment_info():
    ...
    if env_files:
        ...
        print("\n📁 No .env files found")
        print("   💡 Create .env file with: echo 'OPENAI_API_KEY=your_key' > .env")
...
async def demo_ollama_direct_discovery():
    ...
    try:
        ...
        ollama_discoverer = OllamaModelDiscoverer(
            provider_name="ollama", api_base="http://localhost:11434"
        )

        # Create universal manager with the discoverer
        discovery_manager = UniversalModelDiscoveryManager(
            provider_name="ollama", discoverer=ollama_discoverer
        )
        ...
        print("\n📊 Discovery Analysis:")
        print(
            f"   🏷️  Model Families: {dict(sorted(families.items(), key=lambda x: x[1], reverse=True))}"
        )
        print(
            f"   🎯 Specializations: {dict(sorted(specializations.items(), key=lambda x: x[1], reverse=True))}"
        )
        ...
        for i, model in enumerate(sorted_models[:8], 1):
            ...
            if capabilities:
                cap_names = [
                    f.value if hasattr(f, "value") else str(f) for f in capabilities
                ]
        ...
        for model in discovered_models:
            for feature in model.capabilities:
                ...
                capability_counts[feature_name] = (
                    capability_counts.get(feature_name, 0) + 1
                )
        ...
        for capability, count in sorted(
            ...
            print(
                f"   📈 {capability}: {count}/{len(discovered_models)} models ({percentage:.0f}%)"
            )
...
async def demo_enhanced_model_analysis(models: list):
    ...
    for family, family_models in sorted(
        analysis["by_family"].items(), key=lambda x: len(x[1]), reverse=True
    ):
        if family == "unknown":
            continue
        ...
        reasoning_count = sum(
            1 for m in family_models if m.metadata.get("reasoning_capable", False)
        )
    ...
    for tier, tier_models in analysis["by_size_tier"].items():
        if tier_models:
            avg_size = sum(m.metadata.get("size_gb", 0) for m in tier_models) / len(
                tier_models
            )
            print(
                f"   📊 {tier.upper()}: {len(tier_models)} models (avg {avg_size:.1f}GB)"
            )
    ...
    if reasoning_models:
        best_reasoning = max(
            reasoning_models, key=lambda m: m.metadata.get("size_gb", 0)
        )
        print(
            f"   🧠 Best for reasoning: {best_reasoning.name} ({best_reasoning.metadata.get('size_gb', 0):.1f}GB)"
        )
    ...
    if efficient_models:
        ...
        print(
            f"   ⚡ Most efficient: {most_efficient.name} ({most_efficient.metadata.get('size_gb', 0):.1f}GB)"
        )
    ...
    if models:
        ...
        print(
            f"   🏆 Most powerful: {largest.name} ({largest.metadata.get('size_gb', 0):.1f}GB)"
        )
    ...
    if family_counts:
        ...
        print(
            f"   👑 Dominant family: {dominant_family[0]} ({dominant_family[1]} models)"
        )
...
async def demo_openai_direct_discovery():
    ...
    try:
        ...
        discovery_manager = UniversalModelDiscoveryManager(
            provider_name="openai", discoverer=openai_discoverer
        )
        ...
        print(
            f"✅ Discovered {len(discovered_models)} OpenAI models in {discovery_time:.2f}s"
        )
        ...
        # Categorize models
        reasoning_models = [
            m for m in discovered_models if m.metadata.get("is_reasoning", False)
        ]
        vision_models = [
            m for m in discovered_models if m.metadata.get("is_vision", False)
        ]
        standard_models = [
            m
            for m in discovered_models
            if not m.metadata.get("is_reasoning", False)
            and not m.metadata.get("is_vision", False)
        ]
    ...
    fallback_models = [
        {
            "name": "o1-mini",
            "type": "🧠 Reasoning",
            "features": "Chain-of-thought reasoning, no streaming",
        },
        {
            "name": "o1-preview",
            "type": "🧠 Reasoning",
            "features": "Advanced reasoning, 128K context",
        },
        {
            "name": "gpt-4o",
            "type": "👁️ Vision",
            "features": "Vision + text, 128K context",
        },
        {
            "name": "gpt-4o-mini",
            "type": "👁️ Vision",
            "features": "Efficient vision model, 128K context",
        },
        {
            "name": "gpt-4-turbo",
            "type": "💬 Standard",
            "features": "High performance, 128K context",
        },
        {
            "name": "gpt-3.5-turbo",
            "type": "💬 Standard",
            "features": "Fast and efficient, 16K context",
        },
    ]
    ...
    try:
        ...
        sample_config = {
            ...
            "family_rules": {
                ...
                "reasoning": {
                    ...
                    "restrictions": {"no_streaming": True, "no_system_messages": True},
                    "special_params": {"use_max_completion_tokens": True},
                },
            },
        }
        ...
        for model_name, metadata in test_models:
            ...
            model = DiscoveredModel(
                name=model_name, provider="test_provider", metadata=metadata
            )
            ...
            caps = [
                ...
                for f in inferred_model.capabilities
            ]
...
async def demo_model_comparison(ollama_models: list = None, openai_models: list = None):
    ...
    categories = {"reasoning": [], "vision": [], "code": [], "general": []}
    ...
    for provider, model in all_models:
        # Check capabilities
        is_reasoning = (
            model.metadata.get("reasoning_capable", False)
            or model.metadata.get("is_reasoning", False)
            or "reasoning" in (model.family or "")
        )

        is_vision = (
            model.metadata.get("supports_vision", False)
            or model.metadata.get("is_vision", False)
            or "vision" in (model.family or "")
        )

        is_code = (
            model.metadata.get("specialization") == "code"
            or model.metadata.get("is_code", False)
            or "code" in (model.family or "")
        )
    ...
    for category, models in categories.items():
        ...
        icon_map = {"reasoning": "🧠", "vision": "👁️", "code": "💻", "general": "💬"}
        ...
        for model_info in models:
            ...
            if provider == "ollama":
                ...
                print(
                    f"      └─ Performance: {performance} | Context: {context:,}"
                    + (" tokens" if context else "")
                )
    ...
    if ollama_count > 0:
        print(
            f"   🏠 Ollama: {ollama_count} local models (flexible, private, no API costs)"
        )
    if openai_count > 0:
        print(
            f"   ☁️  OpenAI: {openai_count} cloud models (cutting-edge, scalable, API required)"
        )
...
async def main():
    ...
    print(
        "💡 In production, enable discovery through your chuk_llm.yaml configuration."
    )
