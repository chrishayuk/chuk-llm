...
def print_model_info(model, prefix: str = "   •"):
    ...
    if metadata.get("performance_tier"):
        ...
        print(
            f"      └─ Performance: {tier} | Context: {model.context_length or 'unknown'}"
        )
...
async def discover_all_providers():
    ...
    try:
        ...
        print(
            f"🎯 Discovery enabled for {len(providers)} providers: {', '.join(providers)}"
        )
        ...
        for provider, models in results.models_by_provider.items():
            ...
            if models:
                sorted_models = sorted(
                    models, key=lambda m: _get_model_priority(m), reverse=True
                )

                # Show different categories
                reasoning_models = [
                    m for m in models if m.metadata.get("reasoning_capable", False)
                ]
                vision_models = [
                    m for m in models if m.metadata.get("supports_vision", False)
                ]
                code_models = [
                    m for m in models if m.metadata.get("specialization") == "code"
                ]
        ...
        print(
            f"   • Reasoning models: {summary['special_model_counts']['reasoning_models']}"
        )
...
async def run_inference_tests(dynamic_models):
    ...
    for category, test_info in test_categories.items():
        ...
        for model_name in test_info["models"]:
            ...
            for _provider_name, models in dynamic_models.items():
                for model in models:
...
async def test_model_inference_advanced(
    provider: str, model_name: str, prompt: str, max_tokens: int, description: str = ""
):
    ...
    try:
        ...
        if len(response.strip()) < 5:
            ...
            print("   ✨ Good: Response addresses the prompt")
        ...
        if "not found" in error_msg.lower() or "does not exist" in error_msg.lower():
            print(
                "   💡 Model may not be loaded in Ollama. Try: ollama pull "
...
async def analyze_test_results(test_results):
    ...
    # Results by provider
    providers = {r["provider"] for r in test_results}
    print("\n📋 Results by Provider:")
    ...
    for provider in providers:
        ...
        success_rate = (
            (provider_successful / provider_total * 100) if provider_total > 0 else 0
        )
        print(
            f"   {provider}: {provider_successful}/{provider_total} ({success_rate:.1f}%)"
        )
    ...
    categories = {r.get("category", "unknown") for r in test_results}
    if len(categories) > 1:
        ...
        for category in sorted(categories):
            if category == "unknown":
                continue
            category_results = [
                r for r in test_results if r.get("category") == category
            ]
            ...
            if category_total > 0:
                ...
                print(
                    f"   {category.title()}: {category_successful}/{category_total} ({success_rate:.1f}%)"
                )

    # Performance analysis
    successful_with_time = [
        r for r in test_results if r["success"] and r["response_time"]
    ]
    if successful_with_time:
        ...
        for provider in providers:
            ...
            if provider_times:
                ...
                print(
                    f"   • {provider}: {avg_provider_time:.2f}s avg ({fastest:.2f}s - {slowest:.2f}s range)"
                )
        ...
        for i, result in enumerate(speed_rankings[:5], 1):
            print(
                f"   {i}. {result['provider']}/{result['model']}: {result['response_time']:.2f}s"
            )
    ...
    if size_performance:
        ...
        for model, size_gb, resp_time in size_performance:
            ...
            print(
                f"   • {model}: {size_gb}GB, {resp_time:.2f}s (efficiency: {efficiency:.1f})"
            )
    ...
    if successful_tests == total_tests:
        print("   🎉 Perfect! All models are working correctly.")
    elif successful_tests > total_tests * 0.8:
        print("   👍 Great! Most models are working well.")
    elif successful_tests > total_tests * 0.5:
        print("   ⚠️  Mixed results. Some models need attention.")
    else:
        print("   🔧 Many issues found. Check your setup.")

    # Specific recommendations
    failed_models = [r for r in test_results if not r["success"]]
    connection_errors = [
        r for r in failed_models if "connection" in (r.get("error", "") or "").lower()
    ]
    not_found_errors = [
        r for r in failed_models if "not found" in (r.get("error", "") or "").lower()
    ]

    if connection_errors:
        print(
            "   🔌 Connection issues detected. Ensure Ollama is running: ollama serve"
        )

    if not_found_errors:
        print("   📥 Some models not found. Try pulling missing models:")
    ...
    if successful_with_time:
        fastest_provider = min(
            ...
            key=lambda p: sum(
                r["response_time"]
                for r in test_results
                if r["provider"] == p and r["success"] and r["response_time"]
            )
            / max(
                1,
                len(
        ...
        for use_case in use_cases:
            ...
            if recommendations:
                for i, rec in enumerate(recommendations[:5], 1):
                    ...
                    print(
                        f"      └─ Score: {rec['score']:.0f} | {tier} performance | {context} context"
                    )
            else:
...
async def run_health_checks(discovery_manager):
    ...
    try:
        ...
        status_icons = {"healthy": "✅", "degraded": "⚠️", "critical": "❌"}

        print(
            f"🎯 Overall Status: {status_icons.get(overall_status, '❓')} {overall_status.upper()}"
        )
        print(
            f"   • Providers: {health_status['healthy_providers']}/{health_status['total_providers']} healthy"
        )
...
async def main():
    ...
    if (
        any(models for models in dynamic_models.values())
        or DEMO_CONFIG["test_known_models"]
    ):
    ...
    if results:
        ...
        print(
            f"   • Reasoning models: {results.summary['special_model_counts']['reasoning_models']}"
        )
        print(
            f"   • Vision models: {results.summary['special_model_counts']['vision_models']}"
        )
        print(
            f"   • Code models: {results.summary['special_model_counts']['code_models']}"
        )

    print("\n🏆 Your Model Collection Highlights:")
    print("   • Large models (>10GB): mistral-small, gpt-oss")
    print("   • Reasoning models: granite3.3, qwen3, llama3.1")
    print("   • Efficient models (<5GB): gemma3, llama3.2, mistral")
    print("   • Specialized: mistral-nemo (instruction-tuned)")

    print("\n🚀 Next Steps:")
    print("   • Use discovered models in your applications")
    print("   • Update configuration files with generated YAML")
    print("   • Monitor model performance over time")
    print("   • Consider pulling newer model versions as they become available")

    print("\n💬 The discovery system makes ANY discovered model available for use!")
    print("   Just use: await ask(prompt, provider='ollama', model='qwen3:latest')")
...
async def test_known_models_directly():
    ...
    for model_name, category in test_models:
        prompt = (
            ...
            if category == "general"
            else "If I have 3 apples and buy 2 more, how many do I have? Think step by step."
        )
...
async def analyze_model_collection(test_results):
    ...
    size_categories = {"Large (>10GB)": [], "Medium (5-10GB)": [], "Small (<5GB)": []}
    ...
    for category, models in size_categories.items():
        if models:
            ...
            if successful > 0:
                ...
                best_model = min(
                    [r for r, _ in models if r["success"] and r["response_time"]],
                ...
                if best_model:
                    ...
                    print(
                        f"   🏆 Fastest: {best_model['model']} ({best_model['response_time']:.2f}s, {size}GB)"
                    )
    ...
    for result in test_results:
        if (
            ...
            efficiency_scores.append(
                (model_name, efficiency, result["response_time"], size_gb)
            )

    # Sort by efficiency (lower score = more efficient)
    efficiency_scores.sort(key=lambda x: x[1])

    for i, (model_name, efficiency, resp_time, size_gb) in enumerate(
        efficiency_scores[:5], 1
    ):
        print(
            f"   {i}. {model_name}: {resp_time:.2f}s for {size_gb}GB (efficiency: {efficiency:.2f})"
        )
    ...
    if large_working:
        print(
            f"   🦣 Large models working: {len(large_working)} available for complex tasks"
        )

    small_working = [r for r, s in size_categories["Small (<5GB)"] if r["success"]]
    if small_working:
        print(
            f"   🐰 Small models working: {len(small_working)} available for quick tasks"
        )

    reasoning_models = ["granite3.3:latest", "qwen3:latest", "llama3.1:latest"]
    reasoning_working = [
        r for r in test_results if r["model"] in reasoning_models and r["success"]
    ]
    if reasoning_working:
        print(
            f"   🧠 Reasoning capability: {len(reasoning_working)} models ready for complex thinking"
        )
