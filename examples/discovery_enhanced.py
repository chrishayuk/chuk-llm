...
def classify_model_type(model_name: str, family: str) -> Dict[str, Any]:
    ...
    reasoning_specialists = [
        'granite3.3', 'qwen3', 'o1', 'o3', 'reasoning', 'qwq', 'marco-o1'
    ]
    
    # Code specialists
    code_specialists = [
        'codellama', 'deepseek-coder', 'starcoder', 'codegemma', 'devstral'
    ]
    ...
    large_general = [
        'gpt-oss', 'mistral-small', 'mistral-nemo'
    ]
    
    is_reasoning = any(specialist in model_lower for specialist in reasoning_specialists)
...
async def discover_all_models():
    ...
    try:
        ...
        for model_type, type_models in classified_models.items():
            ...
            print(f"   {emoji} {model_type.replace('_', ' ').title()}: {len(type_models)} models")
...
async def test_dynamic_model(model_name: str, model_type: str, test_config: Dict) -> Dict[str, Any]:
...
async def analyze_results(test_results):
    ...
    print(f"🎯 Dynamic Model Test Results:")
...
async def demonstrate_key_capability():
    ...
    try:
        ...
        print(f"   Even though gpt-oss:latest isn't in your static configuration,")
        print(f"   ChukLLM discovered it and made it available for use!")
        
    except Exception as e:
        print(f"   ❌ Error testing gpt-oss:latest: {e}")
        print(f"   💡 This might mean the model isn't loaded in Ollama")
...
async def main():
    ...
    dynamic_only, static_only, common_models = await compare_static_vs_discovered(models)
    
    # Step 3: Test dynamic-only models
    if dynamic_only:
        test_results = await test_dynamic_models(models, classified_models, dynamic_only)
    ...
    if models:
        ...
        general_count = len(classified_models.get('general', [])) + len(classified_models.get('large_general', []))
    ...
    print(f"\n🚀 Key Takeaway:")
    print(f"   ChukLLM's discovery system makes model management effortless!")
    print(f"   Any model you have in Ollama is immediately available,")
    print(f"   regardless of your static configuration.")
    
    print(f"\n💻 Try it yourself:")
    print(f"   await ask('Your question here', provider='ollama', model='any_discovered_model')")
