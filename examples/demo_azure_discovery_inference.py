...
async def test_direct_client(deployment_name: str) -> bool:
    ...
    try:
        ...
        if response.get("response"):
            ...
            print(
                f"  • Using Smart Defaults: {model_info.get('using_smart_defaults', False)}"
            )
            ...
            tool_response = await client.create_completion(
                tool_messages, tools=tools, max_tokens=100
            )

            if tool_response.get("tool_calls"):
                print_success(
                    f"Tool calls supported: {len(tool_response['tool_calls'])} calls"
                )
            else:
...
async def test_client_factory(deployment_name: str) -> bool:
    ...
    try:
        ...
        # Test completion
        messages = [
            {"role": "user", "content": "Say 'Factory works with custom deployment!'"}
        ]
        response = await client.create_completion(messages, max_tokens=50)
        ...
        if response.get("response"):
            ...
            async for chunk in client.create_completion(
                stream_messages, stream=True, max_tokens=100
            ):
...
async def main():
    ...
    if all_passed:
        ...
        print(
            " • You can use them with: get_client('azure_openai', model='your-deployment')"
        )
    else:
        ...
        for deployment_name, results in all_results.items():
            ...
            if not results.get("client_factory"):
                ...
                print(
                    "   The function should skip validation for azure_openai provider"
                )
