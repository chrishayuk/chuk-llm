...
async def universal_vision_example(
    ...
    if not config.supports_feature("watsonx", Feature.VISION, model):
        ...
        print("💡 Vision-capable models: meta-llama/llama-3-2-90b-vision-instruct")
        return None
...
async def comprehensive_benchmark():
    ...
    print(
        f"{'Model':<35} {'Reasoning':<12} {'Creative':<12} {'Coding':<12} {'Avg Time':<10}"
    )
    print("-" * 85)
    ...
    for model, model_results in results.items():
        ...
        print(
            f"{model:<35} {status_icons[0]:<12} {status_icons[1]:<12} {status_icons[2]:<12} {avg_time:.2f}s"
        )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Watson X Provider Example Script - Universal Config"
    )
    parser.add_argument(
        "--model",
    ...
    parser.add_argument(
        "--skip-tools", action="store_true", help="Skip function calling"
    )
    parser.add_argument(
        "--test-benchmark", action="store_true", help="Run comprehensive benchmark"
    )
    ...
    print(
        f"API Key: {'✅ Set' if (os.getenv('WATSONX_API_KEY') or os.getenv('IBM_CLOUD_API_KEY')) else '❌ Missing'}"
    )
    print(
        f"Project ID: {'✅ Set' if os.getenv('WATSONX_PROJECT_ID') else '❌ Missing'}"
    )
    print(
        f"Watson X URL: {os.getenv('WATSONX_AI_URL', 'https://us-south.ml.cloud.ibm.com (default)')}"
    )
    ...
    if not args.quick:
        ...
        if not args.skip_vision:
            examples.append(
                (
                    "Universal Vision",
                    lambda: universal_vision_example(
    ...
    if successful == total:
        ...
        print("🔗 Watson X provider is working perfectly with universal config!")
        print(
        ...
        print("   • For tools + reliability: ibm/granite-3-8b-instruct")
        print("   • For speed: meta-llama/llama-3-2-1b-instruct")
        print("   • For balance: meta-llama/llama-3-2-3b-instruct")
        print("   • For capability: meta-llama/llama-3-3-70b-instruct")
        print("   • For enterprise: mistralai/mistral-large")
        print("   • For vision: meta-llama/llama-3-2-90b-vision-instruct")
