...
from chuk_llm import conversation, ask
...
async def demo_3_multimodal():
    ...
    if not Path(image_path).exists():
        ...
        async with conversation(provider="openai", model="gpt-4o") as chat:
            ...
            response = await chat.say("Describe what you would see in a network diagram")
            print(f"AI: {response[:300]}...")
    else:
        async with conversation(provider="openai", model="gpt-4o") as chat:
            print(f"User: What do you see in this image? [Image: {image_path}]")
            response = await chat.say("What do you see in this image?", image=image_path)
            print(f"AI: {response[:300]}...")
...
async def demo_5_stateless_context():
    ...
    print(f"User: What's the capital?")
    print(f"Context: We're discussing France and its major cities")
    print(f"AI: {response}")
    ...
    previous_messages = [
        ...
        {"role": "assistant", "content": "The solar system consists of the Sun and all objects that orbit it, including eight planets..."},
        {"role": "user", "content": "Focus on the gas giants"},
        {"role": "assistant", "content": "The gas giants are Jupiter, Saturn, Uranus, and Neptune..."}
    ]
    ...
    print(f"User: Which one is the largest?")
    print(f"AI: {response}")
