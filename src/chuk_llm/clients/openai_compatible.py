...
Client for OpenAI-compatible APIs (Groq, DeepSeek, Together, Mistral, etc).
This client follows the OpenAI v1/chat/completions API format.

Separated from OpenAIClient to allow OpenAI to migrate to new Responses API
...
class OpenAICompatibleClient(AsyncLLMClient):
    ...
    - Groq, DeepSeek, Together, Perplexity
    - Mistral, Ollama, Advantage
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        logger.info(
            f"Initialized OpenAI-compatible client: model={model}, "
    ...
    def _prepare_request(self, request: CompletionRequest) -> APIRequest:
        ...
        if request.temperature is not None and not self._is_reasoning:
        ...
        if request.top_p is not None and not self._is_reasoning:
            params[RequestParam.TOP_P.value] = request.top_p

        if request.frequency_penalty is not None and not self._is_reasoning:
            params[RequestParam.FREQUENCY_PENALTY.value] = request.frequency_penalty

        if request.presence_penalty is not None and not self._is_reasoning:
    ...
    async def stream(  # type: ignore[misc,override]
        ...
        try:
            ...
            async for chunk_bytes in self._stream_post(
                ...
                if (
                    not chunk_str
                    or chunk_str == f"{SSEPrefix.DATA.value}{SSEEvent.DONE.value}"
                ):
                    continue

                if chunk_str.startswith(SSEPrefix.DATA.value):
    ...
    def _parse_stream_chunk(
        ...
        # Only yield if we have content or complete tool calls
