...
async def test_working_url_vision():
    ...
    for i, image_url in enumerate(test_urls, 1):
        ...
        try:
            ...
            if response.get('response') and 'could not load' not in response.get('response', '').lower():
        ...
        messages = [
            {
                ...
                "content": [
                    {
                        "type": "text",
                        "text": f"What color is this square? Answer with just the color name."
                    },
                    {
                        "type": "image_url",
                        "image_url": {
    ...
    messages = [
        {
            ...
            "content": [
                ...
                },
                {
                    "type": "image_url",
                    "image_url": {
                        "url": f"data:image/png;base64,{red_square}"
                    }
                },
                {
                    "type": "image_url",
                    "image_url": {
                        "url": f"data:image/png;base64,{blue_square}"
                    }
                }
            ]
        }
    ...
    try:
        ...
        if response.get('response'):
            ...
            if has_red and has_blue:
                print(f"✅ Successfully identified both colors!")
                print(f"   Response: {result}")
                return True
            else:
                print(f"⚠️  Partial success - missing colors")
                print(f"   Response: {result}")
                return False
        else:
            print("❌ No response")
...
async def test_vision_with_tools():
    ...
    messages = [
        {
            ...
            "content": [
                ...
                },
                {
                    "type": "image_url",
                    "image_url": {
                        "url": f"data:image/png;base64,{green_square}"
                    }
                }
            ]
        }
    ...
    try:
        ...
        if response.get('tool_calls'):
            ...
            print(f"❌ No function calls made")
...
async def main():
    ...
    if passed == total:
        print("🎉 All vision tests passed! Your Anthropic vision integration is working perfectly.")
    elif passed > 0:
        print("⚠️  Some tests passed - vision is partially working. Check failed tests above.")
    else:
        print("❌ All tests failed - there may be an issue with your vision implementation.")
    
    # Recommendations
    if passed < total:
        print("\n💡 Troubleshooting Tips:")
        print("   1. Ensure you're using a vision-capable model (Claude 4.x or Claude 3.7.x)")
