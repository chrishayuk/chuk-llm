    ...
    try:
        ...
        try:
            if hasattr(config_manager, '_ensure_model_available'):
                ...
                print(f"   _ensure_model_available method not found")
        ...
        try:
            ...
            print(f"   Testing client functionality...")
            model_info = client.get_model_info()
            if model_info.get('error'):
                print(f"   ⚠️  Client created but has issues: {model_info['error']}")
            else:
                print(f"   ✅ Client fully functional")
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=5.0) as client:
                ...
                actual_models = [model_data["name"] for model_data in data.get("models", [])]
                ...
                if test_model in actual_models and test_model in after_models:
                    print(f"   ✅ Perfect integration: Model exists in both Ollama and chuk_llm")
                elif test_model in actual_models:
                    print(f"   ⚠️  Model exists in Ollama but not accessible via chuk_llm")
                else:
                    print(f"   ❌ Model not in Ollama")
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=3.0) as client:
                ...
                response.raise_for_status()
                data = response.json()
                ollama_models = [model_data["name"] for model_data in data.get("models", [])]
        except:
            pass
        
        initial_models = set(['llama3.3', 'qwen3', 'granite3.3', 'mistral', 'gemma3', 'phi3', 'codellama'])
        ...
        if discovered_models:
            print(f"   🔍 Discovered models: {list(discovered_models)[:5]}{'...' if len(discovered_models) > 5 else ''}")
        
        # Status assessment
        print(f"\n   🎯 DISCOVERY STATUS:")
        if len(discovered_models) > 0 and test_model in current_models:
            print(f"      ✅ FULLY WORKING: Discovery successfully integrated")
            print(f"      ✅ Added {len(discovered_models)} models to provider configuration")
            print(f"      ✅ get_client() can access discovered models")
            print(f"      ✅ Target model '{test_model}' is accessible")
        elif len(discovered_models) > 0:
            print(f"      ⚠️  PARTIALLY WORKING: Discovery found models but target not accessible")
            print(f"      ✅ Added {len(discovered_models)} models")
            print(f"      ❌ Target model '{test_model}' not accessible")
        else:
            print(f"      ❌ NOT WORKING: Discovery didn't add any models")
            print(f"      Issue: Discovery process not updating provider configuration")
