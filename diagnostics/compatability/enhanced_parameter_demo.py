...
class EnhancedParameterCompatibilityDemo:
    ...
    def _map_parameter_for_provider(
        self, provider: str, param_name: str, param_value: Any
    ) -> tuple[str, Any]:
    ...
    def _prepare_params_for_provider(
        ...
        for param_name, param_value in params.items():
            ...
            mapped_name, mapped_value = self._map_parameter_for_provider(
                provider, param_name, param_value
            )
        ...
        for param_name, param_config in self.universal_parameters.items():
            ...
            for provider in self.providers:
                try:
                    ...
                    safe_value = (
                        test_values[1] if len(test_values) > 1 else test_values[0]
                    )
                    ...
                    provider_params = self._prepare_params_for_provider(
                        provider, test_params
                    )
        ...
        for param_name, param_config in self.universal_parameters.items():
            ...
            for provider in self.providers:
                ...
                if provider in provider_ranges:
                    ...
                    edge_status = (
                        "✅ Strict"
                        if all(r == "❌" for r in edge_results)
                        else "⚠️ Lenient"
                    )
                else:
    ...
    def _print_range_results(self, results: dict[str, dict[str, dict[str, str]]]):
        ...
        for param_name, provider_results in results.items():
            ...
            print(
                f"    {'Provider':<12} | {'Range':<15} | {'Edge Cases':<12} | {'Status':<8}"
            )
            print("    " + "-" * 55)
            ...
            for provider in self.providers:
                ...
                print(
                    f"    {provider:<12} | {range_info:<15} | {edge_info:<12} | {status:<8}"
                )
    ...
    def _print_scenario_results(self, results: dict[str, dict[str, dict[str, Any]]]):
        ...
        for scenario_name, provider_results in results.items():
            ...
            print(
                f"    {'Provider':<12} | {'Response':<50} | {'Status':<15} | {'Time':<8}"
            )
            print("    " + "-" * 90)
            ...
            for provider in self.providers:
                ...
                print(
                    f"    {provider:<12} | {response:<50} | {status:<15} | {time_str:<8}"
                )
    ...
    async def _show_parameter_best_practices(self):
        ...
        provider_specifics = [
            ...
            (
                "gemini",
                "Auto-maps max_tokens→max_output_tokens, uses generation_config",
            ),
    ...
    async def _generate_compatibility_summary(self):
        ...
        sorted_providers = sorted(
            compatibility_scores.items(), key=lambda x: x[1], reverse=True
        )
        ...
        if overall_score >= 90:
            ...
            print(
                "✅ GOOD! Strong universal parameter support with minor provider-specific handling needed"
            )
        elif overall_score >= 60:
            print(
                "⚠️ MODERATE: Universal config working but needs provider-specific optimizations"
            )
        else:
            print(
                "🔧 NEEDS WORK: Universal parameter handling requires significant improvement"
            )

        print("\n🎯 Key Recommendations:")
        recommendations = [
            f"✅ {len([s for s in compatibility_scores.values() if s >= 75])} providers have good parameter support",
            "🔧 Implement parameter mapping for provider-specific needs",
            "💡 Use parameter validation pipeline before API calls",
            "⚡ Consider provider-specific parameter profiles for optimization",
        ]
