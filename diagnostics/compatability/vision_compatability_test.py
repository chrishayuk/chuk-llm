...
class VisionCompatibilityTester:
    ...
    async def _test_provider_vision(self, provider: str) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                if not supports_vision:
                    results["error_details"].append(
                        "Model doesn't support vision according to config"
                    )
                    return results
                ...
                results["model_used"] = model
                log.warning(
                    f"Config check failed for {provider}/{model}, continuing anyway: {e}"
                )
            ...
            if results["single_image_test"] and "✅" in str(
                results["single_image_test"]
            ):
                try:
        ...
        try:
            ...
            if response and response.get("response") and not response.get("error"):
                ...
                if "red" in response_text:
                    ...
                    color in response_text
                    for color in ["blue", "green", "yellow", "purple"]
                ):
                    results["single_image_test"] = (
                        f"⚠️ Identified wrong color: {response['response']}"
                    )
                else:
                    results["single_image_test"] = (
                        f"⚠️ No color identified: {response['response']}"
                    )
            else:
                results["single_image_test"] = (
                    f"❌ No valid response: {response.get('response', 'No response')}"
                )
        except Exception as e:
        ...
        try:
            ...
            if response and response.get("response") and not response.get("error"):
                ...
                if has_red and has_blue:
                    results["multiple_images_test"] = (
                        "✅ Identified both red and blue squares"
                    )
                elif has_red or has_blue:
                    results["multiple_images_test"] = (
                        f"⚠️ Partially correct: {response['response']}"
                    )
                else:
                    results["multiple_images_test"] = (
                        f"❌ No colors identified: {response['response']}"
                    )
            else:
                results["multiple_images_test"] = (
                    f"❌ No valid response: {response.get('response', 'No response')}"
    ...
    async def _test_image_formats(self, client, results: dict[str, Any]):
        ...
        try:
            messages = [
                {
                    ...
                    "content": [
                        ...
                        self.get_universal_image_format(
                            self.test_images["green_square"]
                        ),
                    ],
                }
            ]
            ...
            if (
                ...
                and "green" in response["response"].lower()
            ):
                results["format_tests"]["universal_image_url"] = "✅ Works correctly"
            elif response and response.get("response"):
                results["format_tests"]["universal_image_url"] = (
                    f"⚠️ Response but wrong: {response['response']}"
                )
            else:
        ...
        try:
            messages = [
                {
                    ...
                    "content": [
                        {"type": "text", "text": "What color is this?"},
                        self.get_anthropic_native_format(
                            self.test_images["green_square"]
                        ),
                    ],
                }
            ...
            if (
                ...
                results["format_tests"]["anthropic_native"] = (
                    f"⚠️ Response but wrong: {response['response']}"
                )
            else:
    ...
    async def run_vision_compatibility_test(
        ...
        results = {
            "metadata": {
                ...
                "models_used": {
                    p: self.vision_models.get(p, "default") for p in test_providers
                },
            },
            "provider_results": {},
            "summary": {},
        ...
        for provider in test_providers:
            ...
            try:
                results["provider_results"][
                    provider
                ] = await self._test_provider_vision(provider)
                ...
                if result.get("skipped"):
                    ...
                    error_summary = ", ".join(
                        result.get("error_details", ["Unknown"])[:2]
                    )
                    print(f"   ❌ No vision support: {error_summary}")
                elif result["single_image_test"] and "✅" in str(
                    result["single_image_test"]
                ):
    ...
    def _generate_summary(self, provider_results: dict[str, Any]) -> dict[str, Any]:
        ...
        for provider, results in provider_results.items():
            if results.get("skipped"):
                ...
                results["supports_vision"]
                and results.get("single_image_test")
                and "✅" in str(results["single_image_test"])
            ):
        ...
        for provider, results in provider_results.items():
            ...
            if (
                results.get("format_tests", {}).get("universal_image_url")
                == "✅ Works correctly"
            ):
                universal_format_works += 1

        if total_tested > 0:
            summary["universal_format_compatibility"] = (
                f"{universal_format_works}/{total_tested} providers"
            )
        else:
        ...
        total_count = len(
            [p for p in provider_results if not provider_results[p].get("skipped")]
        )

        if working_count == 0:
            summary["recommendations"].append(
                "❌ No providers successfully handled vision inputs"
            )
        elif working_count == total_count:
            summary["recommendations"].append(
                "✅ All tested providers support vision with universal format"
            )
        else:
            summary["recommendations"].append(
        ...
        if universal_format_works == total_tested and total_tested > 0:
            summary["recommendations"].append(
                "✅ Universal image_url format works across all vision providers"
            )
        else:
            summary["recommendations"].append(
                "🔧 Some providers may need format adjustments"
            )
    ...
    def print_results(self, results: dict[str, Any]):
        ...
        if summary["providers_with_working_vision"]:
            ...
            for provider in summary["providers_with_working_vision"]:
                ...
                print(
                    f"     - Single image: {provider_results.get('single_image_test', 'Not tested')}"
                )
                if provider_results.get("multiple_images_test"):
                    print(
                        f"     - Multiple images: {provider_results['multiple_images_test']}"
                    )
        ...
        print("   }")
        print(
            "   Each provider should handle conversion to their native format internally."
        )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test vision compatibility across providers"
    )
    parser.add_argument(
        "--providers",
        nargs="*",
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Test only providers with API keys set"
    )
    ...
    if args.quick:
        ...
        providers = [
            p for p in all_providers if env_vars[p] is None or os.getenv(env_vars[p])
        ]
