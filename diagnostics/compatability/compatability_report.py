...
class CompatibilityReportGenerator:
    ...
    def __init__(self):
        self.test_providers = {
            "openai": {
                ...
                "has_env": self._check_env("OPENAI_API_KEY"),
            },
            "anthropic": {
                "models": ["claude-sonnet-4-20250514"],
                "has_env": self._check_env("ANTHROPIC_API_KEY"),
            },
            "gemini": {
                "models": ["gemini-2.0-flash"],
                "has_env": self._check_env("GEMINI_API_KEY"),
            },
            "mistral": {
                "models": ["mistral-medium-2505"],
                "has_env": self._check_env("MISTRAL_API_KEY"),
            },
            "deepseek": {
                "models": ["deepseek-chat"],
                "has_env": self._check_env("DEEPSEEK_API_KEY"),
            },
            "perplexity": {
                "models": ["sonar-pro"],
                "has_env": self._check_env("PERPLEXITY_API_KEY"),
            },
            "groq": {
                "models": ["llama-3.3-70b-versatile"],
                "has_env": self._check_env("GROQ_API_KEY"),
            },
            "watsonx": {
                "models": ["ibm/granite-3-8b-instruct"],
                "has_env": self._check_env("WATSONX_API_KEY")
                or self._check_env("IBM_CLOUD_API_KEY"),
            },
            "ollama": {
                "models": ["llama3.2"],
                "has_env": True,
    ...
    async def generate_full_compatibility_report(
        ...
        if not available_providers:
            print("❌ No configured providers available. Check API keys.")
            return {}
        ...
        format_results = await self._test_response_format_consistency(
            available_providers
        )
        ...
        report = {
            ...
            "recommendations": self._generate_recommendations(
                executive_summary, available_providers
            ),
        }
        ...
        for provider in providers:
            try:
                ...
                if method:
                    ...
                    for param_name, _expected_type in self.standard_parameters.items():
                ...
                has_model_info = hasattr(client, "get_model_info") and callable(
                    getattr(client, "get_model_info", None)
                )
                ...
                results[provider] = {
                    ...
                    "overall_score": self._calculate_interface_score(
                        method_compat, param_compat, has_model_info
                    ),
                }

            except Exception as e:
                results[provider] = {"error": str(e), "overall_score": 0.0}
    ...
    def _calculate_interface_score(
        self, method_compat: dict, param_compat: dict, has_model_info: bool
    ) -> float:
        """Calculate interface compatibility score"""
        all_checks = (
            list(method_compat.values())
            + list(param_compat.values())
            + [has_model_info]
        )
        if not all_checks:
            return 0.0
    ...
    async def _test_response_format_consistency(
        self, providers: list[str]
    ) -> dict[str, Any]:
        ...
        for provider in providers:
            try:
                with warnings.catch_warnings():
                    ...
                    try:
                        response = await client.create_completion(
                            test_message, stream=False, max_tokens=50
                        )
                        ...
                        if isinstance(response, dict):
                            ...
                            response_analysis["has_response_field"] = (
                                "response" in response
                            )
                            response_analysis["has_tool_calls_field"] = (
                    ...
                    streaming_valid = await self._test_streaming_format(
                        client, test_message
                    )
                ...
                if "401" in error_str or "Authorization" in error_str:
                    ...
                    results[provider] = {"error": error_str[:100], "overall_score": 0.0}
    ...
    def _validate_response_format(self, response: Any) -> bool:
        ...
        return isinstance(response.get("tool_calls"), list)
    ...
    async def _test_tool_compatibility(self, providers: list[str]) -> dict[str, Any]:
        ...
        test_message = [
            {
                "role": "user",
                "content": "What's the weather in London? Use the get_weather function to check.",
            }
        ]
        ...
        for provider in providers:
            try:
                    ...
                    try:
                        ...
                        if response_text:
                            ...
                            has_graceful_degradation = any(
                                ...
                                for phrase in degradation_phrases
                            )
                        ...
                        streaming_tools_valid = await self._test_streaming_tools(
                            client, test_message
                        )
                        ...
                        tool_score = (
                            1.0
                            if has_tool_calls
                            else (0.3 if has_graceful_degradation else 0.0)
                        )
                        structure_score = (
                            1.0
                            if (not tool_calls or tool_structure_valid)
                            else (
                                valid_tool_count / len(tool_calls)
                                if tool_calls
                                else 0.0
                            )
                        )
                        ...
                        results[provider] = {
                            "tool_calling": {
                                ...
                                "response_text_sample": response_text[:100]
                                if response_text
                                else "No response",
                                "score": tool_score,
                            },
                            "streaming_tools": {
                                "success": streaming_tools_valid,
                                "score": streaming_score,
                            },
                            "overall_score": (
                                tool_score + structure_score + streaming_score
                            )
                            / 3,
                        }
                        ...
                        if "'NoneType' object has no attribute 'lower'" in error_str:
                        ...
                        elif "groq" in provider.lower() and (
                            "function" in error_str.lower()
                            ...
                            "gemini" in provider.lower()
                            and "system_instruction" in error_str
                        ):
                            results[provider] = {
                                "tool_calling": {
                                    "success": False,
                ...
                if "401" in error_str or "Authorization" in error_str:
                    ...
                    results[provider] = {"error": error_str[:200], "overall_score": 0.0}
        ...
        return {
            ...
            "compatibility_analysis": {
                ...
                "providers_needing_enhancement": [
                    ...
                    or result.get("tool_calling", {}).get("needs_client_fix", False)
                ],
                "tools_working_count": sum(
                    1
                ...
                "authentication_issues": [
                    ...
                    for provider, result in results.items()
                    if "authentication" in result.get("error", "").lower()
                    or result.get("tool_calling", {}).get("reason")
        ...
        try:
            with warnings.catch_warnings():
                ...
                stream = client.create_completion(
                    messages, tools=[self.test_tool], stream=True, max_tokens=300
                )
    ...
    async def _test_system_message_support(
        self, providers: list[str]
    ) -> dict[str, Any]:
        ...
        for provider in providers:
            try:
                with warnings.catch_warnings():
                    ...
                    try:
                        response = await client.create_completion(
                            test_message, system=test_system, max_tokens=100
                        )
                    ...
                    if not system_param_works:
                        try:
                            ...
                            response = await client.create_completion(
                                sys_messages, max_tokens=100
                            )

                            if response and response.get("response"):
                                response_text = str(response["response"])
                                system_respected = (
                                    "system_test:" in response_text.lower()
                                )
                                ...
                                results[provider] = {
                                    ...
                                    "system_param_error": system_param_error[:100]
                                    if system_param_error
                                    else None,
                                }
                            else:
                                results[provider] = {
        ...
        return {
            ...
            "compatibility_analysis": {
                ...
                "providers_with_system_support": [
                    ...
                    for provider, result in results.items()
                    if result.get("system_parameter_works", False)
                    or result.get("system_messages_work", False)
                ],
                "authentication_issues": [
                    provider
                    for provider, result in results.items()
    ...
    async def _test_streaming_capabilities(
        self, providers: list[str]
    ) -> dict[str, Any]:
        ...
        for provider in providers:
            try:
                with warnings.catch_warnings():
                    ...
                    try:
                        stream = client.create_completion(
                            test_message, stream=True, max_tokens=100
                        )
                        ...
                        results[provider] = {
                            ...
                            "overall_score": 1.0
                            if stream_works and chunk_count > 0
                            else 0.0,
                        }
                ...
                results[provider] = {"error": str(e)[:100], "overall_score": 0.0}
    ...
    def _generate_executive_summary(
        ...
        # Overall score
        if area_scores:
            summary["overall_compatibility_score"] = sum(area_scores.values()) / len(
                area_scores
            )
        ...
        for provider in all_providers:
            ...
            for area, results in all_results.items():
                if isinstance(results, dict) and "provider_results" in results:
                    ...
                    if area == "tools":
                        provider_features["tools"] = provider_result.get(
                            "tool_calling", {}
                        ).get("success", False)
                    elif area == "streaming":
                        provider_features["streaming"] = provider_result.get(
                            "streaming_works", False
                        )
                    elif area == "system_messages":
                        provider_features["system_messages"] = provider_result.get(
                            "system_parameter_works", False
                        ) or provider_result.get("system_messages_work", False)
                    ...
                    if provider_result.get("tool_calling", {}).get(
                        "needs_enhancement", False
                    ) or provider_result.get("tool_calling", {}).get(
                        "needs_client_fix", False
                    ):
                        needs_enhancement = True
    ...
    def _generate_recommendations(
        self, summary: dict[str, Any], tested_providers: list[str]
    ) -> list[str]:
        ...
        if overall_score >= 0.9:
            recommendations.append(
                "🎉 Excellent compatibility! Universal config is working great."
            )
        elif overall_score >= 0.7:
            recommendations.append("👍 Good compatibility with room for improvement.")
        elif overall_score >= 0.5:
            recommendations.append(
                "⚠️ Moderate compatibility - several issues to address."
            )
        else:
        ...
        if fully_compatible:
            recommendations.append(
                f"🏆 Fully compatible providers: {', '.join(fully_compatible)}"
            )
        ...
        if providers_with_issues:
            issue_providers = [
                ...
                for p in providers_with_issues
            ]
            recommendations.append(
                f"🔧 Providers needing fixes: {', '.join(issue_providers)}"
            )

        providers_needing_enhancement = summary.get("providers_needing_enhancement", [])
        if providers_needing_enhancement:
            recommendations.append(
                f"⚡ Providers with enhancement opportunities: {', '.join(providers_needing_enhancement)}"
            )
        ...
        if feature_matrix:
            ...
            streaming_support = sum(
                1
                for features in feature_matrix.values()
                if features.get("streaming", False)
            )
            system_support = sum(
                1
                for features in feature_matrix.values()
                if features.get("system_messages", False)
            )
            ...
            if tools_support < total_providers:
                recommendations.append(
                    f"🔧 Tool calling: {tools_support}/{total_providers} providers working"
                )
            if streaming_support < total_providers:
                recommendations.append(
                    f"📡 Streaming: {streaming_support}/{total_providers} providers working"
                )
            if system_support < total_providers:
                recommendations.append(
                    f"💬 System messages: {system_support}/{total_providers} providers working"
                )

        # Priority actions
        test_summary = summary.get("test_results_summary", {})
        poor_areas = [
            area for area, result in test_summary.items() if result["score"] < 0.5
        ]
        if poor_areas:
            recommendations.append(
                f"🎯 Priority fixes needed for: {', '.join(poor_areas)}"
            )
    ...
    def print_summary(self, report: dict[str, Any]):
        ...
        for area, result in test_summary.items():
            print(
                f"   {area.replace('_', ' ').title()}: {result['status']} ({result['score']:.1%})"
            )
        ...
        if feature_matrix:
            ...
            for provider, provider_features in feature_matrix.items():
                ...
                stream_icon = (
                    "✅" if provider_features.get("streaming", False) else "❌"
                )
                system_icon = (
                    "✅" if provider_features.get("system_messages", False) else "❌"
                )
                print(
                    f"{provider:<15} {tools_icon:<8} {stream_icon:<8} {system_icon:<8}"
                )
            ...
            for provider_info in providers_with_issues:
                if isinstance(provider_info, dict):
                    ...
                    if provider_info.get("enhancement_needed"):
                        print("     ⚡ Enhancement opportunity available")
        ...
        for _test_name, test_results in detailed_results.items():
            if (
                isinstance(test_results, dict)
                and "compatibility_analysis" in test_results
            ):
            ...
            if isinstance(test_results, dict) and "provider_results" in test_results:
                for provider, result in test_results["provider_results"].items():
                    if isinstance(result, dict):
                        if (
                            result.get("note")
                            == "This was a bug in the compatibility tester, not the provider"
                            or "validation_bug" in result.get("error", "")
                        ):
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test provider compatibility with enhanced analysis"
    )
