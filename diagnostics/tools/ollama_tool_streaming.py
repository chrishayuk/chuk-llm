...
async def test_ollama_tool_streaming():
    ...
    try:
        ...
        for model_name, tool_info in test_models:
            print(f"   • {model_name} [{tool_info['tool_quality']}] - {tool_info['reason']}")
                ...
                "function": {
                    ...
                    "parameters": {
                        ...
                        "properties": {
                            ...
                            "numbers": {"type": "array", "items": {"type": "number"}, "description": "Numbers to operate on"}
                        },
                ...
                for prefix in ["[calling:", "[tool:", "using tool:", "executing:"]:
                    if prefix in chunk_lower:
                        try:
                            ...
                            end = chunk_str.find("]", start) if "]" in chunk_str[start:] else start + 20
...
def analyze_ollama_tool_result(chuk_result, raw_result, test_name, test_case, tool_info):
    ...
    chuk_functions = [tc["function"]["name"] for tc in chuk_result.get("tool_calls", [])]
    ...
    if tool_quality == "excellent":
        ...
        if chuk_streamed and (chuk_tool_count > 0 or has_tool_info) and chuk_result["chunks"] >= min_chunks:
            print(f"      ✅ Excellent tool streaming ({chuk_result['chunks']} chunks)")
            print(f"      🔧 ChukLLM: {chuk_tool_count} tool calls, patterns: {list(set(tool_formats))}")
            if expected_detected_chuk:
                print(f"      ✨ Expected function '{expected_function}' detected!")
            success = True
        else:
            print(f"      ⚠️  Expected better performance from excellent model")
            success = False
    
    elif tool_quality == "good":
        # Moderate expectations for good models
        if chuk_streamed and (chuk_tool_count > 0 or has_tool_info or chuk_result["chunks"] >= min_chunks):
            print(f"      ✅ Good tool streaming ({chuk_result['chunks']} chunks)")
            print(f"      🔧 ChukLLM: {chuk_tool_count} tool calls detected")
            success = True
        else:
            print(f"      ⚠️  Tool streaming below expectations")
            success = False
        ...
        if chuk_streamed and chuk_result["chunks"] >= 1:
            ...
            if has_tool_info or chuk_tool_count > 0:
                print(f"      🎉 Bonus: Tool patterns detected!")
            success = True
        else:
    ...
    print(f"      📊 Raw Ollama: {raw_result['chunks']} chunks, {raw_tool_count} tool calls")
    ...
    chuk_preview = chuk_result["final_response"][:150] if chuk_result["final_response"] else ""
    if chuk_preview:
...
def analyze_all_ollama_tool_results(results, overall_success):
    ...
    for model_name, model_data in results.items():
        ...
        if tool_quality not in quality_performance:
            quality_performance[tool_quality] = {"total": 0, "successful": 0, "streaming": 0, "tools": 0}
        ...
        for test_name, test_result in model_results.items():
            ...
            if chuk["success"]:
                ...
                test_success = (chuk["streaming_worked"] and 
                               chuk["chunks"] >= min_chunks and 
                               (has_tool_info or has_tool_calls))
    ...
    for quality, perf in quality_performance.items():
        if perf["total"] > 0:
            ...
            print(f"  {quality.title()}: {success_rate:.0f}% success, {streaming_rate:.0f}% streaming, {tool_rate:.0f}% tools")
...
async def test_specific_ollama_tool_models():
    ...
    for model_name, description in specific_models:
        ...
        tools = [
            {
                ...
                "function": {
                    ...
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "filename": {"type": "string", "description": "CSV file to read"},
                ...
                "function": {
                    ...
                    "parameters": {
                        ...
                        "properties": {
                            ...
                            "stat_type": {"type": "string", "description": "Type of statistic"}
                        },
        ...
        try:
            result = await test_chuk_llm_ollama_tool_streaming(model_name, prompt, tools)
            ...
            if result["success"]:
                ...
                if chunk_count >= 10 and (tool_count > 0 or has_tool_info):
                    print(f"  🏆 Excellent tool performance!")
                    success_count += 1
                elif chunk_count >= 5 and has_tool_info:
                    print(f"  ✅ Good tool performance!")
                    success_count += 0.8
                else:
                    print(f"  ⚠️  Basic performance")
                    success_count += 0.4
