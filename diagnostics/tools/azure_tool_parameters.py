...
def safe_parse_tool_arguments(arguments: Any) -> Dict[str, Any]:
    ...
    if isinstance(arguments, str):
        ...
        if (cleaned_args.startswith('"') and cleaned_args.endswith('"') and 
            len(cleaned_args) > 2):
...
def extract_table_name_from_args(func_args: Union[str, dict]) -> str:
...
def extract_query_from_args(func_args: Union[str, dict]) -> str:
    ...
    try:
        ...
        test_cases = [
            ...
            },
            {
                ...
                "expected_params": {"file_path": "config.json", "container": "documents"}
            }
        ]
                ...
                "function": {
                    ...
                    "parameters": {
                        ...
                        "properties": {
                            ...
                            "service": {"type": "string", "description": "Cognitive service to use"}
                        },
                        "required": ["content"]
                    }
                }
            ...
            if chunk.get("tool_calls"):
                for tc in chunk["tool_calls"]:
                    ...
                    if tool_name and tool_id not in tool_calls_started:
                        ...
                        expected_tools = ["stdio.read_query", "web.api:search", "azure.cognitive:analyze"]
                        if tool_name in expected_tools:
                            print(f"      ✅ Universal tool name correctly restored")
        ...
        if restored_names:
            ...
            if tool_calls_started:
                ...
                for tool_id, tool_name in tool_calls_started.items():
                    status = "✅ RESTORED" if tool_name in restored_names else "⚠️  UNKNOWN"
                    print(f"      • {tool_name} ({tool_id[:12]}...) {status}")
            
            return True
        elif len(tool_calls_started) > 0:
            print(f"   ⚠️  Tools started but names may not be in expected set: {list(tool_calls_started.values())}")
            return True
        else:
            print(f"   ⚠️  No tool calls detected in streaming response")
            return False
...
if __name__ == "__main__":
    print("🚀 Starting Azure OpenAI Complete Tool Chain Test with Universal Compatibility...")
    asyncio.run(main())