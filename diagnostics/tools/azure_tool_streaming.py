...
def is_old_o1_deployment(deployment_name):
    ...
    return (
        deployment_lower in old_o1_patterns
        or deployment_lower.startswith("o1-")
        or deployment_lower.startswith("o1_")
    )
...
async def test_streaming_with_azure_deployments():
    ...
    test_cases = [
        ...
        },
        {
            ...
            "messages": [
                {
                    "role": "user",
                    "content": "If a train leaves at 2pm going 60mph, and another at 3pm going 80mph, when do they meet if they're 200 miles apart? Show your work.",
                }
            ],
            "tools": None,
            "evaluation": "text_streaming",
    ...
    for deployment, deployment_desc in test_deployments:
        is_active = await test_azure_deployment_availability(
            azure_endpoint, azure_api_key, deployment
        )
        if is_active:
    ...
    for deployment, deployment_desc in active_deployments:
        ...
        for test_case in test_cases:
            ...
            should_skip = test_case.get(
                "skip_for_old_o1", False
            ) and is_old_o1_deployment(deployment)

            if should_skip:
                print(
                    f"  ⏭️ Skipping {case_name} ({deployment} is old O1 without tool support)"
                )
                continue
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            response = await client.post(
                url, headers=headers, params=params, json=test_payload
            )
...
def analyze_single_azure_result(chuk_result, raw_result, test_name, test_case):
    ...
    if evaluation_type == "tool_streaming":
        ...
        if chuk_streamed and (has_tool_info or chuk_tool_count > 0):
            print(
                f"      ✅ ChukLLM Azure tool streaming works ({chuk_result['chunks']} chunks)"
            )
            ...
            print(
                f"      ✅ ChukLLM Azure tool streaming likely works ({chuk_result['chunks']} chunks)"
            )
            print("      ℹ️  Tool pattern detection may need enhancement")
            success = True
        else:
            print("      ❌ ChukLLM Azure tool streaming failed")
            success = False

        print(
            f"      📊 Raw Azure: {raw_result['chunks']} chunks, {raw_tool_count} tool calls"
        )
        ...
        if chuk_streamed and chuk_meets_minimum:
            ...
            if chunk_ratio < 0.7:
                ...
                print(
                    f"      ⚠️  Different chunk counts (ChukLLM: {chuk_result['chunks']}, Raw: {raw_result['chunks']})"
                )
                success = chuk_meets_minimum
        elif chuk_streamed and not chuk_meets_minimum:
            print(
                f"      ⚠️  ChukLLM streaming but low chunk count ({chuk_result['chunks']} < {min_chunks})"
            )
            success = False
        elif not chuk_streamed:
            print("      ❌ ChukLLM not streaming")
            success = False
        else:
            success = True
        ...
        for test_name, test_result in deployment_results.items():
            ...
            if chuk["success"] and raw["success"]:
                if evaluation_type == "tool_streaming":
                    ...
                    test_success = chuk["streaming_worked"] and (
                        has_tool_info or has_tool_calls or good_chunk_count
                    )
                    if test_success:
                        tool_calls_worked += 1
                else:  # text_streaming
                    ...
                    test_success = (
                        chuk["streaming_worked"] and chuk["chunks"] >= min_chunks
                    )
...
async def test_azure_reasoning_deployments():
    ...
    if not active_reasoning_deployments:
        print(
            "⚠️  No reasoning deployments found - this is normal for many Azure OpenAI instances"
        )
    ...
    for deployment, deployment_desc in active_reasoning_deployments:
        ...
        try:
            ...
            print(
                f"  🔧 Testing tool calling capability for {deployment} (tools supported: {deployment_supports_tools})..."
            )

            if not deployment_supports_tools:
                print(
                    f"    ⏭️ Skipping tool test - {deployment} is an old O1 deployment without tool support"
                )
                success_count += 0.8
                continue
            ...
            tool_success = tool_chunks > 0 and (
                len(unique_patterns) > 0 or "execute_sql" in tool_response.lower()
            )

            if reasoning_success and tool_success:
                print(
                    f"  ✅ {deployment} excellent: reasoning + tool calling work perfectly"
                )
                success_count += 1
...
async def main():
    ...
    print(
        f"Multi-deployment tests: {'✅ PASS' if multi_deployment_ok else '⚠️ PARTIAL'}"
    )
