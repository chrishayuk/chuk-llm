...
async def test_model_access(model_name: str) -> tuple[bool, str, dict | None]:
...
Think through this step by step."""
        ...
        try:
            ...
            if hasattr(client, "_last_response_usage"):
                ...
                completion_tokens = getattr(
                    usage, "completion_tokens", len(response_text.split())
                )
        ...
        mentions_all_people = all(
            name in response_lower for name in ["alice", "bob", "carol"]
        )
        shows_math = any(
            indicator in response_lower
            for indicator in ["twice", "*", "×", "+", "=", "3", "6", "7"]
        )
        shows_steps = any(
            indicator in response_lower
            for indicator in ["step", "first", "second", "then", "therefore"]
        )

        # Improved scoring system
        reasoning_quality_score = 0
        if has_correct_total:
            reasoning_quality_score += 4  # Correct answer is key
        if mentions_all_people:
            reasoning_quality_score += 2  # Shows understanding
        if shows_math:
            reasoning_quality_score += 2  # Shows mathematical reasoning
        if shows_steps:
            reasoning_quality_score += 2  # Shows explicit reasoning
        if hidden_reasoning_detected:
            reasoning_quality_score += 3  # Hidden reasoning bonus
...
async def main():
    ...
    for model_info in AVAILABLE_REASONING_MODELS:
        ...
        if has_access:
            ...
            if "tier" in message.lower():
                print("      💡 This model requires higher usage tier")
            elif "parameter" in message.lower():
                print("      🔧 Parameter conversion issue - needs client fix")
            elif "configuration" in message.lower():
                print("      ⚙️  Configuration loading issue")
    ...
    if available_models:
        ...
        print("\n🔧 COMMON ISSUES:")
        print(
            "      • Tier restrictions - o1 requires Tier 5, o3-mini requires Tier 3+"
        )
        print("      • Account age - o1 requires 30+ day old accounts")
        print("      • Spending requirements - o1 requires $1000+ spent")
        print(
            "      • Configuration issues - check client_class and model availability"
        )
        return
    ...
    for model_name in available_models:
        ...
        if result.get("success"):
            ...
            print(
                f"      Reasoning level: {result.get('reasoning_level', 'unknown').upper()}"
            )
            ...
            if result.get("hidden_reasoning_detected"):
                indicators.append(
                    f"🧠 Hidden reasoning ({result.get('reasoning_tokens', 0)} tokens)"
                )
            ...
            if result.get("reasoning_ratio", 0) > 0:
                print(
                    f"      Reasoning intensity: {result['reasoning_ratio']}x output tokens"
                )

            if result.get("response_preview"):
                preview = result["response_preview"][:200]
                print(
                    f"      Preview: {preview}{'...' if len(result['response_preview']) > 200 else ''}"
                )
        else:
    ...
    excellent_reasoning = [
        r for r in successful_models if r.get("reasoning_level") == "excellent"
    ]
    good_reasoning = [
        r
        for r in successful_models
        if r.get("reasoning_level") in ["excellent", "good"]
    ]
    hidden_reasoning = [
        r for r in successful_models if r.get("hidden_reasoning_detected")
    ]
    ...
    if reasoning_capable or hidden_reasoning:
        ...
        print("✅ Successfully tested OpenAI reasoning models")
        ...
        for result in successful_models:
            ...
            capability_str = (
                " + ".join(capabilities) if capabilities else "Basic response"
            )
        ...
        for result in best_models[:3]:  # Top 3
            ...
            reasoning_level = result.get("reasoning_level", "unknown")
            print(
                f"   mcp-cli chat --provider openai --model {model_name}  # {reasoning_level} quality"
            )
        ...
        if len(successful_models) > 1:
            ...
            highest_quality = max(
                successful_models, key=lambda x: x.get("reasoning_score", 0)
            )

            print("\n📊 PERFORMANCE ANALYSIS:")
            print(f"   ⚡ Fastest: {fastest['model']} ({fastest['response_time']}s)")
            print(
                f"   🧠 Highest quality: {highest_quality['model']} ({highest_quality.get('reasoning_score', 0)}/13)"
            )
            ...
            if hidden_reasoning:
                ...
                for result in hidden_reasoning:
                    ...
                    print(
                        f"   🧠 {model_name}: {reasoning_tokens} internal reasoning tokens ({reasoning_ratio}x multiplier)"
                    )

                print("\n💡 INSIGHT: These models do extensive internal reasoning")
                print("   Even when they don't show steps, they're thinking deeply!")
        ...
        if fastest and fastest.get("reasoning_level") in ["good", "excellent"]:
            print(
                f"   ⚡ Speed Priority: {fastest['model']} ({fastest['response_time']}s, {fastest.get('reasoning_level')} quality)"
            )

        explicit_reasoning = [r for r in successful_models if r.get("explicit_steps")]
        if explicit_reasoning:
            best_explicit = max(
                explicit_reasoning, key=lambda x: x.get("reasoning_score", 0)
            )
            print(
                f"   🔍 Transparency Priority: {best_explicit['model']} (shows reasoning steps)"
            )

        if highest_quality:
            print(
                f"   🌟 Quality Priority: {highest_quality['model']} (score: {highest_quality.get('reasoning_score', 0)}/13)"
            )

    elif successful_models:
        print("\n⚠️  MODELS RESPONDING BUT LIMITED REASONING DETECTED")
        print("Models are accessible but may need reasoning prompt optimization")
        ...
        for result in successful_models:
            ...
            print(
                f"   • {model_name}: {response_time}s response, {score}/13 reasoning score"
            )

        print("\n💡 SUGGESTIONS:")
        print("   • Try more explicit reasoning prompts")
        print("   • Models may be doing internal reasoning not visible in output")
        print("   • Consider using Responses API for reasoning visibility")
        ...
        for result in reasoning_results:
            if not result.get("success"):
                ...
                if "parameter" in error.lower():
                    error_types["parameter_issues"] = (
                        error_types.get("parameter_issues", 0) + 1
                    )
                elif "tier" in error.lower():
                    error_types["tier_issues"] = error_types.get("tier_issues", 0) + 1
                elif "configuration" in error.lower():
                    error_types["config_issues"] = (
                        error_types.get("config_issues", 0) + 1
                    )
                else:
        ...
        if error_types.get("parameter_issues"):
            print("\n💡 PARAMETER ISSUE FIX:")
            print(
                "   The OpenAI client needs max_completion_tokens parameter conversion"
            )
            print("   This should be handled automatically by the reasoning model fix")

        if error_types.get("tier_issues"):
            print("\n💡 TIER ACCESS INFO:")
            print("   • o1-mini: Most users (Tier 1+)")
            print("   • o1: Tier 5 users ($1000+ spent)")
            print("   • o3-mini: Tier 3+ users (moderate usage)")

    print("\n🔬 DIAGNOSTIC INSIGHTS:")
    print("   • Reasoning models do internal processing (hidden reasoning tokens)")
    print("   • External reasoning steps are just one indicator of capability")
    print("   • Response correctness + speed often indicate internal reasoning quality")
    print("   • Different models have different reasoning exposition styles")

    print("\n🏁 DIAGNOSTIC COMPLETE")

    # Final recommendation
    if len(available_models) > 0:
        print("✨ OpenAI reasoning models are partially working!")
        if len(reasoning_capable) > 0:
            print(f"✨ Reasoning behavior confirmed on {len(reasoning_capable)} models")
    else:
        print("🔧 OpenAI reasoning models need configuration or access fixes")
