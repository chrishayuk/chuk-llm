        ...
        if result:
            ...
            for j, tc in enumerate(result):
                ...
                if func_name == test_case["expected_tool"]:
                    ...
                    print(
                        f"         ⚠️  Tool name mismatch: got {func_name}, expected {test_case['expected_tool']}"
                    )
                ...
                try:
                    ...
                    for key, value in parsed_args.items():
                        if test_case["expected_param"] in str(value):
                            print(
                                f"         ✅ Expected parameter found: {key}='{value}'"
                            )
                            found_expected = True
                            break
                    ...
                    if not found_expected:
                        print(
                            f"         ⚠️  Expected parameter '{test_case['expected_param']}' not found"
                        )

                except json.JSONDecodeError:
                    print("         ⚠️  Could not parse arguments as JSON")
        else:
            print("   ❌ FAILED: No tool calls found")

    print("\n" + "=" * 65)
    print("🎯 SUMMARY:")
    print(
        "If all tests show ✅ SUCCESS, then the enhanced parser will fix your Granite issues!"
    )
