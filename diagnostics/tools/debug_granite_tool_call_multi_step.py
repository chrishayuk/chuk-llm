...
async def test_without_system_prompt():
    ...
    messages = [{"role": "user", "content": "Get the weather for Tokyo, then stop."}]
...
async def test_with_minimal_system_prompt():
    ...
    messages = [
        {
            "role": "system",
            "content": "You are a helpful assistant. Use tools when asked, then stop after completing the task.",
        },
...
async def test_with_explicit_stop_system_prompt():
    ...
    messages = [
        {
            "role": "system",
            "content": """You are an assistant that uses tools efficiently.
IMPORTANT RULES:
...
async def test_multi_city_behavior():
    ...
    messages = [
        {
            "role": "user",
            "content": "Get weather for Tokyo, New York, and Paris. Call get_weather once for each city.",
        }
    ]
    ...
    async for chunk in stream(
        ...
        if isinstance(chunk, dict) and chunk.get("tool_calls"):
            for tc in chunk["tool_calls"]:
                ...
                print(
                    f"  🔧 Tool call {call_count}: {name}({args.get('location', '?')})"
                )
...
async def compare_models():
    ...
    for model in ["granite3.3:latest", "gpt-oss:latest"]:
        ...
        response = await client.chat(
            model=model, messages=messages, tools=tools, options={"temperature": 0.1}
        )
