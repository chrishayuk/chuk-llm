...
async def test_granite_tool_streaming():
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        chuk_result = await test_chukllm(
...
async def test_chukllm(
    model_name: str, prompt: str, tools: list[dict]
) -> dict[str, Any]:
...
def analyze_results(
    chuk_result: dict, raw_result: dict, test_name: str, expected_function: str
):
    ...
    if chuk_result["success"]:
        ...
        print(
            f"      ChukLLM: {chuk_result['chunks']} chunks, {chuk_tools} tools, {chuk_time:.2f}s"
        )
    ...
    if raw_result["success"]:
        ...
        print(
            f"      Raw API: {raw_result['chunks']} chunks, {raw_tools} tools, {raw_time:.2f}s"
        )
...
def print_summary(results: dict, model_name: str):
    ...
    for test_name, data in results.items():
        ...
        print(
            f"  {status} {test_name}: ChukLLM={chuk_tools} tools, Raw={raw_tools} tools"
        )
