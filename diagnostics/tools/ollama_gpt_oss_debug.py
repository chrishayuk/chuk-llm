...
async def test_tool_response():
    ...
    try:
        completion = client.create_completion(
            messages=messages, tools=tools, stream=True, max_tokens=200, temperature=0.1
        )
        ...
        async for chunk in completion:
            ...
            if response_text:
                ...
                print(
                    f"  📄 Text chunk {chunk_count}: '{response_text[:50]}{'...' if len(response_text) > 50 else ''}'"
                )
...
async def test_thinking_levels():
    ...
    print(
        "   Reasoning models like gpt-oss will still think when prompted appropriately"
    )
    ...
    try:
        ...
        if hasattr(sync_response, "message"):
            ...
            if hasattr(msg, "thinking") and msg.thinking:
                print(
                    f"✅ Sync client supports thinking! Thinking content length: {len(msg.thinking)}"
                )
            else:
                print(
                    "ℹ️  Sync client accepted think parameter but no thinking content returned"
                )
...
async def main():
    ...
    try:
        ...
        logging.getLogger("chuk_llm.llm.providers._config_mixin").setLevel(
            logging.WARNING
        )
