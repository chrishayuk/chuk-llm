            ...
            if tool_calls:
                ...
                for i, tc in enumerate(tool_calls):
                    ...
                    if hasattr(tc, 'function'):
                        ...
                        print(f"      Function.name: {getattr(func, 'name', 'NO NAME')}")
                        print(f"      Function.arguments: {getattr(func, 'arguments', 'NO ARGS')}")
...
async def debug_ollama_client():
    ...
    async def debug_stream(self, messages, tools=None, **kwargs):
        ...
        async for chunk in original_stream(self, messages, tools, **kwargs):
            print(f"\n  📤 Yielding chunk:")
            print(f"    Response: '{chunk.get('response', '')[:50]}...' ({len(chunk.get('response', ''))} chars)")
...
async def debug_stream_function():
    ...
    print(f"Calling stream() with return_tool_calls=True")
...
async def debug_extraction_logic():
    ...
    client = OllamaLLMClient("granite3.3:latest")
