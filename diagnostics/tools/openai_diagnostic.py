...
async def test_model_access(model_name: str) -> tuple[bool, str, dict | None]:
    ...
    try:
        ...
        params = {"messages": [{"role": "user", "content": "Hello"}], "stream": False}
...
async def test_reasoning_capability(
    model_name: str, model_info: dict
) -> dict[str, Any]:
...
Reason through each step clearly."""
            ...
            params = {
                "messages": [
                    {
                        "role": "system",
                        "content": "You are a helpful assistant. Use medium reasoning effort.",
                    },
...
def analyze_reasoning_response(response_text: str, model_info: dict) -> dict[str, Any]:
    ...
    entity_score = sum(
        1 for indicator in entity_indicators if indicator in response_lower
    )
    reasoning_score = sum(
    ...
    if family == "gpt5":
        ...
        unified_score = sum(
            1
            for indicator in unified_reasoning_indicators
            if indicator in response_lower
        )

        total_score = (
            step_score + math_score + entity_score + reasoning_score + unified_score
        )
        ...
        cot_score = sum(
            1 for indicator in cot_indicators if indicator in response_lower
        )

        total_score = (
            step_score + math_score + entity_score + reasoning_score + cot_score
        )
        if has_correct_answer:
            total_score += 3
...
async def main():
        ...
        for model_info in models:
            ...
            if has_access:
                ...
                if "tier" in message.lower():
                    print("      💡 Requires higher usage tier")
                elif "not found" in message.lower():
                    print("      📅 Model may not be released yet")
                elif "parameter" in message.lower():
                    print("      🔧 Parameter handling needs update")
    ...
    if not available_models:
        ...
        print("   • Account tier restrictions")
        print("   • Models not yet released")
    ...
    for model_name in available_models:
        # Find model info
        model_info = next(
            (m for m in AVAILABLE_OPENAI_MODELS if m["name"] == model_name), {}
        )
    ...
    for family, data in family_analysis.items():
        models = data["models"]
        if models:
            data["avg_response_time"] = sum(m["response_time"] for m in models) / len(
                models
            )
            data["avg_quality_score"] = sum(
                m.get("reasoning_score", 0) for m in models
            ) / len(models)

    print("📊 FAMILY PERFORMANCE ANALYSIS:")
    print("-" * 35)
    ...
    for family, data in family_analysis.items():
        ...
        if reasoning_models > 0:
            best_model = max(
                data["reasoning_capable"], key=lambda x: x.get("reasoning_score", 0)
            )
            print(
                f"   Best performer: {best_model['model']} (score: {best_model.get('reasoning_score', 0)})"
            )
    ...
    if reasoning_capable:
        ...
        top_quality = sorted(
            reasoning_capable, key=lambda x: x.get("reasoning_score", 0), reverse=True
        )
        ...
        for i, result in enumerate(top_quality[:3], 1):
            ...
            print(
                f"   {i}. {model_name} ({family}) - {level} quality, {score} score, {time}s"
            )
        ...
        overall_best = max(
            reasoning_capable,
            key=lambda x: (
                x.get("reasoning_score", 0) * 2
                - x["response_time"]  # Quality weighted higher
            ),
        )
        print(
            f"   🏆 Best Overall: {overall_best['model']} ({overall_best.get('family', '').upper()})"
        )
        ...
        production_models = [
            r for r in reasoning_capable if r.get("family") in ["gpt5", "gpt4"]
        ]
        if production_models:
            best_prod = max(
                production_models, key=lambda x: x.get("reasoning_score", 0)
            )
            print(
                f"   🏭 Best for Production: {best_prod['model']} ({best_prod.get('family', '').upper()})"
            )

        # Best for experimentation (latest features)
        latest_models = [
            r for r in reasoning_capable if r.get("family") in ["gpt5", "o3"]
        ]
        if latest_models:
        ...
        for result in top_quality[:2]:
            ...
            print(
                f"   mcp-cli chat --provider openai --model {model_name}  # {family} family"
            )

    else:
        print("⚠️  Models responding but limited reasoning detected")
        print("Try different prompting strategies or check model availability")
    ...
    for family, features in features_by_family.items():
        available_in_family = [
            r for r in successful_models if r.get("family") == family
        ]
    ...
    if len(reasoning_capable) > 0:
        print("✨ OpenAI's new model ecosystem is ready for advanced reasoning tasks!")
    else:
        print("🔧 Consider checking API access or model availability")
