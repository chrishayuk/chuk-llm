...
async def test_raw_anthropic_proper_accumulation(api_key, messages, tools):
    ...
    try:
        ...
        print("  Using model: claude-3-5-sonnet-20241022")
                    ...
                    if event.type == "content_block_start":
                        ...
                        if hasattr(event, "delta") and hasattr(event.delta, "type"):
                            if event.delta.type == "input_json_delta":
                                ...
                                print(
                                    f"    JSON delta #{input_json_events}: '{event.delta.partial_json}'"
                                )
                                ...
                                for tool_id, tool_data in tool_calls.items():
                                    if tool_data.get("content_index") == content_index:
                                        tool_data["input_json_parts"].append(
        ...
        print("  Strategy: PROPER ACCUMULATION")
        print(
            f"  Events: {event_count}, Tool events: {tool_events}, JSON events: {input_json_events}"
        )
        ...
        if tool_calls:
            ...
            final_input = first_tool.get(
                "final_input", first_tool.get("initial_input", {})
            )
...
async def test_raw_anthropic_broken_accumulation(api_key, messages, tools):
    ...
    try:
        ...
        print("  Strategy: BROKEN (last part only)")
        print(
            f"  Events: {event_count}, Tool events: {tool_events}, JSON events: {input_json_events}"
        )
...
def analyze_anthropic_results(results):
    ...
    try:
        ...
        if len(proper_parsed) > len(broken_parsed):
            ...
            print(
                "   Anthropic streaming requires accumulating input_json_delta events"
            )
...
async def test_duplication_specifically_anthropic():
    ...
    messages = [
        {
            "role": "user",
            "content": "Call test_anthropic_tool with message 'hello world' and count 42",
        }
    ]
