    ...
    try:
        ...
        if converted:
            ...
            if sanitized_name == converted_name:
                print("  Status: ✅ CONSISTENT PROCESSING")
            else:
                print("  Status: ❌ INCONSISTENT PROCESSING")
...
async def test_anthropic_features():
    ...
    try:
        ...
        if "vision" in features:
            ...
            vision_messages = [
                {
                    ...
                    "content": [
                        {
                            "type": "text",
                            "text": "What color is this 1x1 pixel image? Be specific about the color.",
                        },
                        {
                            "type": "image_url",
                            "image_url": {
            ...
            print(
                f"   Sending image data URL: data:image/png;base64,{red_pixel_b64[:20]}..."
            )

            try:
                vision_response = await client.create_completion(
                    vision_messages, stream=False
                )
                if isinstance(vision_response, dict) and vision_response.get(
                    "response"
                ):
                    ...
                    if any(
                        ...
                        for word in ["red", "color", "pixel", "image"]
                    ):
                        print("   ✅ Vision works: Model analyzed the image content")
                    elif (
                        "don't see" in response_text.lower()
                        or "no image" in response_text.lower()
                    ):
                        print("   ❌ Vision failed: Model didn't receive the image")
                    else:
                        print(
                            "   ⚠️  Vision unclear: Model responded but unclear if image was processed"
                        )
                elif vision_response.get("error"):
                    print(f"   ❌ Vision error: {vision_response.get('error')}")
                else:
                    print("   ❌ Vision test failed: Unexpected response format")
            except Exception as e:
...
async def test_anthropic_vs_competitors():
    ...
    if len(valid_results) >= 2:
        ...
        if "anthropic" in valid_results:
            ...
            print(
                f"   Universal tools: {'✅' if anthropic_result['has_universal_tools'] else '❌'}"
            )

            # Compare to others
            other_providers = [k for k in valid_results if k != "anthropic"]
            if other_providers:
                avg_first_chunk = sum(
                ...
                if anthropic_result["first_chunk"] < avg_first_chunk:
                    print(
                        f"   ✅ Anthropic faster than average first chunk by {(avg_first_chunk - anthropic_result['first_chunk']) * 1000:.0f}ms"
                    )
                else:
                    print(
                        f"   ⚠️  Anthropic slower than average first chunk by {(anthropic_result['first_chunk'] - avg_first_chunk) * 1000:.0f}ms"
                    )

                if anthropic_result["chunks"] > avg_chunks:
                    print(
                        f"   ✅ Anthropic more granular than average ({anthropic_result['chunks']:.1f} vs {avg_chunks:.1f} chunks)"
                    )
                else:
                    print(
                        f"   ⚠️  Anthropic less granular than average ({anthropic_result['chunks']:.1f} vs {avg_chunks:.1f} chunks)"
                    )
...
async def main():
    ...
    if all([test1_passed, test2_passed, test3_passed, test4_passed, test5_passed]):
        ...
        print("\n🔧 Tool Name Examples:")
        print(
            "   stdio.read_query -> stdio_read_query (sanitized for API, restored in response)"
        )
        print(
            "   web.api:search -> web_api_search (sanitized for API, restored in response)"
        )
        print(
            "   database.sql.execute -> database_sql_execute (sanitized for API, restored in response)"
        )
    else:
