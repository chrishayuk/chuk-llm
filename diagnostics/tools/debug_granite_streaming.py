    ...
    try:
                    ...
                    if tc:
                        ...
                        if isinstance(tc, list) and len(tc) > 0:
                            ...
                            if hasattr(first_tc, '__dict__'):
                                print(f"    Tool call attrs: {list(first_tc.__dict__.keys())}")
        ...
        if tool_chunks:
            ...
            if len(tool_chunks) == 1 and tool_chunks[0] == 1:
                print(f"  📍 Pattern: Tool calls sent in FIRST chunk (optimal for latency)")
            elif all_chunks and tool_chunks[-1] == len(all_chunks):
                print(f"  📍 Pattern: Tool calls sent in FINAL chunk")
            else:
        ...
        if all_chunks:
            ...
            if hasattr(final, 'done') and final.done:
                print(f"  ✅ Stream properly terminated (done=True)")
            ...
            if tool_chunks:
                if has_final_tools:
                    print(f"  📍 Tool calls in final chunk: YES")
                else:
                    print(f"  📍 Tool calls in final chunk: NO (sent earlier in chunk {tool_chunks})")
                print(f"  ✅ STREAMING WORKING CORRECTLY - Tool calls captured!")
            else:
                print(f"  ⚠️ No tool calls found - model may not have recognized the tool use case")
        ...
        async for chunk in stream:
            ...
            if chunk.get("tool_calls"):
                for tc in chunk["tool_calls"]:
                    ...
                    fn = tc.get('function', {})
                    print(f"  🔧 Tool call in chunk {chunk_num}: {fn.get('name')}({fn.get('arguments')})")
...
async def main():
    ...
    print("\n📊 TEST RESULTS:")
    print(f"  Raw Ollama Streaming:  {'✅ PASS' if results['raw_ollama'] else '❌ FAIL'}")
    print(f"  ChukLLM Streaming:     {'✅ PASS' if results['chukllm'] else '❌ FAIL'}")
    print(f"  Non-Streaming Mode:    {'✅ PASS' if results['non_streaming'] else '❌ FAIL'}")
    print(f"  Multiple Tool Calls:   {'✅ PASS' if results['multiple_tools'] else '❌ FAIL'}")
