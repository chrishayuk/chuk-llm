...
async def test_complete_tool_chain_with_universal_names(model_name: str):
    ...
    try:
        ...
        print("✅ Official chat template applied successfully")
        ...
        if response.get("tool_calls"):
            ...
            if "stdio.list_tables" in first_call["function"]["name"]:
                ...
                return await test_step_2_list_tables_result(
                    client, conversation, response, tools, model_name, tokenizer
                )
            else:
                print(
                    f"⚠️  AI called different tool first: {first_call['function']['name']}"
                )
            ...
            if parsed_tool_calls:
                print(
                    f"✅ Parsed {len(parsed_tool_calls)} tool calls from Granite response"
                )
...
async def test_step_2_list_tables_result(
    client, conversation, response1, tools, model_name, tokenizer
):
        ...
        for i, call in enumerate(response2["tool_calls"]):
            ...
            if "stdio.describe_table" in func_name:
                ...
                if table_name:
                    ...
                    print("   ❌ stdio.describe_table called without table_name")
                    return False
        ...
        if parsed_tool_calls:
            ...
            for call in parsed_tool_calls:
                ...
                if "describe_table" in func_name:
                    ...
                    if table_name:
                        print(
                            f"   ✅ Granite tool calling successful: {func_name}(table_name='{table_name}')"
                        )
                        return True
...
async def test_step_3_schema_result(
    client, conversation, response2, tools, table_name, model_name, tokenizer
):
    ...
    if response3.get("tool_calls"):
        for _i, call in enumerate(response3["tool_calls"]):
            func_name = call["function"]["name"]
        ...
        if (
            ...
            print("   ✅ Granite provided SQL query in text response")
            return True
...
async def test_universal_parameter_extraction(model_name: str):
    ...
    try:
        ...
        for i, test_case in enumerate(test_cases):
            ...
            if "granite" in model_name.lower():
                print("   🧠 Using official WatsonX chat template for Granite")
                ...
                print("   🌟 Using official WatsonX chat template for Mistral")
                ...
                if func_name == test_case["expected_tool"]:
                    ...
                    for key, expected_value in expected_params.items():
                        ...
                        if expected_value.lower() in actual_value.lower():
                            print(
                                f"   ✅ {key}: '{actual_value}' contains expected '{expected_value}'"
                            )
                            success = True
                        else:
                            print(
                                f"   ❌ {key}: '{actual_value}' doesn't contain '{expected_value}'"
                            )
                ...
                if parsed_tool_calls:
                    ...
                    if test_case["expected_tool"] in func_name:
                        ...
                        for key, expected_value in expected_params.items():
                            actual_value = parsed_args.get(key, "")
                            if expected_value.lower() in actual_value.lower():
                                print("   ✅ Granite parameter extraction successful")
                                success = True
                                break
...
async def main():
    ...
    if len(passed_models) == total_models:
        ...
        print("\n🔧 PROVEN CAPABILITIES:")
        print("   ✅ Official IBM Granite chat templates with tools")
        print("   ✅ Universal tool names work on all WatsonX models")
        print("   ✅ Granite and Mistral models both compatible")
        print("   ✅ Tool name sanitization and restoration")
        print("   ✅ Complete conversation flows work")
        print("   ✅ Parameter extraction from various formats")

        print("\n🚀 PRODUCTION READY:")
        print("   • mcp-cli chat --provider watsonx --model ibm/granite-3-8b-instruct")
        print(
            "   • mcp-cli chat --provider watsonx --model mistralai/mistral-medium-2505"
        )
        print("   • Official chat templates ensure compatibility")
        print("   • No hacky workarounds needed")
...
if __name__ == "__main__":
    print(
        "🚀 Starting WatsonX Complete Tool Chain Test with Official Chat Templates..."
    )
    asyncio.run(main())
