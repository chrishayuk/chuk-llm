...
def parse_granite_streaming_response(text: str) -> list[dict[str, Any]]:
    ...
    try:
        ...
        if tool_calls:
            print(
                f"    🧠 Parsed {len(tool_calls)} Granite tool calls from streaming text"
            )
...
async def test_streaming_duplication_prevention(model_name: str):
    ...
    try:
        ...
        if "granite" in model_name.lower():
            ...
            async for chunk in client.create_completion(
                ...
                if chunk.get("response"):
                    ...
                    if any(
                        ...
                        print(
                            f"    📝 Granite format in chunk {chunk_count}: {text[:50]}..."
                        )
                ...
                if chunk.get("tool_calls"):
                    tool_chunks += 1
                    print(
                        f"    🔧 Structured tool chunk {chunk_count}: {len(chunk['tool_calls'])} tool(s)"
                    )
            ...
            async for chunk in client.create_completion(
                messages=conversation, tools=tools, stream=True, max_tokens=600
            ):
                chunk_count += 1
        ...
        has_tool_activity = (
            ...
            or len(watsonx_parsed_calls) > 0
            or granite_format_chunks > 0
        )
        ...
        if no_duplicates:
            ...
            if has_tool_activity:
                print("   ✅ Tool calling activity detected")

                if len(all_tool_calls) > 0:
                    print("   ✅ Structured tool calls working")
                if len(watsonx_parsed_calls) > 0:
                    print("   ✅ WatsonX text-based tool calls working")
                if granite_format_chunks > 0:
                    print("   ✅ WatsonX tool format patterns detected")

                return True
            else:
                print("   ⚠️  No tool calling activity detected")
                return False
        else:
...
async def test_granite_vs_mistral_streaming():
    ...
    for model_id, model_name in models:
        ...
        try:
            ...
            if "granite" in model_id.lower():
                ...
                print("   🧠 Using official Granite chat template")
                ...
                async for chunk in client.create_completion(
                    ...
                    if chunk.get("response"):
                        ...
                        granite_calls = parse_granite_streaming_response(
                            chunk["response"]
                        )
                ...
                print("   🌟 Using standard WatsonX API")

                async for chunk in client.create_completion(
                    messages=conversation, tools=tools, stream=True, max_tokens=300
                ):
                    chunk_count += 1
    ...
    if len(successful_models) >= 1:
        ...
        duplication_free = [
            ...
            if not result.get("duplication_detected")
        ]
        if duplication_free:
            print(f"   ✅ Duplication-free models: {duplication_free}")

        print("   ✅ STREAMING COMPATIBILITY ACHIEVED!")
        return True
    else:
        print("   ❌ No successful streaming models")
        return False
...
async def main():
    ...
    duplication_tests = [k for k in results if "duplication" in k]
    duplication_passed = sum(1 for k in duplication_tests if results[k])

    print(
        f"   Duplication Prevention: {duplication_passed}/{len(duplication_tests)} models passed"
    )
    print(
        f"   Granite vs Mistral Comparison: {'✅ PASS' if results['comparison'] else '❌ FAIL'}"
    )
    ...
    if total_passed == total_tests:
        ...
        print("   • Official AutoTokenizer.apply_chat_template() works")
        print("   • Tool call duplication prevented")
        print("   • Cross-model compatibility achieved")
