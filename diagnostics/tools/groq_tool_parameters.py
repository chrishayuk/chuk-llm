...
async def test_groq_specific_features():
    ...
    try:
        ...
        response = await client.create_completion(
            messages=messages, tools=complex_tools, stream=False, max_tokens=300
        )
        ...
        if response.get("tool_calls"):
            ...
            if parsed_args.get("model_type"):
                print("   ✅ Groq successfully handled complex schema")
                return True
            else:
                print("   ⚠️  Complex schema partially handled")
                return True

        elif response.get("response"):
            print("   ⚠️  Groq provided text response instead of tool call")
            if "mixtral" in response["response"].lower():
                print("   ✅ Response mentions model type - acceptable fallback")
                return True
