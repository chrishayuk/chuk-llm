...
async def test_streaming_with_models():
    ...
    test_cases = [
        ...
        },
        {
            ...
            "messages": [
                {
                    "role": "user",
                    "content": "If a train leaves at 2pm going 60mph, and another at 3pm going 80mph, when do they meet if they're 200 miles apart? Think step by step.",
                }
            ],
            "tools": None,
    ...
    for model, model_desc in test_models:
        ...
        print(
            f"   📋 Family: {model_info['family']} | Generation: {model_info['generation']}"
        )
        print(
            f"   🔧 Tools: {model_info['supports_tools']} | Streaming: {model_info['supports_streaming']}"
        )
        ...
        for test_case in test_cases:
            ...
            if (
                test_case.get("requires_tools", False)
                and not model_info["supports_tools"]
            ):
                should_skip = True
            ...
            if model_info["supports_streaming"]:
                ...
                raw_result = {
                    "success": False,
                    "error": "Model doesn't support streaming",
...
async def test_chuk_llm_streaming(model, model_info, messages, tools):
    ...
    try:
        ...
        stream_params = {"provider": "openai", "model": model}
        ...
        async for chunk in stream(messages[-1]["content"], **stream_params):
            chunk_count += 1
            if chunk:
                chunk_str = str(chunk)
...
def analyze_single_result_updated(
    chuk_result, raw_result, test_name, test_case, model_info
):
    ...
    if not chuk_result["success"]:
        if "doesn't support streaming" in chuk_result.get("error", ""):
            print(
                f"      ℹ️  ChukLLM: {chuk_result['error']} (expected for {model_info['family']})"
            )
    ...
    if not raw_result["success"]:
        if "doesn't support streaming" in raw_result.get("error", ""):
            print("      ℹ️  Raw OpenAI: Model limitation (expected)")
    ...
    if model_info["family"] == "gpt5":
        ...
        print(
            f"      ℹ️  {model_info['generation']} doesn't support streaming (expected)"
        )
        return True
    else:
    ...
    if evaluation_type == "tool_streaming":
        ...
        if chuk_streamed and (has_tool_info or chuk_tool_count > 0):
            print(
                f"      ✅ ChukLLM tool streaming works ({chuk_result['chunks']} chunks)"
            )
            ...
            print(
                f"      ✅ ChukLLM tool streaming likely works ({chuk_result['chunks']} chunks)"
            )
            success = True
        else:
            print("      ❌ ChukLLM tool streaming failed")
            success = False

        print(
            f"      📊 Raw OpenAI: {raw_result['chunks']} chunks, {raw_tool_count} tool calls"
        )
        ...
        chuk_reasoning = sum(
            ...
            if indicator in chuk_result["final_response"].lower()
        )

        if chuk_streamed and chuk_result["chunks"] >= expected_chunks:
            print(
                f"      ✅ ChukLLM reasoning streaming works ({chuk_result['chunks']} chunks)"
            )
            print(f"      🧠 Reasoning indicators: {chuk_reasoning}")
            success = True
        else:
            print("      ❌ ChukLLM reasoning streaming needs improvement")
            success = False
        ...
        if chuk_streamed and chuk_result["chunks"] >= expected_chunks:
            ...
            chunk_ratio = (
                chunk_diff / max(raw_result["chunks"], 1)
                if raw_result["chunks"] > 0
                else 0
            )

            if chunk_ratio < (1 - success_threshold):
                print(
                    f"      ✅ Both streamed well (ChukLLM: {chuk_result['chunks']}, Raw: {raw_result['chunks']})"
                )
                success = True
            else:
                print(
                    f"      ✅ ChukLLM streaming works ({chuk_result['chunks']} chunks)"
                )
                success = True
        else:
            print("      ❌ ChukLLM streaming needs improvement")
            success = False

    # Show response previews
    chuk_preview = (
        chuk_result["final_response"][:100] if chuk_result["final_response"] else ""
    )
    raw_preview = (
        raw_result["final_response"][:100] if raw_result["final_response"] else ""
    )
...
def analyze_all_results_updated(results, overall_success):
    ...
    for model, model_data in results.items():
        ...
        for _test_name, test_result in model_data["results"].items():
            ...
            if chuk["success"] and chuk.get("error") is None:
                ...
                if test_result["evaluation_type"] == "tool_streaming" and chuk.get(
                    "has_tool_call_info", False
                ):
                ...
                chuk["success"]
                and chuk.get("error")
                and "doesn't support" in chuk.get("error", "")
            ):
    ...
    for family, stats in family_stats.items():
        ...
        family_emoji = {"gpt5": "🌟", "o3_plus": "🚀", "o1": "🧠", "gpt4": "⚡"}.get(
            family, "📦"
        )
        ...
        print(
            f"    Success rate: {stats['successes']}/{stats['tests']} ({100 * success_rate:.0f}%)"
        )
    ...
    if gpt5_stats:
        gpt5_rate = (
            gpt5_stats["successes"] / gpt5_stats["tests"]
            if gpt5_stats["tests"] > 0
            else 0
        )
        print(
            f"  🌟 GPT-5 Family: {100 * gpt5_rate:.0f}% success rate - {'Excellent' if gpt5_rate >= 0.8 else 'Needs work'}"
        )
    ...
    if o3_stats:
        o3_rate = (
            o3_stats["successes"] / o3_stats["tests"] if o3_stats["tests"] > 0 else 0
        )
        print(
            f"  🚀 Modern Reasoning (O3+): {100 * o3_rate:.0f}% success rate - {'Excellent' if o3_rate >= 0.7 else 'Needs work'}"
        )
    if o1_stats:
        print("  🧠 Legacy Reasoning (O1): Expected limitations (no streaming/tools)")

    # GPT-4 Analysis
    gpt4_stats = family_stats.get("gpt4", {})
    if gpt4_stats:
        gpt4_rate = (
            gpt4_stats["successes"] / gpt4_stats["tests"]
            if gpt4_stats["tests"] > 0
            else 0
        )
        print(
            f"  ⚡ GPT-4 Family: {100 * gpt4_rate:.0f}% success rate - {'Excellent' if gpt4_rate >= 0.9 else 'Good' if gpt4_rate >= 0.7 else 'Needs work'}"
        )
    ...
    for model, model_desc in gpt5_models:
        ...
        try:
            ...
            shows_work = any(
                ...
                for indicator in ["eat", "give", "buy", "cookies"]
            )
            ...
            has_tool_mention = (
                "calculate_sqrt" in tool_response or "sqrt" in tool_response.lower()
            )
            ...
            if reasoning_success and tool_success:
                print(
                    f"  ✅ {model} excellent: unified reasoning + tools work perfectly"
                )
                success_count += 1
