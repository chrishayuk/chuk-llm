...
async def test_gemini_streaming():
    ...
    try:
        ...
        messages = [
            {
                "role": "user",
                "content": "Write a creative story about a time traveler who discovers something unexpected. Make it at least 150 words with vivid details.",
            }
        ]
...
async def test_gemini_streaming_patterns():
    ...
    test_cases = [
        ...
        (
            "Long response",
            "Write a detailed story about space exploration with multiple characters",
        ),
        (
            "Technical response",
            "Explain the differences between neural networks and decision trees",
        ),
        (
            "Creative response",
            "Create a dialogue between two AI systems discussing creativity",
        ),
    ]
    ...
    try:
        ...
        for test_name, prompt in test_cases:
            ...
            streaming_duration = (
                total_time - first_chunk_time if first_chunk_time else 0
            )

            print(
                f"   Chunks: {chunk_count}, First: {first_chunk_time:.3f}s, "
...
async def test_gemini_vs_others():
    ...
    if len(valid_results) >= 2:
        ...
        fastest_provider = min(
            valid_results.keys(), key=lambda p: valid_results[p]["first_chunk"]
        )
        ...
        most_chunks_provider = max(
            valid_results.keys(), key=lambda p: valid_results[p]["chunks"]
        )
        most_chunks = valid_results[most_chunks_provider]["chunks"]

        print(
            f"   📊 Most granular streaming: {most_chunks_provider} ({most_chunks} chunks)"
        )

        # Gemini-specific analysis
        if "gemini" in valid_results:
            gemini_result = valid_results["gemini"]
            other_providers = [p for p in valid_results if p != "gemini"]

            if other_providers:
                avg_other_first_chunk = sum(
