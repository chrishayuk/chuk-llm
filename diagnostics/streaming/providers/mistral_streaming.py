...
async def test_mistral_model_capabilities():
    ...
    for model, description in models_to_test:
        ...
        try:
            ...
            print(
                f"   Streaming: {chunk_count} chunks, first at {first_chunk_time:.3f}s"
                if first_chunk_time
...
async def test_mistral_vs_competitors():
    ...
    for provider, model in providers:
        ...
        try:
            ...
            print(
                f"   {provider}: {chunk_count} chunks, first at {first_chunk_time:.3f}s, {content_length} chars, total {total_time:.3f}s"
            )
    ...
    if len(valid_results) >= 2:
        ...
        if "mistral" in valid_results:
            ...
            # Compare to others
            other_providers = [k for k in valid_results if k != "mistral"]
            if other_providers:
                avg_first_chunk = sum(
                ...
                if mistral_result["first_chunk"] < avg_first_chunk:
                    print(
                        f"   ✅ Mistral faster than average first chunk by {(avg_first_chunk - mistral_result['first_chunk']) * 1000:.0f}ms"
                    )
                else:
                    print(
                        f"   ⚠️  Mistral slower than average first chunk by {(mistral_result['first_chunk'] - avg_first_chunk) * 1000:.0f}ms"
                    )

                if mistral_result["chunks"] > avg_chunks:
                    print(
                        f"   ✅ Mistral more granular than average ({mistral_result['chunks']:.1f} vs {avg_chunks:.1f} chunks)"
                    )
                else:
                    print(
                        f"   ⚠️  Mistral less granular than average ({mistral_result['chunks']:.1f} vs {avg_chunks:.1f} chunks)"
                    )
...
async def test_mistral_features():
    ...
    try:
        ...
        if isinstance(response, dict) and response.get("tool_calls"):
            print(
                f"   ✅ Function calling works: {len(response['tool_calls'])} tool calls"
            )
            for tool_call in response["tool_calls"]:
                print(f"      Tool: {tool_call.get('function', {}).get('name')}")
        elif isinstance(response, dict) and response.get("response"):
            print(
                f"   ⚠️  Got text response instead of tool call: {response['response'][:100]}..."
            )
        else:
    ...
    try:
        ...
        messages = [
            {
                "role": "user",
                "content": "Think step by step: If a train leaves station A at 2 PM traveling at 60 mph, and another train leaves station B at 3 PM traveling at 80 mph toward station A, and the stations are 200 miles apart, when will they meet?",
            }
        ]
        ...
        if isinstance(response, dict) and response.get("response"):
            ...
            if any(
                ...
                for phrase in ["step", "first", "then", "calculate", "therefore"]
            ):
                print("   ✅ Shows reasoning structure")
            else:
                print("   ⚠️  May not show explicit reasoning")
        else:
