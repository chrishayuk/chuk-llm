...
async def test_ollama_streaming():
    ...
    try:
        ...
        messages = [
            {
                "role": "user",
                "content": "Write a short creative story about a robot learning to cook. Make it at least 100 words with good details.",
            }
        ]
        ...
        if hasattr(response, "__aiter__"):
            ...
            if streaming_duration > 0 and len(full_response) > 0:
                ...
                print(
                    f"   📈 Local performance: {chars_per_second:.0f} chars/sec, ~{tokens_per_second:.0f} tokens/sec"
                )
...
async def test_ollama_model_comparison():
    ...
    models_to_test = ["gemma3", "llama3", "granite3.3", "mistral"]
    ...
    for model in models_to_test:
        ...
        try:
            ...
            successful_tests += 1
            print(
                f"   ✅ {model}: {chunk_count} chunks, first at {first_chunk_time:.3f}s, {total_chars} chars"
            )
    ...
    if successful_tests >= 2:
        ...
        for model, data in valid_results.items():
            speed_rating = (
                ...
                if data["first_chunk"] < 5.0
                else "⚠️"
            )
            chunk_rating = "📊" if data["chunks"] > 10 else "📉"
            print(
                f"   {speed_rating}{chunk_rating} {model}: {data['first_chunk']:.1f}s first chunk, {data['chunks']} chunks"
            )
