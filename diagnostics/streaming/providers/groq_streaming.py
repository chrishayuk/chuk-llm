...
async def test_groq_streaming():
    ...
    try:
        ...
        messages = [
            {
                "role": "user",
                "content": "Write an engaging short story about a detective solving a mystery in a futuristic city. Make it at least 200 words with rich descriptions.",
            }
        ]
...
async def test_groq_speed_variants():
    ...
    if len(valid_results) >= 2:
        ...
        best_streaming = min(
            valid_results.keys(), key=lambda m: valid_results[m]["avg_interval"]
        )
        best_interval = valid_results[best_streaming]["avg_interval"]

        print(
            f"   ⚡ Best streaming: {best_streaming} ({best_interval * 1000:.1f}ms avg)"
        )

        # Speed rankings
        print("\n   🏆 Speed Rankings:")
        sorted_by_speed = sorted(
            valid_results.items(), key=lambda x: x[1]["first_chunk"]
        )
