...
async def test_watsonx_streaming():
    ...
    try:
        ...
        messages = [
            {
                "role": "user",
                "content": "Write a creative story about artificial intelligence helping solve climate change. Make it at least 150 words with vivid details.",
            }
        ]
        ...
        if hasattr(response, "__aiter__"):
            ...
            if streaming_duration > 0 and len(full_response) > 0:
                ...
                print(
                    f"   📈 Enterprise performance: {chars_per_second:.0f} chars/sec, ~{tokens_per_second:.0f} tokens/sec"
                )
                ...
                if "granite" in model_used.lower():
                    ...
                    print(
                        "   🦙 Meta Llama: Open-source model on enterprise infrastructure"
                    )
...
async def test_watsonx_model_comparison():
    ...
    for model_id, model_name in models_to_test:
        ...
        try:
            ...
            successful_tests += 1
            print(
                f"   ✅ {model_name}: {chunk_count} chunks, first at {first_chunk_time:.3f}s, {total_chars} chars"
            )
    ...
    if successful_tests >= 2:
        ...
        # Find fastest model
        fastest_model = min(
            valid_results.keys(), key=lambda m: valid_results[m]["first_chunk"]
        )
        ...
        best_streaming = min(
            valid_results.keys(),
            key=lambda m: valid_results[m]["total_time"]
            / max(valid_results[m]["chunks"], 1),
        )
        ...
        granite_models = [
            (k, v) for k, v in valid_results.items() if "granite" in k.lower()
        ]
        if granite_models:
            ...
            for model_id, data in granite_models:
                ...
                speed_rating = (
                    "🚀"
                    if data["first_chunk"] < 2.0
                    else "✅"
                    if data["first_chunk"] < 4.0
                    else "⚠️"
                )
                print(
                    f"     {speed_rating} Granite {size}: {data['first_chunk']:.1f}s first chunk, {data['chunks']} chunks"
                )

        llama_models = [
            (k, v) for k, v in valid_results.items() if "llama" in k.lower()
        ]
        if llama_models:
            ...
            for model_id, data in llama_models:
                ...
                print(
                    f"     {speed_rating} Llama {size}: {data['first_chunk']:.1f}s first chunk, {data['chunks']} chunks"
                )
        ...
        if granite_models:
            print(
                "   • Granite models: Best for enterprise compliance, IBM ecosystem integration"
            )
        if llama_models:
            print("   • Llama models: Good for cost-effective enterprise workloads")
        print(
            "   • Mistral models: European AI compliance, strong reasoning capabilities"
        )
...
async def test_watsonx_enterprise_features():
    ...
    try:
        ...
        if successful_tests > 0:
            ...
            print(
                f"   Success rate: {successful_tests}/{total_tests} ({successful_tests / total_tests * 100:.0f}%)"
            )
