...
def classify_ollama_model(model_name):
    ...
    # Large general models
        ...
        for pattern in ["gpt-oss", "mistral-small", "mistral-nemo"]
    ):
        return {
            "type": "large_general",
            "supports_tools": True,
...
async def test_streaming_with_ollama_models():
    ...
    try:
        ...
        for model_data in models:
            ...
            if (
                classification["type"] != "embedding"
                and classification["supports_streaming"]
            ):
        ...
        for model_name, classification in test_models:
            print(
                f"   • {model_name} [{classification['type']}] - {classification['specialization']}"
            )
    ...
    test_cases = [
        ...
        },
        {
            ...
            "messages": [
                {
                    "role": "user",
                    "content": "Write a Python function to calculate the factorial of a number. Include comments.",
                }
            ],
        ...
        },
        {
            ...
            "messages": [
                {
                    "role": "user",
                    "content": "What are the main advantages and disadvantages of renewable energy? Be comprehensive.",
                }
            ],
    ...
    for model_name, classification in test_models:
        ...
        for test_case in test_cases:
            ...
            if classification["type"] == "code" and case_name not in [
                "Simple Response",
                "Code Generation",
            ]:
            ...
            if classification["type"] == "reasoning" and case_name == "Simple Response":
                print(
                    f"  ⏭️ Skipping {case_name} (testing reasoning capabilities instead)"
                )
                continue
...
def analyze_ollama_result(
    chuk_result, raw_result, test_name, test_case, classification
):
    ...
    if (
        evaluation_type == "reasoning_streaming"
        and classification["type"] == "reasoning"
    ):
        ...
        if chuk_streamed and chuk_meets_minimum and reasoning_found >= 2:
            print(
                f"      ✅ Excellent reasoning streaming ({chuk_result['chunks']} chunks, {reasoning_found} indicators)"
            )
            success = True
            ...
            print("      ⚠️  Reasoning streaming needs improvement")
            success = False
        ...
        if chuk_streamed and chuk_meets_minimum and code_found >= 2:
            print(
                f"      ✅ Good code streaming ({chuk_result['chunks']} chunks, code detected)"
            )
            success = True
        elif chuk_streamed and chuk_meets_minimum:
            ...
            print("      ⚠️  Code streaming could be better")
            success = False

    else:  # General text streaming
        if chuk_streamed and chuk_meets_minimum:
            # Compare with raw Ollama performance
            chunk_ratio = abs(chuk_result["chunks"] - raw_result["chunks"]) / max(
                raw_result["chunks"], 1
            )

            if chunk_ratio < 0.5:  # Similar performance
                print(
                    f"      ✅ Excellent streaming (ChukLLM: {chuk_result['chunks']}, Raw: {raw_result['chunks']})"
                )
                success = True
            else:
                print(
                    f"      ✅ Good streaming (ChukLLM: {chuk_result['chunks']}, Raw: {raw_result['chunks']})"
                )
                success = chuk_meets_minimum
        elif chuk_streamed:
            print(
                f"      ⚠️  Streaming but low chunks ({chuk_result['chunks']} < {min_chunks})"
            )
            success = False
        else:
    ...
    chuk_preview = (
        chuk_result["final_response"][:120] if chuk_result["final_response"] else ""
    )
    if chuk_preview:
...
def analyze_all_ollama_results(results, overall_success):
    ...
    for model_type, perf in type_performance.items():
        if perf["total"] > 0:
            ...
            print(
                f"  {model_type.title()}: {success_rate:.0f}% success, {streaming_rate:.0f}% streaming"
            )
    ...
    final_success = (
        success_rate >= 0.7 and streaming_rate >= 0.8
    )  # 70% success, 80% streaming
...
async def test_ollama_specific_features():
    ...
    for model_name, description in specific_tests:
        ...
        try:
            ...
            async for chunk in stream(
                prompt, provider="ollama", model=model_name, max_tokens=400
            ):
                chunk_count += 1
                if chunk:
...
async def main():
    ...
    print(
        f"Multi-model streaming tests: {'✅ PASS' if multi_model_ok else '⚠️ PARTIAL'}"
    )
