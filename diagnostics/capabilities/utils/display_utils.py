...
class DiagnosticDisplay:
    ...
    def _print_performance_summary(results: list[ProviderResult]):
        ...
        providers_with_all_features = len(
            [r for r in results if len(r.feature_set) >= 4]
        )
        ...
        console.print(
            Panel(perf_text, title="🚀 Performance Overview", border_style="blue")
        )
    ...
    def _print_error_analysis(results: list[ProviderResult]):
        ...
        for provider, stage, message in errors:
            ...
            error_table.add_row(
                provider, f"[{stage_style}]{stage}[/{stage_style}]", message
            )
    ...
    def _print_results_table(results: list[ProviderResult]):
        ...
        results_table.add_column(
            "Success Rate", justify="center", style="bold", min_width=12
        )
        ...
        for result in results:
            ...
            model_summary = ", ".join(
                f"{cap}:{model}" for cap, model in result.models.items()
            )
    ...
    def _print_plain_summary(results: list[ProviderResult]):
        ...
        for result in results:
            ...
            print(
                f"  Capabilities: {result.successful_capabilities}/{result.total_capabilities}"
            )
            ...
            if result.timings:
                ...
                print(
                    f"  Timing: avg={avg_time:.2f}s, fastest={fastest}, slowest={slowest}"
                )
    ...
    @staticmethod
    def print_live_tick(
        provider: str, capability: str, success: bool | None, timing: float = None
    ):
        ...
        if HAS_RICH:
            console.print(
                f"[cyan][{provider}][/cyan] {capability:<12} {status_icon}{timing_str}"
            )
        else:
    ...
    def print_capabilities_matrix(results: list[ProviderResult]):
        ...
        if HAS_RICH:
            ...
            for result in results:
                ...
                row.append(
                    DiagnosticDisplay._format_bool(result.streaming_function_call)
                )
            ...
            for result in results:
                ...
                stream_icon = {True: "✅", False: "❌", None: "—"}[
                    result.streaming_text
                ]
                tools_icon = {True: "✅", False: "❌", None: "—"}[result.function_call]
                stream_tools_icon = {True: "✅", False: "❌", None: "—"}[
                    result.streaming_function_call
                ]
                vision_icon = {True: "✅", False: "❌", None: "—"}[result.vision]

                print(
                    f"{result.provider:<12} {text_icon:<6} {stream_icon:<8} {tools_icon:<7} {stream_tools_icon:<5} {vision_icon:<8}"
                )
