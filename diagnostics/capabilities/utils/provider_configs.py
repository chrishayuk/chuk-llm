...
class DynamicProviderConfig:
    ...
    def provider_config(self):
        ...
        if self._provider_config is None:
            try:
                self._provider_config = self.config_manager.get_provider(
                    self.provider_name
                )
            except ValueError:
                ...
                self._provider_config = ProviderConfig(
                    name=self.provider_name, features=set(), models=[], model_aliases={}
                )
        return self._provider_config

    def supports_feature(self, feature: str, model: str | None = None) -> bool:
        ...
        try:
            ...
            if feature in feature_map:
                return self.provider_config.supports_feature(
                    feature_map[feature], model
                )
    ...
    def get_error_categories(self) -> dict[str, list[str]]:
        ...
        base_categories = {
            ...
            "auth_error": ["invalid api key", "authentication failed", "unauthorized"],
            "connection_error": [
                "connection refused",
                "connection failed",
                "connection error",
    ...
    def get_supported_features(self, model: str | None = None) -> set[str]:
...
def is_vision_supported(provider: str, model: str | None = None) -> bool:
...
def validate_provider_setup(provider: str) -> dict[str, Any]:
    ...
    try:
        from chuk_llm.configuration.unified_config import ConfigValidator, get_config
...
def check_capability_support(
    provider: str, feature: str, model: str | None = None
) -> bool:
