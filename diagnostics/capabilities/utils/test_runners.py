...
class CapabilityTester:
    ...
    async def safe_client_call(self, provider: str, model: str, operation_name: str, operation_func):
        ...
        for attempt in range(self.connection_retries + 1):
            try:
                ...
                if provider_lower in self.problematic_providers:
                    try:
                        result = await asyncio.wait_for(operation_func(client), timeout=30.0)
                        return result
                    except asyncio.TimeoutError:
                        if attempt < self.connection_retries:
                            print(f"🔄 {provider}: Timeout on {operation_name} (attempt {attempt + 2})")
                            await asyncio.sleep(1.0)
                            continue
                        else:
                            raise Exception(f"Timeout after {self.connection_retries + 1} attempts")
                else:
                ...
                if is_connection_error and attempt < self.connection_retries:
                    print(f"🔄 {provider}: Retrying {operation_name} (attempt {attempt + 2}) due to connection issue")
    ...
    def _get_best_model_for_capability(self, provider: str, model: str, capability: str) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            for available_model in provider_config.models:
                if provider_config.supports_feature(feature, available_model):
                    print(f"🔄 {provider}: Using {available_model} for {capability} (default {model} doesn't support it)")
            ...
            for alias, actual_model in provider_config.model_aliases.items():
                if provider_config.supports_feature(feature, actual_model):
                    print(f"🔄 {provider}: Using {actual_model} (alias: {alias}) for {capability}")
    ...
    def _find_vision_model(self, provider_config, default_model: str) -> Tuple[bool, Optional[str]]:
        ...
        for alias, actual_model in provider_config.model_aliases.items():
            if "vision" in alias.lower() or "pixtral" in alias.lower():
                if provider_config.supports_feature(Feature.VISION, actual_model):
                    print(f"🔄 {provider_name}: Using vision model {actual_model} (alias: {alias})")
                    return True, actual_model
    ...
    def _should_mark_as_unsupported(self, provider: str, capability: str, error_msg: str) -> bool:
        ...
        if provider_lower == "mistral" and capability in ["tools", "stream_tools"] and "i/o operation" in error_lower:
            return True
        
        if provider_lower == "perplexity" and "i/o operation" in error_lower:
            return True
    ...
    async def test_text_completion(self, provider: str, model: str, result_obj, tick_fn: Callable):
        ...
        try:
            response = await self.safe_client_call(provider, model, "text completion", do_text_test)
            ...
            if self._should_mark_as_unsupported(provider, "text", error_msg):
                print(f"💤 {provider}: Marking text as unsupported due to persistent issues")
    ...
    async def test_streaming(self, provider: str, model: str, result_obj, tick_fn: Callable):
        ...
        supports, best_model = self._get_best_model_for_capability(provider, model, "streaming")
        if not supports:
        ...
        try:
            success = await self.safe_client_call(provider, best_model, "streaming", do_streaming_test)
            ...
            if self._should_mark_as_unsupported(provider, "stream", error_msg):
                print(f"💤 {provider}: Marking streaming as unsupported due to persistent issues")
                result_obj.record("streaming_text", None)
                tick_fn("stream", None)
            else:
    ...
    async def test_tools(self, provider: str, model: str, result_obj, tick_fn: Callable):
        ...
        supports, best_model = self._get_best_model_for_capability(provider, model, "tools")
        if not supports:
        ...
        try:
            response = await self.safe_client_call(provider, best_model, "tools", do_tools_test)
            ...
            if any(phrase in error_msg for phrase in unsupported_patterns):
                ...
                print(f"💤 {provider}: Marking tools as unsupported due to persistent I/O issues")
                result_obj.record("function_call", None)
                tick_fn("tools", None)
            else:
    ...
    async def test_streaming_tools(self, provider: str, model: str, result_obj, tick_fn: Callable):
        ...
        streaming_supports, streaming_model = self._get_best_model_for_capability(provider, model, "streaming")
        tools_supports, tools_model = self._get_best_model_for_capability(provider, model, "tools")
        ...
        try:
            success = await self.safe_client_call(provider, best_model, "streaming tools", do_streaming_tools_test)
            ...
            if any(phrase in error_msg for phrase in unsupported_patterns):
                ...
                print(f"💤 {provider}: Marking streaming tools as unsupported due to persistent I/O issues")
                result_obj.record("streaming_function_call", None)
                tick_fn("stream_tools", None)
            else:
    ...
    async def test_vision(self, provider: str, model: str, result_obj, tick_fn: Callable):
        ...
        supports, best_model = self._get_best_model_for_capability(provider, model, "vision")
        if not supports:
        ...
        try:
            response = await self.safe_client_call(provider, best_model, "vision", do_vision_test)
            ...
            if any(pattern in error_msg for pattern in unsupported_patterns):
                ...
                print(f"💤 {provider}: Marking vision as unsupported due to persistent I/O issues")
                result_obj.record("vision", None)
                tick_fn("vision", None)
            else:
