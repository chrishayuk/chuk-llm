...
class BaseURLDiagnostic:
        ...
        for pattern in patterns:
            ...
            try:
                ...
                if resolved == test_url:
                    self.add_result(
                        f"Pattern {pattern}", True, f"Resolved to {test_url}"
                    )
                else:
                    self.add_result(
                        f"Pattern {pattern}",
                ...
                self.add_result(f"Pattern {pattern}", False, f"Error: {e}")
    ...
    def test_custom_env_variable(self):
        ...
        try:
            ...
            self.add_result("Custom env variable", False, f"Error: {e}")
    ...
    def test_priority_order(self):
        ...
        try:
            ...
            if resolved == configured_url:
                self.add_result(
                    "Configured URL only", True, "Uses configured URL when no env vars"
                )
            else:
                self.add_result(
                    "Configured URL only",
            ...
            if resolved == custom_url:
                self.add_result(
                    "Custom env override", True, "Custom env overrides standard pattern"
                )
            else:
                self.add_result(
                    "Custom env override",
            ...
            self.add_result("Priority order", False, f"Error: {e}")
    ...
    def test_update_with_env(self):
        ...
        try:
            ...
            update_provider("update_test", api_base_env=update_env)
            ...
            self.add_result("Update with api_base_env", False, f"Error: {e}")
    ...
    def test_existing_providers(self):
        ...
        try:
            ...
            self.add_result("OpenAI provider env override", False, f"Error: {e}")
