...
async def test_latest_resolution():
    ...
    try:
        ...
        result_devstral_latest = manager._ensure_model_available('ollama', 'devstral:latest')
        
        print(f"   🧪 _ensure_model_available('devstral'): {result_devstral}")
        print(f"   🧪 _ensure_model_available('devstral:latest'): {result_devstral_latest}")
    ...
    try:
        ...
        print(f"   🔍 'devstral:latest' in discovered: {'devstral:latest' in discovered_names}")
        ...
        print(f"   🔍 'devstral:latest' in provider: {'devstral:latest' in provider_after.models}")
...
async def main():
    ...
    print(f"\n🎯 Summary:")
    print(f"   • Discovery system finds 48 models including devstral:latest")
    print(f"   • devstral:latest works perfectly")
    print(f"   • devstral (without :latest) should resolve to devstral:latest")
    print(f"   • If resolution fails, the issue is in client factory or model resolution")
