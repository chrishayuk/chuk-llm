...
# noqa: E402 - imports must come after load_dotenv()
import os
...
def test_basic_configuration():
    ...
    try:
        ...
        ollama_functions = [
            name for name in functions if "ollama" in name and name.startswith("ask_")
        ]
        ...
        if hasattr(chuk_llm, func_name):
            try:
                ...
                if func_name.endswith("_sync"):
                    ...
                    print(
                        f"   (Skipping async function {func_name} - would need await)"
                    )
                    continue
...
def test_provider_status():
    ...
    print("🔧 Ollama: Local service (no API key needed)")
...
def test_import_structure():
    ...
    functions = [
        ...
        if callable(getattr(chuk_llm, attr, None)) and not attr.startswith("_")
    ]
    classes = [
        attr for attr in all_attrs if attr[0].isupper() and not attr.startswith("_")
    ]
    constants = [
        attr
        for attr in all_attrs
        if attr.isupper() and not callable(getattr(chuk_llm, attr, None))
    ]
    ...
    key_functions = [
        ...
        if any(keyword in attr for keyword in ["ask", "stream", "config", "show"])
    ]
...
def test_ollama_specific():
    ...
    try:
        ...
        ollama_sync = [
            name
            for name in functions
            if name.startswith("ask_ollama_") and name.endswith("_sync")
        ]
        ollama_stream = [
            name for name in functions if name.startswith("stream_ollama_")
        ]
        ...
        print(
            f"Total Ollama functions: {len(ollama_ask) + len(ollama_sync) + len(ollama_stream)}"
        )
