...
def check_generated_functions():
    ...
    try:
        ...
        for func_name in sorted(ollama_functions):
            if any(expected in func_name for expected in ['devstral', 'qwen3_32b', 'phi4_reasoning', 'llama3_2_vision']):
...
def inspect_function_generation():
    ...
    try:
        ...
        print(f"📋 Providers module attributes:")
        provider_attrs = [attr for attr in dir(providers_module) if not attr.startswith('_')]
        ...
        if hasattr(providers_module, '_generate_provider_functions'):
            print(f"   ✅ Dynamic generation function found")
        else:
            print(f"   ⚠️  Dynamic generation function not found")
        
        # Check for discovered models in __all__
        if hasattr(providers_module, '__all__'):
            all_exports = providers_module.__all__
            discovered_exports = [exp for exp in all_exports if any(disc in exp for disc in ['devstral', 'qwen3_32b', 'phi4'])]
...
async def main():
    ...
    if functions:
        ...
        if any('devstral' in f for f in functions):
            print(f"✅ Dynamic function generation is working!")
            print(f"💡 You can use: await chuk_llm.ask_ollama_devstral('Write code')")
        else:
            print(f"⚠️  Dynamic function generation may not be working")
            print(f"🔧 Functions might need manual refresh or different trigger")
    else:
        print(f"\n💥 No generated functions found")
        print(f"🔧 Dynamic function generation may need debugging")
    
    print(f"\n📖 Expected Usage:")
    print(f"   await chuk_llm.ask_ollama_devstral('Write Python code')")
    print(f"   await chuk_llm.ask_ollama_qwen3_32b('Solve math problem')")
    print(f"   await chuk_llm.ask_ollama_phi4_reasoning('Logical reasoning')")
