...
def list_all_functions():
    ...
    try:
        ...
        for func in sync_functions:
            ...
            is_provider_function = any(
                func.startswith(f"ask_{provider}_") for provider in providers
            )
        ...
        for provider in sorted(providers):
            ...
            provider_async = [
                f for f in async_functions if f.startswith(f"ask_{provider}_")
            ]
            provider_stream = [
                f for f in stream_functions if f.startswith(f"stream_{provider}_")
            ]

            total_provider_funcs = (
                len(provider_sync) + len(provider_async) + len(provider_stream)
            )

            print(f"\n📦 {provider.upper()} ({total_provider_funcs} functions)")
            print(
                f"    Sync: {len(provider_sync)}, Async: {len(provider_async)}, Stream: {len(provider_stream)}"
            )
