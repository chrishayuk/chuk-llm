...
def debug_function_generation():
    ...
    try:
        ...
        print(f"✅ OpenAI aliases: {list(openai_config.model_aliases.keys())[:5]}...")  # Show first 5
    ...
    try:
        ...
        test_names = ["gpt-4o-mini", "gpt4o", "claude-3-sonnet-20240229", "llama-3.3-70b-versatile"]
    ...
    try:
        from chuk_llm.api.providers import list_provider_functions, _all_functions
        
        functions = list_provider_functions()
        print(f"✅ Generated {len(functions)} total functions")
        
        # Look for OpenAI functions specifically
        openai_functions = [f for f in functions if f.startswith('ask_openai_') and f.endswith('_sync')]
    ...
    try:
        ...
        for model in models:
            ...
            print(f"   No model sanitizes to 'gpt4o_mini'")
