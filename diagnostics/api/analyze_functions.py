...
def analyze_function_generation():
    ...
    try:
        ...
        openai_functions = [
            ...
            if f.startswith("ask_openai_") or f.startswith("stream_openai_")
        ]
        ...
        print(
            f"  OpenAI generates functions for {len(openai_models)} different models/aliases:"
        )
        ...
        for provider_name in providers:
            try:
                ...
                print(
                    f"    Base: {base_funcs}, Models: {model_funcs} ({len(provider_config.models)} models), Aliases: {alias_funcs} ({len(provider_config.model_aliases)} aliases)"
                )
