...
def mock_anthropic():
    """Mock Anthropic SDK"""
    with patch('chuk_llm.llm.providers.anthropic_client.anthropic') as mock_anthropic_module:
        mock_async_client = AsyncMock()
        mock_sync_client = MagicMock()
...
def mock_mistral():
    ...
    with patch('chuk_llm.llm.providers.mistral_client.mistralai') as mock_mistral_module:
        mock_async_client = AsyncMock()
        mock_sync_client = MagicMock()
...
def mock_config():
    ...
    providers = [
        MockProviderConfig("openai", "OpenAILLMClient", "https://api.openai.com/v1",
                          models=["gpt-4", "gpt-3.5-turbo"]),
        MockProviderConfig("anthropic", "AnthropicLLMClient", "https://api.anthropic.com",
                          models=["claude-3-opus", "claude-3-sonnet"]),
        MockProviderConfig("groq", "GroqAILLMClient", "https://api.groq.com/openai/v1",
                          models=["llama-3.3-70b-versatile"]),
        MockProviderConfig("mistral", "MistralLLMClient", "https://api.mistral.ai",
                          models=["mistral-large"]),
        MockProviderConfig("gemini", "GeminiLLMClient", "https://generativelanguage.googleapis.com",
                          models=["gemini-pro"]),
        MockProviderConfig("ollama", "OllamaLLMClient", "http://localhost:11434",
                          models=["llama2"]),
        MockProviderConfig("watsonx", "WatsonxLLMClient", "https://us-south.ml.cloud.ibm.com",
...
def mock_provider_client_methods(client, provider_name="test_provider"):
    ...
    client.supports_feature = lambda feature: feature in [
        "streaming", "tools", "system_messages", "text"
    ]
...
def setup_test_environment():
    ...
    required_modules = [
        ...
        'google.generativeai', 'ollama', 'ibm_watsonx_ai'
    ]
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "provider: mark test as provider-specific"
    )
