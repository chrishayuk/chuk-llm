...
class TestOpenAICompatibleDiscoverer:
    ...
    def test_get_deepseek_specifics(self):
        ...
        discoverer = OpenAICompatibleDiscoverer("deepseek", "key", "https://api.deepseek.com")
    ...
    def test_get_perplexity_specifics(self):
        ...
        discoverer = OpenAICompatibleDiscoverer("perplexity", "key", "https://api.perplexity.ai")
    ...
    def test_get_provider_specifics_unknown_provider(self):
        ...
        discoverer = OpenAICompatibleDiscoverer("unknown", "key", "https://api.unknown.com")
...
class TestHuggingFaceModelDiscoverer:
    ...
    def setup_method(self):
        """Setup test discoverer"""
        self.discoverer = HuggingFaceModelDiscoverer(
            api_key="test-hf-key",
            limit=10
        )
    ...
    def test_is_suitable_for_inference(self):
        ...
        assert self.discoverer._is_suitable_for_inference(unsupported_lib_model) is False
    ...
    def test_determine_hf_family(self):
        ...
        assert self.discoverer._determine_hf_family("mistralai/mixtral-8x7b", []) == "mistral"
        assert self.discoverer._determine_hf_family("qwen/qwen2-7b", []) == "qwen"  # Check actual implementation
    ...
    def test_estimate_hf_size(self):
        ...
        assert self.discoverer._estimate_hf_size("model1.5b", []) == "5B"  # Actual behavior
    ...
    def test_determine_hf_specialization(self):
        ...
        assert self.discoverer._determine_hf_specialization("instruct-model", []) == "chat"
        assert self.discoverer._determine_hf_specialization("math-model", []) == "reasoning"
        assert self.discoverer._determine_hf_specialization("general-model", []) == "general"
...
class TestIntegration:
    ...
    async def test_openai_compatible_end_to_end(self):
        ...
        discoverer = OpenAICompatibleDiscoverer("groq", "test-key", "https://api.groq.com/openai/v1")
    ...
    async def test_huggingface_end_to_end(self):
        ...
        with patch('httpx.AsyncClient') as mock_client:
            ...
            assert characteristics["supports_tools"] is True  # Model name contains 'instruct'
            assert characteristics["popularity_score"] > 0
    
    def test_provider_specific_filtering(self):
        """Test that different providers apply their specific filters"""
        groq_discoverer = OpenAICompatibleDiscoverer("groq", "key", "https://api.groq.com")
        deepseek_discoverer = OpenAICompatibleDiscoverer("deepseek", "key", "https://api.deepseek.com")
