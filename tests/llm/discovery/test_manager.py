...
from chuk_llm.llm.discovery.manager import (
...
class TestUniversalDiscoveryManager:
    ...
    def test_manager_initialization_without_config(self):
        ...
        assert manager._discovery_cache == {}
    ...
    def test_manager_initialization_with_config(self):
        ...
        config_manager = MockConfigManager({
            "openai": Mock(extra={
                "dynamic_discovery": {"enabled": True}
            }),
            "ollama": Mock(extra={})  # No discovery config
        })
        
        with patch.object(UniversalDiscoveryManager, '_setup_provider_discovery') as mock_setup:
    ...
    def test_setup_provider_discovery_success(self):
        ...
        with patch('chuk_llm.llm.discovery.manager.DiscovererFactory') as mock_factory:
            ...
            assert isinstance(self.manager.provider_managers["test-provider"], UniversalModelDiscoveryManager)
    ...
    def test_build_discoverer_config_huggingface(self):
        ...
        with patch.dict('os.environ', {'HUGGINGFACE_API_KEY': 'hf-key'}):
            config = self.manager._build_discoverer_config("huggingface", discovery_config)
    ...
    async def test_discover_all_models_success(self):
        ...
        # Setup mock managers
        mock_manager1 = Mock()
        mock_manager1.discover_models = AsyncMock(return_value=[
            DiscoveredModel("model1", "provider1"),
            DiscoveredModel("model2", "provider1")
        ])
        
        mock_manager2 = Mock()
        mock_manager2.discover_models = AsyncMock(return_value=[
            DiscoveredModel("model3", "provider2")
        ])
    ...
    async def test_discover_all_models_with_failures(self):
        ...
        mock_manager2 = Mock()
        mock_manager2.discover_models = AsyncMock(side_effect=Exception("Provider2 failed"))
    ...
    async def test_discover_all_models_caching(self):
        ...
        mock_manager = Mock()
        mock_manager.discover_models = AsyncMock(return_value=[
            DiscoveredModel("model1", "provider1")
        ])
    ...
    async def test_discover_provider_models_not_found(self):
        ...
        with pytest.raises(ValueError, match="No discovery manager for provider: nonexistent"):
    ...
    def test_register_custom_provider(self):
        ...
        self.manager.register_custom_provider("custom-provider", mock_discoverer, inference_config)
    ...
    def test_generate_discovery_summary(self):
        ...
        models_by_provider = {
            "provider1": [
                DiscoveredModel("model1", "provider1", family="llama", capabilities={Feature.TEXT, Feature.REASONING}),
                DiscoveredModel("model2", "provider1", family="gpt4", capabilities={Feature.TEXT, Feature.VISION})
            ],
            "provider2": [
                DiscoveredModel("model3", "provider2", family="llama", capabilities={Feature.TEXT})
            ]
        }
        ...
        self.manager.provider_managers = {"provider1": Mock(), "provider2": Mock(), "provider3": Mock()}
        
        summary = self.manager._generate_discovery_summary(models_by_provider, errors, 2.5)
    ...
    def test_generate_discovery_summary_with_metadata(self):
        ...
        model_with_reasoning.metadata = {"reasoning_capable": True, "is_reasoning": True}
        ...
        models_by_provider = {"provider1": [model_with_reasoning, model_with_vision, model_with_code]}
        
        summary = self.manager._generate_discovery_summary(models_by_provider, {}, 1.0)
        
        special_counts = summary["special_model_counts"]
        assert special_counts["reasoning_models"] == 1  # reasoning_capable OR is_reasoning
    ...
    def test_get_model_recommendations_reasoning(self):
        ...
        model1.metadata = {"reasoning_capable": True, "supports_tools": True, "performance_tier": "high"}
    ...
    async def test_health_check_all_healthy(self):
        ...
        assert health["providers"]["provider1"]["response_time"] >= 0  # Allow 0 for very fast mock calls
    ...
    async def test_health_check_some_unhealthy(self):
        ...
        mock_manager2 = Mock()
        mock_manager2.discover_models = AsyncMock(side_effect=Exception("Connection failed"))
...
class TestIntegration:
    ...
    async def test_full_discovery_workflow(self):
        ...
        with patch('chuk_llm.llm.discovery.manager.DiscovererFactory') as mock_factory:
            mock_discoverer = MockDiscoverer("test-provider", mock_models=[
