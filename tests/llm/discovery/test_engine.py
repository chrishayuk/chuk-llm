...
class TestDiscoveredModelEngine:
    ...
    def test_discovered_model_with_none_values(self):
        ...
        model = DiscoveredModel(
            "test-model", "test-provider", capabilities=None, metadata=None
        )
    ...
    def test_to_dict_with_feature_objects(self):
        ...
        model = DiscoveredModel(
            "test-model", "test-provider", capabilities={Feature.TEXT, Feature.VISION}
        )
    ...
    def test_to_dict_with_string_capabilities(self):
        ...
        model = DiscoveredModel(
            "test-model", "test-provider", capabilities={mock_feature, plain_string}
        )
...
class TestConfigDrivenInferenceEngine:
    ...
    def test_family_inference_from_capabilities(self):
        ...
        assert Feature.REASONING in result.capabilities
        assert (
            result.family == "reasoning"
        )  # It does infer reasoning family from capabilities
    ...
    def test_complex_inference_chain(self):
        ...
        model = DiscoveredModel(
            "llama-3.1-70b-vision", "test-provider", size_bytes=50000000
        )
...
class TestUniversalModelDiscoveryManager:
    ...
    def setup_method(self):
        ...
        self.inference_config = {
            ...
            "default_max_output_tokens": 4096,
            "family_rules": {
                "llama": {"patterns": [r"llama"], "features": ["text", "tools"]}
            },
        }

        self.manager = UniversalModelDiscoveryManager(
            "test-provider", self.discoverer, self.inference_config
        )
...
class TestIntegration:
    ...
    async def test_end_to_end_discovery_and_inference(self):
        ...
        manager = UniversalModelDiscoveryManager(
            "test-provider", discoverer, inference_config
        )
    ...
    def test_manager_with_config_integration(self, mock_get_config):
        ...
        mock_provider_config.extra = {
            "dynamic_discovery": {
                "inference_config": {"default_features": ["text", "streaming"]}
            },
            "model_inference": {"family_rules": {"test": {"patterns": [r"test"]}}},
        }
