...
class TestDiscoveredModel:
    ...
    def test_from_dict_with_invalid_capabilities(self):
        ...
        with patch.object(Feature, 'from_string') as mock_from_string:
            mock_from_string.side_effect = lambda x: Feature.TEXT if x == "text" else Feature.VISION if x == "vision" else None
...
class TestBaseModelDiscoverer:
    ...
    async def test_discover_with_cache_uses_cache(self):
        ...
        discoverer = MockDiscoverer("test-provider", mock_models=mock_models, cache_timeout=60)
    ...
    async def test_discover_with_cache_expired(self):
        ...
        mock_models = [{"name": "model"}]
        discoverer = MockDiscoverer("test-provider", mock_models=mock_models, cache_timeout=0.1)
    ...
    async def test_discover_with_cache_handles_failure(self):
        ...
        discoverer._discovery_cache["test-provider_models"] = ([{"name": "cached"}], time.time())
    ...
    def test_get_discovery_info(self):
        ...
        discoverer = MockDiscoverer("test-provider", cache_timeout=300, custom_param="value")
        discoverer._last_discovery = 1234567890.0
        discoverer._discovery_cache["test-provider_models"] = ([{"name": "model1"}, {"name": "model2"}], time.time())
...
class TestDiscovererFactory:
    ...
    def test_create_discoverer_not_found(self):
        ...
        with pytest.raises(ValueError, match="No discoverer available for provider: nonexistent"):
    ...
    def test_auto_import_discoverers_success(self, mock_log):
        ...
        with patch('chuk_llm.llm.discovery.base.DiscovererFactory._discoverers', {"ollama": MockDiscoverer}):
            DiscovererFactory._imported = False
    ...
    def test_auto_import_discoverers_failure(self, mock_log):
        ...
        assert DiscovererFactory._imported is True  # Should still set to True to avoid retrying
    ...
    def test_create_discoverer_triggers_auto_import(self):
        ...
        with patch.object(DiscovererFactory, '_auto_import_discoverers') as mock_auto_import:
            with pytest.raises(ValueError):  # Will fail because no discoverer registered
...
class TestIntegration:
    ...
    async def test_full_discovery_workflow(self):
        ...
        # Create discoverer
        discoverer = DiscovererFactory.create_discoverer("test", mock_models=mock_models)
