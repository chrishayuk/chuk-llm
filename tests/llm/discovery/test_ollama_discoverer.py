...
class TestOllamaModelDiscoverer:
    ...
    async def test_discover_models_success(self):
        ...
        assert abs(llama_model["size_gb"] - 65.2) < 0.1  # Allow for floating point precision
    ...
    async def test_discover_models_connection_error(self):
        ...
        with patch('httpx.AsyncClient') as mock_client:
            mock_client.return_value.__aenter__ = AsyncMock(return_value=mock_client.return_value)
            mock_client.return_value.__aexit__ = AsyncMock(return_value=None)
            mock_client.return_value.get = AsyncMock(side_effect=httpx.ConnectError("Connection failed"))
    ...
    async def test_get_model_metadata_failure(self):
        ...
        with patch('httpx.AsyncClient') as mock_client:
            mock_client.return_value.__aenter__ = AsyncMock(return_value=mock_client.return_value)
            mock_client.return_value.__aexit__ = AsyncMock(return_value=None)
            mock_client.return_value.post = AsyncMock(side_effect=httpx.HTTPStatusError(
    ...
    def test_determine_capabilities_basic(self):
        ...
        capabilities = self.discoverer._determine_capabilities("basic-model:7b", 7000000000)
    ...
    def test_determine_capabilities_instruct_model(self):
        ...
        capabilities = self.discoverer._determine_capabilities("llama3-instruct:8b", 8000000000)
    ...
    def test_determine_capabilities_large_model(self):
        ...
        # Very large model
        huge_capabilities = self.discoverer._determine_capabilities("model:405b", 400000000000)
    ...
    def test_determine_context_length_llama_variants(self):
        ...
        # Llama 2
        context1 = self.discoverer._determine_context_length("llama2:7b", {"family": "llama", "base_context": 8192, "context_rules": {r'llama-?2': 4096}})
        assert context1 == 4096
        
        # Llama 3.2 (matches llama.*3\.[23] pattern)
        context2 = self.discoverer._determine_context_length("llama3.2:70b", {"family": "llama", "base_context": 8192, "context_rules": {r'llama.*3\.[23]': 128000}})
        assert context2 == 128000
        
        # Phi 4
        context3 = self.discoverer._determine_context_length("phi4:14b", {"family": "phi", "base_context": 4096, "context_rules": {r'phi4': 128000}})
    ...
    def test_estimate_parameters_from_name(self):
        ...
        assert self.discoverer._estimate_parameters("mixtral:8x7b", 0) == "7B"  # Takes the last number from x7b pattern
    
    def test_estimate_parameters_from_size(self):
        """Test parameter estimation from model size"""
        assert self.discoverer._estimate_parameters("model", int(0.5 * 1024**3)) == "< 1B"
        ...
        assert self.discoverer._estimate_parameters("model", int(10 * 1024**3)) == "7-13B"
        assert self.discoverer._estimate_parameters("model", int(20 * 1024**3)) == "13-30B"
        assert self.discoverer._estimate_parameters("model", int(50 * 1024**3)) == "30-70B"
        assert self.discoverer._estimate_parameters("model", int(100 * 1024**3)) == "70B+"
    ...
    def test_estimate_performance_tier(self):
        ...
        assert self.discoverer._estimate_performance_tier(int(20 * 1024**3)) == "extra-large"
        assert self.discoverer._estimate_performance_tier(int(100 * 1024**3)) == "massive"
    
    def test_model_sort_key_reasoning_priority(self):
        """Test model sorting prioritizes reasoning models"""
        reasoning_model = {"reasoning_capable": True, "size_gb": 7, "capabilities": ["reasoning"], "model_family": "llama"}
        basic_model = {"reasoning_capable": False, "size_gb": 70, "capabilities": [], "model_family": "unknown"}
    ...
    def test_model_sort_key_size_scoring(self):
        ...
        small_model = {"reasoning_capable": False, "size_gb": 7, "capabilities": [], "model_family": "llama"}
        huge_model = {"reasoning_capable": False, "size_gb": 200, "capabilities": [], "model_family": "llama"}
    ...
    def test_model_sort_key_capability_bonus(self):
        ...
        model_with_reasoning = {"reasoning_capable": False, "size_gb": 7, "capabilities": ["reasoning"], "model_family": "unknown"}
        model_with_tools = {"reasoning_capable": False, "size_gb": 7, "capabilities": ["tools"], "model_family": "unknown"}
        basic_model = {"reasoning_capable": False, "size_gb": 7, "capabilities": [], "model_family": "unknown"}
    ...
    async def test_enhance_model_data(self):
        ...
        with patch.object(self.discoverer, 'get_model_metadata', return_value={"detailed": "info"}):
            enhanced = await self.discoverer._enhance_model_data(model_data)
        
        # Check all expected fields
        assert enhanced["name"] == "llama3.1:70b-instruct"
        assert abs(enhanced["size_gb"] - 65.2) < 0.1  # Allow for floating point precision
        ...
        assert enhanced["estimated_context_length"] == 128000  # llama3.1 gets 128k context
    ...
    async def test_delete_model_failure(self):
        ...
        with patch('httpx.AsyncClient') as mock_client:
            mock_client.return_value.__aenter__ = AsyncMock(return_value=mock_client.return_value)
            mock_client.return_value.__aexit__ = AsyncMock(return_value=None)
            mock_client.return_value.delete = AsyncMock(side_effect=Exception("Delete failed"))
...
class TestOllamaIntegration:
    ...
    def test_comprehensive_model_analysis(self):
        ...
        test_cases = [
            {
                ...
                "expected_capabilities": ["text", "streaming", "tools", "system_messages", "reasoning"]
            },
            {
                ...
                "expected_capabilities": ["text", "streaming", "tools", "reasoning", "system_messages"]
            },
            {
                "name": "llava:13b-v1.6",
                ...
                "expected_capabilities": ["text", "streaming", "vision", "multimodal", "system_messages"]
            },
            {
                "name": "phi4:14b",
                "expected_family": "phi",
                "expected_reasoning": True,
                "expected_context": 128000,
                "expected_capabilities": ["text", "streaming", "system_messages", "reasoning"]
            }
        ]
        ...
        for case in test_cases:
            ...
            context_length = discoverer._determine_context_length(case["name"], family_info)
            
            assert family_info["family"] == case["expected_family"], f"Failed for {case['name']}"
            assert family_info["reasoning_capable"] == case["expected_reasoning"], f"Failed for {case['name']}"
            assert context_length == case["expected_context"], f"Failed for {case['name']}"
            
            for expected_cap in case["expected_capabilities"]:
                assert expected_cap in capabilities, f"Missing {expected_cap} for {case['name']}"