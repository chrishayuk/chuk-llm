...
class TestOpenAIModelDiscoverer:
    ...
    def test_categorize_model_gpt35(self):
        ...
        model_data = {"id": "gpt-3.5-turbo", "created": 1640995200, "owned_by": "openai"}
    ...
    def test_estimate_performance_tier(self):
        ...
        assert self.discoverer._estimate_performance_tier("o3-mini") == "advanced-reasoning"
    ...
    def test_model_sort_key(self):
        ...
        o1_model = {"is_reasoning": True, "performance_tier": "reasoning", "generation": "o1"}
        gpt4_model = {"is_reasoning": False, "performance_tier": "high", "generation": "gpt4"}
        gpt35_model = {"is_reasoning": False, "performance_tier": "medium", "generation": "gpt35"}
    ...
    def test_normalize_model_data(self):
        ...
        assert result.metadata["parameter_requirements"]["use_max_completion_tokens"] is True
        ...
        with patch.dict('os.environ', {}, clear=True):  # Clear environment
            ...
            async def mock_test_availability(model_name: str) -> bool:
                ...
                if not discoverer.api_key:
                    return False
                if isinstance(discoverer.api_key, str) and not discoverer.api_key.strip():
                    return False
        ...
        with patch.dict('os.environ', {}, clear=True):
            ...
            async def mock_test_availability1(model_name: str) -> bool:
                if not discoverer1.api_key:
                    return False
                if isinstance(discoverer1.api_key, str) and not discoverer1.api_key.strip():
                    return False
                return True
        ...
        with patch.dict('os.environ', {}, clear=True):
            ...
            async def mock_test_availability2(model_name: str) -> bool:
                if not discoverer2.api_key:
                    return False
                if isinstance(discoverer2.api_key, str) and not discoverer2.api_key.strip():
                    return False
                return True
        ...
        with patch.dict('os.environ', {}, clear=True):
            ...
            async def mock_test_availability3(model_name: str) -> bool:
                if not discoverer3.api_key:
                    return False
                if isinstance(discoverer3.api_key, str) and not discoverer3.api_key.strip():
                    return False
                return True
        ...
        with patch.dict('os.environ', {}, clear=True):
            ...
            async def mock_test_availability4(model_name: str) -> bool:
                if not discoverer4.api_key:
                    return False
                if isinstance(discoverer4.api_key, str) and not discoverer4.api_key.strip():
                    return False
                return True
...
class TestOpenAIIntegration:
    ...
    def test_comprehensive_model_analysis(self):
        ...
        for case in test_cases:
            model_data = {"id": case["name"], "created": 1640995200, "owned_by": "openai"}
            result = discoverer._categorize_model(case["name"], model_data)
            
            assert result["is_reasoning"] == case["expected_reasoning"], f"Reasoning failed for {case['name']}"
            assert result["is_vision"] == case["expected_vision"], f"Vision failed for {case['name']}"
            assert result["model_family"] == case["expected_family"], f"Family failed for {case['name']}"
            assert result["supports_streaming"] == case["expected_streaming"], f"Streaming failed for {case['name']}"
            assert result["supports_system_messages"] == case["expected_system_messages"], f"System messages failed for {case['name']}"
            assert result["performance_tier"] == case["expected_tier"], f"Performance tier failed for {case['name']}"
