...
if 'chuk_llm.configuration' not in sys.modules:
    sys.modules['chuk_llm.configuration'] = create_mock_module(
...
if 'chuk_llm.llm.core.base' not in sys.modules:
    sys.modules['chuk_llm.llm.core.base'] = create_mock_module(
        'chuk_llm.llm.core.base',
        BaseLLMClient=type('BaseLLMClient', (), {})
    )
...
if 'chuk_llm.llm.providers._config_mixin' not in sys.modules:
    sys.modules['chuk_llm.llm.providers._config_mixin'] = create_mock_module(
        ...
        ConfigAwareProviderMixin=type('ConfigAwareProviderMixin', (), {
...
if 'chuk_llm.llm.providers._tool_compatibility' not in sys.modules:
    sys.modules['chuk_llm.llm.providers._tool_compatibility'] = create_mock_module(
        ...
        ToolCompatibilityMixin=type('ToolCompatibilityMixin', (), {
            '__init__': lambda self, *args, **kwargs: None
        })
    )
...
def test_parse_watsonx_tool_formats_partial_format():
    ...
    text = '<tool_call>[{"arguments": {"table_name": "products"}, "name": "describe_table"'
...
def test_parse_watsonx_response_with_list_content():
    ...
    mock_response = types.SimpleNamespace(
        choices=[
            types.SimpleNamespace(
                message=types.SimpleNamespace(
                    content=[{"text": "Hello from Watson X"}],
                    tool_calls=None
                )
            )
        ]
...
def test_client_initialization_granite_tokenizer_unavailable(mock_configuration, mock_env, monkeypatch):
    """Test client initialization when Granite tokenizer is unavailable."""
    # Mock GRANITE_TOKENIZER_AVAILABLE to False
    monkeypatch.setattr('chuk_llm.llm.providers.watsonx_client.GRANITE_TOKENIZER_AVAILABLE', False)
    
    client = WatsonXLLMClient(model="ibm/granite-3-3-8b-instruct")
    
    assert client.granite_tokenizer is None

def test_client_initialization_granite_tokenizer_error(mock_configuration, mock_env, monkeypatch):
    ...
    monkeypatch.setattr('chuk_llm.llm.providers.watsonx_client.AutoTokenizer', mock_tokenizer)
    monkeypatch.setattr('chuk_llm.llm.providers.watsonx_client.GRANITE_TOKENIZER_AVAILABLE', True)
...
def test_get_model_info_with_error(client, monkeypatch):
    ...
    monkeypatch.setattr('chuk_llm.llm.providers._config_mixin.ConfigAwareProviderMixin.get_model_info', mock_get_model_info)
...
def test_detect_model_family_comprehensive(client):
    ...
    test_cases = [
        ...
        ("codellama/CodeLlama-7b-Python-hf", "llama"),  # CodeLlama contains "llama" so returns "llama"
    ...
    for model_name, expected_family in test_cases:
        ...
        assert actual_family == expected_family, f"Expected {expected_family} for {model_name}, got {actual_family}"
...
async def test_stream_completion_with_granite_template(client):
    ...
    client.granite_tokenizer.apply_chat_template.return_value = "Formatted chat template"
...
def test_format_messages_edge_cases(client):
    ...
    messages = [
        ...
        {"role": "user", "content": {"type": "text", "text": "Dict content"}},  # Dict content
...
def test_should_use_granite_chat_template_comprehensive(client):
    ...
    # No tokenizer
    client.granite_tokenizer = None
    assert client._should_use_granite_chat_template([], [{"function": {"name": "test"}}]) is False
    ...
    assert client._should_use_granite_chat_template(messages_with_tool_calls, [{"function": {"name": "test"}}]) is False
    ...
    assert client._should_use_granite_chat_template(messages_with_complex_content, [{"function": {"name": "test"}}]) is False
    ...
    assert client._should_use_granite_chat_template(messages_with_none_content, [{"function": {"name": "test"}}]) is False
...
def test_format_granite_chat_template_jinja2_import_error(client):
    ...
    mock_tokenizer = MagicMock()
    mock_tokenizer.apply_chat_template.side_effect = ImportError("No module named 'jinja2'")
    client.granite_tokenizer = mock_tokenizer
...
async def test_regular_completion_generate_text_with_tool_parsing(client):
    ...
    mock_model = MagicMock()
    mock_model.generate_text.return_value = '{"function": "get_weather", "arguments": {"location": "NYC"}}'
    client._get_model_inference = lambda params: mock_model
...
def test_validate_request_unsupported_features(client, monkeypatch):
    ...
    validated_messages, validated_tools, validated_stream, validated_kwargs = client._validate_request_with_config(
        messages, None, True  # Request streaming
    )
...
def test_validate_request_multimodal_info_logging(client, monkeypatch, caplog):
    ...
    monkeypatch.setattr(client, "supports_feature", lambda feature: feature != "multimodal")
...
async def test_regular_completion_response_object_fallback(client):
    ...
    mock_response = types.SimpleNamespace(
        results=[
            types.SimpleNamespace(
                generated_text="Fallback response text"
            )
        ]
    )
    mock_model.chat.return_value = mock_response
...
def test_convert_tools_malformed_comprehensive(client):
    ...
    try:
        ...
        # at least verify it fails in a predictable way
        assert "function" in str(e) or "name" in str(e) or "get" in str(e)
...
def test_parse_watsonx_tool_formats_very_long_input():
    ...
    long_text = "Some text " * 1000 + '{"function": "test_tool", "arguments": {"param": "value"}}'
...
async def test_stream_completion_async_mixed_chunk_types(client):
    ...
    assert "string_chunk" in responses
    assert "dict_chunk" in responses
    assert "object_chunk" in responses
...
def test_format_messages_for_watsonx_template_mode(client):
    ...
    client._format_granite_chat_template = lambda messages, tools: "Template string result"
...
async def test_create_completion_parameter_validation_integration(client):
    ...
    original_validate = client.validate_parameters
    ...
    assert completion_params.get("validated") is True
...
async def test_complete_tool_workflow_with_sanitization(client):
    ...
    client._restore_tool_names_in_response = lambda response, mapping=None: response  # Simple pass-through
...
async def test_streaming_with_granite_template_integration(client):
    ...
    client.granite_tokenizer = MagicMock()
    client.granite_tokenizer.apply_chat_template.return_value = "# Granite Template\nUser: Hello\nAssistant:"
