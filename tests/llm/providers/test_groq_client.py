...
class GroqAILLMClient(OpenAILLMClient):
    ...
    def _get_optimization_profile(self) -> str:
        ...
        # Reasoning models (check before size-based)
        elif any(pattern in model_lower for pattern in ["reasoning", "r1", "distill"]):
            # But if it's also a large model, prefer throughput
            if any(pattern in model_lower for pattern in ["70b", "120b"]):
                return "high_throughput"
            return "deep_reasoning"

        # Large models
        elif any(pattern in model_lower for pattern in ["70b", "120b", "large"]):
            return "high_throughput"
