...
class MockProviderConfig:
    def __init__(self, name="openai", client_class="OpenAILLMClient"):
        ...
        self.models = ["gpt-4", "gpt-3.5-turbo", "gpt-4o", "gpt-4o-mini"]
        self.model_aliases = {}
...
class TestOpenAIErrorRecovery:
    ...
    async def test_recovery_from_network_error(self, client):
        ...
        result = await client.create_completion(messages, stream=False)
        assert (
            "Recovered" in result["response"] or "timeout" in result["response"].lower()
        )
    ...
    async def test_streaming_error_recovery(self, client):
        ...
        assert len(chunks) >= 1
        assert any(
            "error" in str(chunk) or "Starting" in chunk.get("response", "")
            for chunk in chunks
        )
...
class TestOpenAIParameterValidationEdgeCases:
    ...
    async def test_parameter_boundary_values(self, client):
        ...
        for params in test_cases:
            ...
            def create_mock_create(captured):
                async def mock_create(**kwargs):
...
class TestOpenAIResponseFormats:
    ...
    @pytest.mark.asyncio
    async def test_json_response_format(self, client, monkeypatch):
        """Test JSON response format."""
        messages = [{"role": "user", "content": "Return JSON"}]

        # Mock the client to support JSON mode
        monkeypatch.setattr(
