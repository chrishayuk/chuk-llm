...
async def test_streaming_interface_compliance(provider_class, provider_name):
    ...
    elif provider_name == "ollama" and "ollama" not in sys.modules:
        # Mock ollama module
    ...
    try:
        ...
        if provider_name == "gemini":
            client = client_class(api_key="fake-key")
        elif provider_name == "ollama":
            client = client_class(model="test-model")
        else:
            client = client_class(api_key="fake-key")
        ...
        assert hasattr(stream_result, "__aiter__"), (
            f"{provider_name} streaming should return async generator"
        )
        assert not hasattr(stream_result, "__await__"), (
            f"{provider_name} streaming should not be awaitable"
        )
        ...
        if asyncio.iscoroutine(non_stream_result):
            assert hasattr(non_stream_result, "__await__"), (
                f"{provider_name} non-streaming should be awaitable"
            )
            assert not hasattr(non_stream_result, "__aiter__"), (
                f"{provider_name} non-streaming should not be async generator"
            )
            # Await the result to prevent coroutine warnings
            result = await non_stream_result
            assert isinstance(result, dict), (
                f"{provider_name} should return dict after awaiting"
            )
        else:
            assert isinstance(non_stream_result, dict), (
                f"{provider_name} should return dict directly"
            )
...
def test_base_interface():
    ...
    create_completion_method = BaseLLMClient.create_completion
