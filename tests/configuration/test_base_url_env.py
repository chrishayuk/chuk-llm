...
from chuk_llm.configuration.unified_config import UnifiedConfigManager, get_config
...
class TestBaseURLEnvironment:
    ...
    def test_custom_env_variable(self):
        ...
        provider = config.register_provider(
            name="test_custom",
    ...
    def test_multiple_providers_isolation(self):
        ...
        p1 = config.register_provider(
            name="provider1",
            api_base="https://p1.com/v1",
            models=["model1"]
        )
        
        p2 = config.register_provider(
            name="provider2",
            api_base="https://p2.com/v1",
            models=["model2"]
        )
...
class TestBaseURLIntegration:
    ...
    def test_client_uses_env_base_url(self):
        ...
        # Save original
        self.original_env["TEST_CLIENT_API_BASE"] = os.environ.get("TEST_CLIENT_API_BASE")
    ...
    def test_gateway_providers_env_support(self):
        ...
        for provider_name, env_var, test_url in gateways:
            ...
            try:
                ...
                assert resolved == test_url, f"{provider_name} failed: expected {test_url}, got {resolved}"
            finally:
                # Clean up
