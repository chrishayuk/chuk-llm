...
from chuk_llm.configuration.models import Feature, ModelCapabilities, ProviderConfig, DiscoveryConfig
...
class TestFeature:
    ...
    def test_feature_enum_values(self):
        ...
        expected_features = {
            ...
            "parallel_calls", "system_messages", "multimodal", "reasoning"
        }
...
class TestConfigValidator:
    ...
    def test_is_valid_url(self):
        ...
        for url in valid_urls:
            assert ConfigValidator._is_valid_url(url) is True, f"Expected {url} to be valid"
        
        for url in invalid_urls:
            assert ConfigValidator._is_valid_url(url) is False, f"Expected {url} to be invalid"
...
class TestUnifiedConfigManager:
    ...
    def test_process_config_with_inheritance(self):
        ...
        with patch.object(UnifiedConfigManager, '_load_environment'):
            ...
            base_provider = manager.providers["base"]
...
class TestCapabilityChecker:
    ...
    def test_get_best_provider_for_features(self):
        ...
        with patch('chuk_llm.configuration.unified_config.get_config') as mock_get_config:
            ...
            provider2 = Mock()
            provider2.get_model_capabilities.return_value.features = {Feature.TEXT, Feature.TOOLS}
            ...
            assert best == "provider2"  # Has higher rate limit and supports required features
    ...
    def test_inheritance_circular_reference(self):
        ...
        with patch.object(UnifiedConfigManager, '_load_environment'):
            ...
            config = {
                "provider_a": {
                    "inherits": "provider_b",
                    "client_class": "ClientA"
                },
                "provider_b": {
                    "inherits": "provider_a",
                    "client_class": "ClientB"
                }
            }
...
class TestPackageResourceHandlingSimplified:
    ...
    def test_get_package_config_path_no_resources(self):
        ...
        with patch.object(UnifiedConfigManager, '_load_environment'):
            with patch('chuk_llm.configuration.unified_config._importlib_resources_available', False):
                with patch('chuk_llm.configuration.unified_config._pkg_resources_available', False):
                    manager = UnifiedConfigManager()
...
class TestYAMLFileHandlingSimplified:
    ...
    def test_find_config_files_no_files(self):
        ...
        with patch.object(UnifiedConfigManager, '_load_environment'):
            ...
            # Mock no files exist
            with patch('pathlib.Path.exists', return_value=False):
                with patch.object(manager, '_get_package_config_path', return_value=None):
    ...
    def test_load_single_yaml_package_resource(self):
        ...
        with patch.object(UnifiedConfigManager, '_load_environment'):
            ...
            with patch.object(manager, '_is_package_resource_path', return_value=True):
                with patch.object(manager, '_load_package_yaml', return_value={'package': 'config'}):
    ...
    def test_load_single_yaml_regular_file(self):
        ...
        with patch.object(UnifiedConfigManager, '_load_environment'):
            ...
            with patch('builtins.open', return_value=mock_file):
                with patch.object(manager, '_is_package_resource_path', return_value=False):
                    with patch('chuk_llm.configuration.unified_config.yaml.safe_load', return_value=yaml_content):
