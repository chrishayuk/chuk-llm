...
class MockLLMClient(BaseLLMClient):
    ...
    def __init__(self, mock_response=None, mock_stream=None, should_raise=None):
        self.mock_response = mock_response or {"response": "Test response", "tool_calls": []}
...
class TestBaseLLMClient:
    ...
    async def test_concurrent_streaming_calls(self):
        ...
        task1 = asyncio.create_task(self._collect_stream(
            client.create_completion(messages, stream=True)
        ))
        task2 = asyncio.create_task(self._collect_stream(
            client.create_completion(messages, stream=True)
        ))
        task3 = asyncio.create_task(self._collect_stream(
            client.create_completion(messages, stream=True)
        ))
...
async def test_tools_parameter_variations(tools, simple_messages):
    ...
    result = await client.create_completion(simple_messages, tools=tools, stream=False)
