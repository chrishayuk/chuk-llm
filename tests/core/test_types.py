...
from pydantic import BaseModel, Field, field_validator, ConfigDict, ValidationError, BeforeValidator
...
class TestLLMResponse:
    ...
    def test_empty_response_validation_error(self):
        ...
        with pytest.raises(ValidationError, match="Response must have either text content or tool calls"):
            LLMResponse(
                response=None,
                tool_calls=[],
                error=False
...
class TestResponseValidator:
    ...
    def test_validate_valid_response(self):
        ...
        validated = ResponseValidator.validate_response(raw_response, is_streaming=False)
...
# Parametrized tests
@pytest.mark.parametrize("arguments,expected_json", [
    ({}, "{}"),
    ({"key": "value"}, '{"key": "value"}'),
...
@pytest.mark.parametrize("response,tool_calls,error,should_validate", [
    ("Hello", [], False, True),           # Text response
    (None, [{"id": "call_1", "function": {"name": "test", "arguments": "{}"}}], False, True),  # Tool calls
    ("Text", [{"id": "call_1", "function": {"name": "test", "arguments": "{}"}}], False, True),  # Both
    (None, [], True, True),               # Error response
    (None, [], False, False),             # Empty response (should fail)
])
def test_llm_response_validation_scenarios(response, tool_calls, error, should_validate):
