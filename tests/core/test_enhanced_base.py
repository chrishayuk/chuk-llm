...
class MockEnhancedBaseLLMClient:
    ...
    def __init__(self, middleware: Optional[List[MockMiddleware]] = None):
    ...
    async def _create_enhanced_completion_wrapper(self, messages, tools, **kwargs):
        ...
        try:
            ...
            if "_cached_response" in processed_kwargs:
                ...
                return await self.middleware_stack.process_response(cached, duration, False)
    ...
    async def _create_enhanced_stream_wrapper(self, messages, tools, **kwargs):
        ...
        try:
            ...
            if "_cached_response" in processed_kwargs:
                cached = processed_kwargs.pop("_cached_response")
                duration = time.time() - start_time
                processed_response = await self.middleware_stack.process_response(cached, duration, True)
    ...
    async def _create_enhanced_completion(
        ...
        **kwargs
        ...
        duration = time.time() - start_time
        return await self.middleware_stack.process_response(response, duration, is_streaming=False)
...
class TestEnhancedBaseLLMClient:
    ...
    async def test_non_streaming_with_middleware(self):
        ...
        resp_response, resp_duration, resp_is_streaming = middleware.process_response_calls[0]
    ...
    async def test_streaming_with_middleware(self):
        ...
        assert len(middleware.process_request_calls) == 1
        assert len(middleware.process_stream_chunk_calls) == 2  # Two chunks in mock_stream
        assert len(middleware.process_response_calls) == 0  # No response processing for streaming
        
        # Check chunks were processed
        for i, (chunk, chunk_index, duration) in enumerate(middleware.process_stream_chunk_calls):
...
async def test_variable_middleware_count(middleware_count, simple_messages):
    ...
    middleware_list = [MockMiddleware(f"middleware_{i}") for i in range(middleware_count)]
