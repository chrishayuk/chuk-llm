...
async def test_streaming_function_call(client, provider_name, result):
    ...
    try:
        ...
        # Special handling for OpenAI and Groq that have a different tool_calls structure in streaming
        special_providers = ["openai", "groq"]
        
        async for chunk in stream:
            chunks.append(chunk)
            
            # Handle special providers differently
            if provider_name.lower() in special_providers:
                # For OpenAI/Groq, directly check the chunk structure
                if hasattr(chunk, "tool_calls") and chunk.tool_calls:
                    tool_calls_found = True
                    print(f"\nTool call found in stream")
            else:
                # For other providers, use the standard dict approach
...
async def test_image_recognition(client, provider_name, result, model: Optional[str] = None):
    ...
    try:
        ...
        if provider_name.lower() == "openai":
            try:
                # Properly encode the sample image
                image_data = base64.b64decode(SAMPLE_IMAGE_BASE64.strip())
                # Create a simple test image instead of using the sample
                try:
            ...
            try:
                ...
                try:
                    ...
                    # Fall back to basic image
                    image_data = base64.b64decode(SAMPLE_IMAGE_BASE64.strip())
                
                # Create parts without arguments
                parts = []
            ...
            try:
                ...
                messages = [{
                    ...
                    "content": [
                        {"type": "text", "text": IMAGE_PROMPT},
                        {"type": "image_url", "image_url": {"url": f"data:image/png;base64,{SAMPLE_IMAGE_BASE64.strip()}"}}
                    ]
                }]
            except Exception as e:
...
async def main():
    ...
    # Use llama-3-70b-instruct for Ollama image testing if not specified
    if "ollama" in providers and "ollama" not in model_overrides and args.model != "qwen3":
        model_overrides["ollama"] = "llama3"
        print("Using llama3 for Ollama tests (supports image recognition)")
