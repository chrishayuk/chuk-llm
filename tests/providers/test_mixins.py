        ...
        return {
            ...
            'with_tools': MockMessage(
                ...
                tool_calls=[MockToolCall(
                    id="call_123",
                    function=MockFunction(name="test_func", arguments='{"param": "value"}')
                )]
            ),
            'tool_only': MockMessage(
                content="",
                tool_calls=[MockToolCall(
                    function=MockFunction(name="solo_func", arguments='{"key": "val"}')
                )]
            ),
    ...
    def _create_chunks(content_list=None, include_tools=False):
        ...
        for i, content in enumerate(content_list):
            if include_tools and i == 1:  # Add tool call to second chunk
                ...
                chunks.append(MockChunk([MockChoice(delta=MockDelta(content, tool_calls=[tool_call]))]))
            else:
...
def test_debug_message_structure_basic(mixin, mock_logger):
    ...
    assert any("DEBUG MESSAGE STRUCTURE (test_context)" in str(call) for call in debug_calls)
...
async def test_stream_from_async_custom_normalize_error(mixin, mock_logger):
    ...
    assert len(results) == 1
    assert results[0]["response"] == "Test"  # Should continue despite normalization error
...
async def test_stream_from_async_mixed_chunk_formats(mixin):
    ...
    mixed_chunks = [
        ...
        MockChunk([MockChoice(message=MockMessage(content="Message "))]),  # Message format
