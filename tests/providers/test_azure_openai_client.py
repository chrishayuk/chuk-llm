...
class MockModelCapabilities:
    def __init__(self, features=None, max_context_length=128000, max_output_tokens=4096):
        self.features = features or {
            ...
            MockFeature.JSON_MODE, MockFeature.PARALLEL_CALLS
        }
...
def client(mock_configuration, mock_env, monkeypatch):
    ...
    monkeypatch.setattr(cl, "supports_feature", lambda feature: feature in [
        "text", "streaming", "tools", "vision", "system_messages", "multimodal", "json_mode", "parallel_calls"
    ])
    ...
    monkeypatch.setattr(cl, "get_model_info", lambda: {
        ...
        "features": ["text", "streaming", "tools", "vision", "system_messages", "multimodal", "json_mode", "parallel_calls"],
        "supports_text": True,
...
class TestAzureOpenAIBasic:
    ...
    def test_client_initialization_custom_deployment(self, mock_configuration, mock_env):
...
class TestAzureOpenAIStreaming:
    ...
    async def test_streaming_with_name_mapping(self, client):
                ...
                if chunk.choices[0].delta.tool_calls:
                    yield {
                        ...
                        "tool_calls": [{
                            "id": "call_123",
                            "type": "function",
                            "function": {"name": "test_tool_sanitized", "arguments": '{"arg": "value"}'}
                        }]
                    }
        ...
        chunks = []
        async for chunk in client._stream_completion_async(messages, tools, name_mapping):
            chunks.append(chunk)
...
class TestAzureOpenAIMessageNormalization:
    ...
    def test_normalize_message_azure_argument_formats(self, client):
        ...
        type(mock_tool_call.function).arguments = PropertyMock(return_value='{"city": "NYC"}')
        ...
        try:
            ...
            assert False, f"Arguments are not valid JSON: {result['tool_calls'][0]['function']['arguments']}"
...
class TestAzureOpenAIInterface:
    ...
    async def test_create_completion_non_streaming(self, client):
        ...
        async def mock_regular_completion(messages, tools=None, name_mapping=None, **kwargs):
    ...
    async def test_create_completion_streaming(self, client):
        ...
        async def mock_stream_completion(messages, tools=None, name_mapping=None, **kwargs):
    ...
    @pytest.mark.asyncio
    async def test_create_completion_with_tools_not_supported(self, client, monkeypatch):
...
class TestAzureOpenAIFeatureSupport:
    ...
    def test_feature_support_validation(self, client, monkeypatch):
        ...
        supported_features = ["text", "streaming", "tools", "vision", "system_messages", "multimodal", "json_mode", "parallel_calls"]
...
class TestAzureOpenAIComplexScenarios:
    ...
    async def test_unsupported_features_graceful_handling(self, client, monkeypatch):
        ...
        result = await client.create_completion(
            ...
            response_format={"type": "json_object"}  # Should be removed when not supported
        )
