...
class TestAskFunction:
            ...
            with patch('chuk_llm.api.core.get_client', return_value=mock_client):
                with patch('chuk_llm.api.core.get_config') as mock_get_config:
                    ...
                    with patch('chuk_llm.api.core.ConfigValidator') as mock_validator:
                        ...
                        mock_client.create_completion.return_value = {"response": "Anthropic response"}
                        ...
                        mock_get_client = patch('chuk_llm.api.core.get_client').start()
    ...
    async def test_ask_with_all_parameters(self, setup_mocks):
        ...
        mock_client.create_completion.return_value = {"response": "Full parameter response"}
    ...
    async def test_ask_with_session_tracking(self, setup_mocks, mock_session_manager):
        ...
        mock_client.create_completion.return_value = {"response": "Session tracked response"}
        
        with patch('chuk_llm.api.core._get_session_manager', return_value=mock_session_manager):
            ...
            mock_session_manager.ai_responds.assert_called_once_with(
                "Session tracked response",
                model="gpt-4",
                provider="openai"
            )

    @pytest.mark.asyncio
    async def test_ask_with_tools_and_tool_calls(self, setup_mocks, mock_session_manager):
    ...
    async def test_ask_with_json_mode_openai(self, setup_mocks):
        ...
        mock_client.create_completion.return_value = {"response": '{"result": "json response"}'}
    ...
    async def test_ask_with_json_mode_gemini(self, setup_mocks):
        ...
        mock_client.create_completion.return_value = {"response": '{"result": "gemini json"}'}
    ...
    async def test_ask_with_json_mode_unsupported_provider(self, setup_mocks):
        ...
        mock_client.create_completion.return_value = {"response": "JSON instruction added"}
        
        response = await ask("Give me JSON", json_mode=True, provider="unsupported")
    ...
    async def test_ask_with_context_and_previous_messages(self, setup_mocks):
        ...
        mock_client.create_completion.return_value = {"response": "Context aware response"}
        ...
        assert "Important context information" in messages[0]["content"]  # Context in system
    ...
    async def test_ask_validation_warnings(self, setup_mocks):
        ...
        mock_client.create_completion.return_value = {"response": "Response despite warnings"}
            ...
            with patch('chuk_llm.api.core.get_client', return_value=mock_client):
                with patch('chuk_llm.api.core.get_config') as mock_get_config:
                    ...
                    with patch('chuk_llm.api.core._get_session_manager', return_value=mock_session_manager):
                        ...
                        mock_session_manager.user_says.assert_called_once_with("Test streaming")
                        mock_session_manager.ai_responds.assert_called_once_with(
                            "Streamed response",
                            model="gpt-4",
                            provider="openai"
    ...
    async def test_streaming_fallback_to_non_streaming(self, mock_client, mock_config):
        ...
        with patch('chuk_llm.api.core.get_current_config', return_value=mock_config):
            with patch('chuk_llm.api.core.get_client', return_value=mock_client):
                with patch('chuk_llm.api.core.get_config') as mock_get_config:
                    mock_config_manager = Mock()
                    mock_config_manager.supports_feature.return_value = False  # No streaming support
                    mock_get_config.return_value = mock_config_manager
...
class TestSessionManagement:
    ...
    async def test_get_session_stats_with_session(self, mock_session_manager):
        ...
        with patch('chuk_llm.api.core._get_session_manager', return_value=mock_session_manager):
            ...
            mock_session_manager.get_stats.assert_called_once_with(include_all_segments=True)
    ...
    async def test_get_session_history_with_session(self, mock_session_manager):
        ...
        with patch('chuk_llm.api.core._get_session_manager', return_value=mock_session_manager):
            ...
            mock_session_manager.get_conversation.assert_called_once_with(include_all_segments=False)
...
class TestUtilityFunctions:
    ...
    async def test_ask_with_tools(self, mock_config):
        ...
        with patch('chuk_llm.api.core.ask') as mock_ask:
            with patch('chuk_llm.api.core.get_current_config', return_value=mock_config):
                with patch('chuk_llm.api.core.get_current_session_id', return_value="session-123"):
    ...
    async def test_ask_json(self):
        ...
        with patch('chuk_llm.api.core.ask') as mock_ask:
            ...
            mock_ask.assert_called_once_with("Give me JSON", json_mode=True, temperature=0.5)
...
class TestMessageBuilding:
    ...
    def test_build_messages_with_tools(self):
        ...
        with patch('chuk_llm.api.core.get_config') as mock_get_config:
            ...
            with patch('chuk_llm.llm.system_prompt_generator.SystemPromptGenerator') as mock_gen:
                mock_generator = Mock()
    ...
    def test_build_messages_no_system_support(self):
        ...
        with patch('chuk_llm.api.core.get_config') as mock_get_config:
            mock_config_manager = Mock()
            mock_config_manager.supports_feature.return_value = False  # No system support
            mock_get_config.return_value = mock_config_manager
...
class TestErrorHandlingAndEdgeCases:
            ...
            with patch('chuk_llm.api.core.get_config') as mock_get_config:
                ...
                with patch('chuk_llm.api.core.ConfigValidator') as mock_validator:
                    ...
                    with patch('chuk_llm.api.core.get_client') as mock_get_client:
                        ...
                        mock_client.create_completion.return_value = {"response": "Fallback response"}
    ...
    async def test_session_tracking_errors_dont_break_functionality(self):
        ...
        mock_config = {"provider": "openai", "model": "gpt-4", "api_key": "sk-test", "api_base": None}
            ...
            with patch('chuk_llm.api.core._get_session_manager', return_value=mock_session_manager):
                with patch('chuk_llm.api.core.get_config') as mock_get_config:
                    ...
                    with patch('chuk_llm.api.core.ConfigValidator') as mock_validator:
                        ...
                        with patch('chuk_llm.api.core.get_client') as mock_get_client:
                            mock_client = AsyncMock()
                            mock_client.create_completion.return_value = {"response": "Success despite session errors"}
                            mock_get_client.return_value = mock_client
